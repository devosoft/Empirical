...
private:
  ...
  bool GenerateCPP() {
    ...
    std::stringstream ss;
    ss << "/**************************************************************************\n"
          " * This file was autogenerated by Emplex on "
       << std::format("{:%B %d, %Y}", std::chrono::system_clock::now()) << ".\n"
          " * You may freely use, modify, and distribute this code under the terms of\n"
          " * the MIT Public License (see https://opensource.org/licenses/MIT).\n"
          " * Emplex was developed at Michigan State University by Dr. Charles Ofria.\n"
          " * To use Emplex, go to https://www.cse.msu.edu/~cse450/Emplex.html\n"
          " **************************************************************************/\n"
          "\n";
    file.Write(ss);
    output_text.Clear();
  ...
  void LoadExampleLexer() {
    ...
    AddTableRow("LIT_INTEGER", "[0-9]+");
    AddTableRow("LIT_FLOAT",   "([0-9]+\\.[0-9]*)|(\\.[0-9]+)");
  ...
  void UpdateIntro(emp::String mode) {
    ...
    if (mode == "home") {
      ...
      static emp::String trigger_text =
        MakeTrigger("<b>load an example</b>", [this](){ LoadExampleLexer(); });
      ...
      intro_div << HeadingName("Regular Expressions") <<
        ...
        "<tr><th>^</th>      <td>Match the beginning of a line (no characters are consumed)</td>"
                            "<td><code>^[ \\t]+</code></td>"
                            "<td>Match indentation whitespace at the start of a line.</td>\n"
        "<tr><th>$</th>      <td>Match the end of a line (no characters are consumed)</td>"
                            "<td><code>[ \\t]+$</code></td>"
                            "<td>Match excess whitespace at the end of a line.</td>\n"
      ...
      intro_div << HeadingName("About") <<
        ...
        << MakeLink("Emscripten", "https://emscripten.org/") << " LLVM compiler. "
        "The name is short for 'Empirical Lexer'.</p>"
        ...
        "<li>Follow up with Empala, a simple parser generator!</li>\n"
        "</ul>\n"
        "<br><br>"
        ;
    }
