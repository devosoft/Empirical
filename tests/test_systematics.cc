...
emp::World_file AddDominantFile(WORLD_TYPE & world){
    ...
    std::function<int(void)> dom_mut_count = [&world](){
      return CountMuts(world.GetGenotypeAt(0));
    };
    std::function<int(void)> dom_del_step = [&world](){
      return CountDeleteriousSteps(world.GetGenotypeAt(0));
    };
    std::function<size_t(void)> dom_phen_vol = [&world](){
      return CountPhenotypeChanges(world.GetGenotypeAt(0));
    };
    std::function<size_t(void)> dom_unique_phen = [&world](){
      return CountUniquePhenotypes(world.GetGenotypeAt(0));
    };
...
TEST_CASE("Run world", "[evo]") {
  ...
  emp::Random random;
  emp::World<org_t, data_t> world(random, "AvidaWorld");
  ...
  record_fit_sig.AddAction([&world](size_t pos, double fit){
    world.GetGenotypeAt(pos)->GetData().RecordFitness(fit);
  });

  record_phen_sig.AddAction([&world](size_t pos, emp::vector<double> phen){
    world.GetGenotypeAt(pos)->GetData().RecordPhenotype(phen);
  });

  world.OnOrgPlacement([&last_mutation, &world](size_t pos){
    world.GetGenotypeAt(pos)->GetData().RecordMutation(last_mutation);
  });
