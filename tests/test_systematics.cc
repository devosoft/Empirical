...
//   emp::Ptr<emp::Systematics<int, int, emp::mut_landscape_info<int> >> sys(true, true, true);
...
//     dynamic_cast<emp::Ptr<systematics_t>>(world.GetSystematics(0))->GetTaxonAt(pos)->GetData().RecordFitness(fit);
//   });

//   record_phen_sig.AddAction([&world](size_t pos, emp::vector<double> phen){
//     dynamic_cast<emp::Ptr<systematics_t>>(world.GetSystematics(0))->GetTaxonAt(pos)->GetData().RecordPhenotype(phen);
//   });

//   world.OnOrgPlacement([&last_mutation, &world](size_t pos){
//     dynamic_cast<emp::Ptr<systematics_t>>(world.GetSystematics(0))->GetTaxonAt(pos)->GetData().RecordMutation(last_mutation);
//   });
