...
namespace emp {
  ...
  //Handles user-defined JSON_TYPE
  template<typename C, class = typename C::value_type>
  ...
  //until a non-array type is found.
  template<typename T>
  void pass_array_to_javascript(emp::vector<emp::vector<T> > \
		values, emp::vector<int> recursive_el = emp::vector<int>()) {
  ...
  //We can handle strings in a similar way
  void pass_vector_to_cpp(emp::vector<std::string> & arr, \
			   bool recurse = false) {
  ...
  //We can handle nested arrays through recursive calls on chunks of them
  template <typename T>
    void pass_vector_to_cpp(emp::vector<emp::vector<T> > & arr,\
			   bool recurse = false) {
    ...
    if (recurse == 0) {
      EM_ASM({emp_i.__temp_array = [emp_i.__outgoing_array];      console.log(emp_i.__outgoing_array);});
    } else{
      //This is a little wasteful of space, but the alternatives are
      //surprisingly ugly
      EM_ASM({emp_i.__temp_array.push(emp_i.__outgoing_array);});
