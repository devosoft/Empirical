...
//
//
//  The first arg template is ORG to identify the type of organism in the population.
//  The remaining args can be any number of MANAGERS.
//
...
namespace evo {
  ...
  // Main world class...
  template <typename ORG, typename... MANAGERS>
  class World : public NextWorldManager< ORG, WorldManagers<ORG,MANAGERS...> > {
  public:
    // Some useful types...
    using fit_fun_t = std::function<double(ORG*)>;        // Type for fitness functions
    using dist_fun_t = std::function<double(ORG*,ORG*)>;  // Type for distance functions
    using iterator_t = PopulationIterator<popM_t>;        // Type for iterators through population
  ...
  public:
    World(emp::Random * r_ptr, const std::string & w_name=UniqueName("emp::evo::World"))
      ...
      , callbacks(w_name)
    {
      World_Base::random_ptr = r_ptr;
      World_Base::random_owner = false;
      World_Base::world_name = w_name;
    ...
    World(int seed=-1, const std::string & w_name=UniqueName("emp::evo::World"))
      : World(new Random(seed), w_name) { World_Base::random_owner = true; }
