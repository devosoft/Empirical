...
namespace evo {
  ...
  class PopulationManager_Base : public NextWorldManager<ORG, WM> {
  protected:
    using ptr_t = ORG *;
    using pop_t = emp::vector<ptr_t>;
    using this_t = PopulationManager_Base<ORG,WM>;
    using parent_t = : NextWorldManager<ORG, WM>;
  ...
  private:
    ...
    class Proxy {
    private:
      PopulationManager_Base<ORG,WM> & popM;  // Which pop manager is this proxy from?
      size_t id;                              // Which position does it represent?
    public:
      Proxy(PopulationManager_Base<ORG,WM> & _p, size_t _id) : popM(_p), id(_id) { ; }
  ...
  public:
    PopulationManager_Base() : num_orgs(0) { ; }
    ...
    using iterator_t = PopulationIterator<PopulationManager_Base<ORG,WM> >;

    size_t GetPopSize() const { return pop.size(); }
    ...
    double CalcFitness(size_t id, const std::function<double(ORG*)> & fit_fun) const {
      return parent_t::CalcFitness(id, pop[id], fit_fun);
    }
    void CalcFitnessAll(const std::function<double(ORG*)> & fit_fun) const {
      for (size_t id = 0; id < pop.size(); id++) parent_t::CalcFitness(id, pop[id], fit_fun);
    }

    // AddOrgAt, AddOrgAppend, and SetOrgs are only ways new organisms enter pop (others use these)
    ...
    void Clear() {
      ...
      ClearPopFitness();                // Clear the fitness manager cache.
      num_orgs = 0;
    }
    ...
    void DoBottleneck(const size_t new_size, bool choose_random=true) {
      ...
      ClearPopFitness();  // Everyone is either removed or in the wrong place!
    }

    // --- PRINTING ---

    void PrintPop(std::function<std::string(ORG*)> string_fun, std::ostream & os = std::cout,
    ...
    void PrintPop(std::ostream & os = std::cout, std::string empty="X", std::string spacer=" ") {
  ...
  class PopulationManager_Plugin : public PopulationManager_Base<ORG, WM> {
  protected:
    using pm_base_t = PopulationManager_Base<ORG,WM>
    using base_t = World_Base<ORG,WM>;
    using base_t::world_name;
  ...
  public:
    ...
    void Clear() {
      ...
      else pm_base_t::Clear();  // If no actions are linked to sig_clear, use default.
    }
  ...
  template <typename ORG, typename WM>
  class PopulationManager_EA : public PopulationManager_Base<ORG,WM> {
  protected:
    using pm_base_t = PopulationManager_Base<ORG,FIT_MANAGER>;

    emp::vector<ORG *> next_pop;

  public:
    PopulationManager_EA() { ; }
    ~PopulationManager_EA() { pm_base_t::Clear(); ClearNext(); }
    ...
    void Update() {
      pm_base_t::SetOrgs(next_pop);  // Move over the next generation.
  ...
  template <typename ORG, typename PM>
  ...
  public:
    ...
    size_t AddOrgBirth(ORG * new_org, size_t parent_pos) {
      if (pm_base_t::GetSize() >= max_size) {
        pm_base_t::DoBottleneck(bottleneck_size);
        ++num_bottlenecks;
      }
      return pm_base_t::AddOrgAppend(new_org);
    }
  };

  template <typename ORG, typename PM>
  class PopulationManager_Grid : public PopulationManager_Base<ORG,PM> {
  protected:
    using pm_base_t = PopulationManager_Base<ORG,FIT_MANAGER>;
    using base_t = World_Base<ORG,PM>;
  ...
  public:
    ...
    void ConfigPop(size_t w, size_t h) { width = w; height = h; pm_base_t::Resize(width*height); }

    // Injected orgs go into a random position.
    size_t AddOrg(ORG * new_org) {
      emp::vector<size_t> empty_spots = pm_base_t::GetEmptyPopIDs();
      const size_t pos = (empty_spots.size()) ?
        empty_spots[ random_ptr->GetUInt(empty_spots.size()) ] :
        random_ptr->GetUInt(pm_base_t::GetSize());
      return pm_base_t::AddOrgAt(new_org, pos);
    }

    // Newly born orgs go next to their parents.
    size_t AddOrgBirth(ORG * new_org, size_t parent_pos) {
      return pm_base_t::AddOrgAt(new_org, GetRandomNeighbor(parent_pos));
    }
    ...
    void PrintPop(std::function<std::string(ORG*)> string_fun, std::ostream& os=std::cout,
      ...
      for (size_t y=0; y < height; y++) {
        for (size_t x = 0; x < width; x++) {
          ORG * org = pm_base_t::GetOrg(x+y*width);
    ...
    void PrintPop(std::ostream& os=std::cout, const std::string & empty="X", std::string spacer=" ") {
      PrintPop( [](ORG * org){return emp::to_string(*org);}, os, empty, spacer);
    }
  };
  ...
  public:
    ...
    using pm_base_t = PopulationManager_Base<ORG,PM>;
    using pm_base_t::pop;
    using pm_base_t::num_orgs;
