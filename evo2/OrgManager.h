...
//    template <typename ORG> auto org_to_genome_t(emp_int_decoy(ORG::genome)) -> decltype(ORG::genome);
  ...
  class OrgManager_Base : public NextWorldManager<ORG, WM> {
  protected:
    using ptr_t = ORG *;
  ...
  class OrgManager_Direct : public OrgManager_Base<ORG,WM> {
  protected:
  public:
    OrgManager_Direct() { ; }
    ~OrgManager_Direct() { ; }
  ...
  template <typename ORG, typename WM>
  class OrgManager_Dynamic : public OrgManager_Base<ORG,WM> {
  protected:
