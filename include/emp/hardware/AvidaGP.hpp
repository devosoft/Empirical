...
namespace emp {
  ...
  public:
    ...
    struct Instruction : public inst_lib_t::InstructionBase {
      ...
      Instruction(size_t _id = 0, size_t a0 = 0, size_t a1 = 0, size_t a2 = 0) : id(_id), args() {
        args[0] = a0;
        args[1] = a1;
        args[2] = a2;
      }
      ...
      size_t GetIndex() const override { return id; }
    };
    ...
    void BypassScope(size_t scope) {
      ...
      if (CurScope() < scope) {
        return;  // Only continue if break is relevant for current scope.
      }
  ...
  public:
    ...
    bool operator<(const this_t & other) const { return genome < other.genome; }
    ...
    void ResetIP() {
      ...
      while (scope_stack.size() > 1) {
        ExitScope();  // Forcibly exit all scopes except root.
      }
  ...
  template <typename HARDWARE>
  size_t AvidaCPU_Base<HARDWARE>::InstScope(
    const typename AvidaCPU_Base<HARDWARE>::inst_t & inst) const {
  ...
  void AvidaCPU_Base<HARDWARE>::PrintGenome(std::ostream & os) const {
    ...
    for (const inst_t & inst : genome) {
      ...
      if (new_scope) {
        ...
        if (new_scope < cur_scope) { cur_scope = new_scope - 1; }
      }
  ...
  size_t AvidaCPU_Base<HARDWARE>::PredictNextInst() const {
    ...
    if (CurScopeType() == ScopeType::LOOP) { return scope_stack.back().start_pos; }
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_AVIDA_GP_HPP_GUARD
