...
namespace emp {
  ...
  /// instructions.
  /// By default, each CPU features four heads, two stacks, multiple registers, and
  /// a circular genome.
  ...
  public:
    ...
    bool are_regs_expanded = false;  ///< Flag signaling if the number of registers have
                                     ///< been expanded to accommodate the number of NOP
    ...
    bool Load(std::istream & input) {
      ...
      if (file.GetNumLines() == 0) {
        emp::notify::Error("VirtualCPU trying to load a genome from an empty stream!");
      }
    ...
    bool LoadFromChars(const std::string & new_genome) {
      ClearGenome();
      for (size_t idx = 0; idx < new_genome.size(); ++idx) { PushInst(new_genome[idx]); }
      nops_need_curated = true;
      return true;
    }
    ...
    inst_t GetDefaultInst() const { return inst_t(GetInstLib()->GetIndex(0), 0); }
    ...
    void PushRandomInst(Random & random, const size_t count = 1) {
      for (size_t i = 0; i < count; i++) { PushInst(GetRandomInst(random)); }
      nops_need_curated = true;
    }
    ...
    void ResetCooldown() { cooldown_timer = 0; }
    ...
    void ResetFH() { flow_head = 0; }  ///< Move flow head to beginning of the genome.
    ...
    void ResetModdedHead(size_t head_idx) {
      ...
      if (modded_idx == 0) {
        SetIP(0);
      } else if (modded_idx == 1) {
        SetRH(0);
      } else if (modded_idx == 2) {
        SetWH(0);
      } else if (modded_idx == 3) {
        SetFH(0);
      }
    }
    ...
    void ResetMemory() {
      ...
      for (size_t i = 0; i < num_regs; i++) { regs[i] = (data_t) i; }
      for (size_t i = 0; i < NUM_STACKS; ++i) { stacks[i].resize(0); }
      active_stack_idx = 0;
    }
    ...
    void CurateNops() {
      ...
      if (label_inst_present && (genome_working[genome_working.size() - 1].id == label_inst_id)) {
      ...
      for (auto it = genome_working.rbegin() + 1; it != genome_working.rend(); ++it) {
        if (emp::Has(nop_id_map, (it - 1)->id)) {
          ...
          std::copy((it - 1)->nop_vec.begin(), (it - 1)->nop_vec.end(), it->nop_vec.begin() + 1);
        }
      }
    ...
    nop_vec_t GetComplementNopSequence(const nop_vec_t & nop_vec) {
      nop_vec_t res_vec;
      for (size_t nop : nop_vec) { res_vec.push_back(GetComplementNop(nop)); }
      return res_vec;
    }
    ...
    bool CheckIfLastCopied(const nop_vec_t & label) {
      ...
      for (auto copied_it = copied_inst_id_vec.rbegin(); copied_it != copied_inst_id_vec.rend();
           copied_it++) {
        if (*copied_it != label[idx]) { return false; }
        idx--;
        if (idx < 0) { break; }
      }
      return true;
    ...
    /// @param start_idx Position in the genome to start the search
    ...
    /// @param search_vec The sequence of NOP instructions to search for
    /// @param start_idx Position in the genome to start the search
    size_t FindNopSequence(const nop_vec_t & search_vec, size_t start_idx, bool reverse = false) {
    ...
    /// start of the genome
    /// @param reverse If true, traverse the genome backward. If false, traverse forward
    size_t FindNopSequence(const nop_vec_t & search_vec, bool start_local, bool reverse = false) {
    ...
    void StackPush(size_t reg_idx) { stacks[active_stack_idx].push_back(regs[reg_idx]); }
    ...
    void PrintDetails(std::ostream & os = std::cout) {
      os << "IP: " << inst_ptr;
      os << " RH: " << read_head;
      os << " WH: " << write_head;
      os << " FH: " << flow_head;
      os << "(nops: " << num_nops << "; regs: " << num_regs << ")" << std::endl;
      for (size_t reg_idx = 0; reg_idx < regs.size(); ++reg_idx) {
        os << "[" << reg_idx << "] " << regs[reg_idx] << std::endl;
      }
    }
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_VIRTUAL_CPU_HPP_GUARD
