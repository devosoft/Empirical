  ...
  class VirtualCPU{
    public:
      ...
      struct Instruction {
        ...
        Instruction(size_t _id=0, emp::vector<size_t> _nop_vec = {})
          : id(_id), args(), nop_vec(_nop_vec) { }
        ...
        void Set(size_t _id, size_t _a0=0, size_t _a1=0, size_t _a2=0, 
          ...
          { id = _id; args[0] = _a0; args[1] = _a1; args[2] = _a2; nop_vec=_nop_vec;}
      };
      ...
      void AdvanceIP(size_t steps=1){
        inst_ptr += steps;
        while(inst_ptr >= genome_working.size()) inst_ptr -= genome_working.size();
      }
      ...
      bool CheckIfLastCopied(const nop_vec_t& label){
        if(label.size() > copied_inst_id_vec.size())
          return false;
