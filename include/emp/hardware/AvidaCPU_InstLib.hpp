  ...
  struct AvidaCPU_InstLib : public InstLib<HARDWARE_T, ARG_T, ARG_COUNT> {
    ...
    static void Inst_Div(hardware_t & hw, const inst_t & inst) {
      ...
      if (denom == 0.0) {
        ...
        hw.regs[inst.args[2]] = hw.regs[inst.args[0]] / denom;
      }
    }
    ...
    static void Inst_Mod(hardware_t & hw, const inst_t & inst) {
      ...
      if (base == 0.0) {
        ...
        hw.regs[inst.args[2]] = emp::Mod(hw.regs[inst.args[0]], base);
      }
    }
    ...
    static void Inst_If(hardware_t & hw, const inst_t & inst) {  // args[0] = test, args[1] = scope
      if (hw.UpdateScope(inst.args[1]) == false) {
        return;  // If previous scope is unfinished, stop!
      }
      if (hw.regs[inst.args[0]] == 0.0) {
        hw.BypassScope(inst.args[1]);  // If test fails, move to scope end.
      }
    }
    ...
    static void Inst_Countdown(
      hardware_t & hw,
      const inst_t & inst) {  // Same as while, but auto-decrements test each loop.
      // UpdateScope returns false if previous scope isn't finished (e.g., while needs to loop)
      if (hw.UpdateScope(inst.args[1], ScopeType::LOOP) == false) { return; }
      if (hw.regs[inst.args[0]] == 0.0) {
        hw.BypassScope(inst.args[1]);  // If test fails, move to scope end.
    ...
    static void Inst_Define(hardware_t & hw, const inst_t & inst) {
      if (hw.UpdateScope(inst.args[1]) == false) {
        return;  // Update which scope we are in.
      }
    ...
    static void Inst_Call(hardware_t & hw, const inst_t & inst) {
      ...
      if (def_pos >= hw.genome.size() ||
          hw.GetScopeType(hw.genome[def_pos].id) != ScopeType::FUNCTION) {
        return;
      }
    ...
    static const this_t & DefaultInstLib() {
      ...
      if (inst_lib.GetSize() == 0) {
        ...
        inst_lib.AddInst("If",
                         Inst_If,
                         2,
                         "If reg Arg1 != 0, scope -> Arg2; else skip scope",
                         ScopeType::BASIC,
                         1);
        inst_lib.AddInst("While",
                         Inst_While,
                         2,
                         "Until reg Arg1 != 0, repeat scope Arg2; else skip",
                         ScopeType::LOOP,
                         1);
        inst_lib.AddInst("Countdown",
                         Inst_Countdown,
                         2,
                         "Countdown reg Arg1 to zero; scope to Arg2",
                         ScopeType::LOOP,
                         1);
        ...
        inst_lib.AddInst("Define",
                         Inst_Define,
                         2,
                         "Build function Arg1 in scope Arg2",
                         ScopeType::FUNCTION,
                         1);
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_AVIDA_CPU_INST_LIB_HPP_GUARD
