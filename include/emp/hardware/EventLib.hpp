...
namespace emp {
  ...
  public:
    ...
    using fun_t =
      std::function<void(hardware_t &,
                         const event_t &)>;  //< Function type alias for event handler functions.
    using properties_t = std::unordered_set<std::string>;  //< Type for event definition properties.
    using fun_set_t =
      FunctionSet<void(hardware_t &, const event_t &)>;  //< Type for event dispatch function sets.
    ...
    struct EventDef {
      ...
      EventDef(const std::string & _n,
               fun_t _handler,
               const std::string & _d,
               const properties_t & _properties)
        : name(_n), handler(_handler), desc(_d), properties(_properties), dispatch_funs() {
        ;
      }
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_EVENT_LIB_HPP_GUARD
