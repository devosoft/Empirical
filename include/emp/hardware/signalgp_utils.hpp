...
namespace emp {
  ...
  BitSet<TAG_WIDTH> GenRandSignalGPTag(emp::Random & rnd,
                                       const emp::vector < BitSet <
                                         TAG_WIDTH >> & unique_from = emp::vector < BitSet <
                                                                      TAG_WIDTH >>()) {
#ifndef DOXYGEN_SHOULD_SKIP_THIS
    using tag_t = BitSet<TAG_WIDTH>;
    emp_assert(
      unique_from.size() < emp::Pow2(TAG_WIDTH),
      "Tag width is not large enough to be able to guarantee requested number of unique tags");
  ...
  template <size_t TAG_WIDTH>
    emp::vector < BitSet < TAG_WIDTH >>
    GenRandSignalGPTags(emp::Random & rnd,
                        size_t count,
                        bool guarantee_unique = false,
                        const emp::vector < BitSet <
                          TAG_WIDTH >> & unique_from = emp::vector < BitSet < TAG_WIDTH >>()) {
    using tag_t = BitSet<TAG_WIDTH>;
    emp_assert(
      !guarantee_unique || (unique_from.size() + count <= emp::Pow2(TAG_WIDTH)),
      "Tag width is not large enough to be able to guarantee requested number of unique tags");
  ...
  typename EventDrivenGP_AW<TAG_WIDTH, TRAIT_T, MATCHBIN_T>::Instruction GenRandSignalGPInst(
    emp::Random & rnd,
    ...
    emp_assert(
      ...
      "Minimum argument value must be less than maximum argument value to generate a number between the two.");
  ...
  template <size_t TAG_WIDTH,
            ...
            typename MATCHBIN_T = emp::MatchBin<size_t,
                                                ...
                                                emp::RankedSelector<std::ratio<1, 2>>,
                                                emp::MultiplicativeCountdownRegulator<>>>
  typename EventDrivenGP_AW<TAG_WIDTH, TRAIT_T, MATCHBIN_T>::Function GenRandSignalGPFunction(
    emp::Random & rnd,
    const emp::InstLib<EventDrivenGP_AW<TAG_WIDTH, TRAIT_T, MATCHBIN_T>> & inst_lib,
    ...
    for (size_t i = 0; i < inst_cnt; ++i) {
      new_fun.PushInst(emp::GenRandSignalGPInst(rnd, inst_lib, min_arg_val, max_arg_val));
    }
    return new_fun;
  }
  ...
  template <size_t TAG_WIDTH,
            ...
            typename MATCHBIN_T = emp::MatchBin<size_t,
                                                ...
                                                emp::RankedSelector<std::ratio<1, 2>>,
                                                emp::MultiplicativeCountdownRegulator<>>>
  typename EventDrivenGP_AW<TAG_WIDTH, TRAIT_T, MATCHBIN_T>::Program GenRandSignalGPProgram(
    emp::Random & rnd,
    const emp::InstLib<EventDrivenGP_AW<TAG_WIDTH, TRAIT_T, MATCHBIN_T>> & inst_lib,
    ...
    for (size_t f = 0; f < fun_cnt; ++f) {
      program.PushFunction(emp::GenRandSignalGPFunction(rnd,
                                                        inst_lib,
                                                        min_fun_len,
                                                        max_fun_len,
                                                        min_arg_val,
  ...
  public:
    ...
    // - Hardware aliases
    using hardware_t =
      EventDrivenGP_AW<TAG_WIDTH, TRAIT_T, MATCHBIN_T>;  ///< SignalGP hardware type
    ...
    using mutator_fun_t =
      std::function<size_t(program_t &, emp::Random &)>;  ///< Mutator function type
    ...
    struct MutatorDef {
      ...
      size_t
        last_mut_cnt;  ///< Number of mutations caused by this mutator on the last application of this mutator
  ...
  protected:
    ...
    std::map<std::string, size_t>
      param_name_map;  ///< Associate parameter names with location in param_lib

    emp::vector<MutatorDef> mutator_lib;  ///< To hold added mutators
    std::map<std::string, size_t>
      mutator_name_map;  ///< Associate mutator name with location in mutator_lib

    // -- General SignalGP program constraints --
    // Program constraints
    size_t
      prog_min_func_cnt;  ///< Minimum number of functions mutations are allowed to reduce a SignalGP program to.
    size_t
      prog_max_func_cnt;  ///< Maximum number of functions a mutated SignalGP program can grow to.
    ...
    size_t
      prog_max_total_len;  ///< Maximum number of *total* instructions a SignalGP program can grow to.
    ...
    size_t
      arg_sub__per_arg__id;  ///< Rate ID for: Rate to apply substitutions to instruction arguments.

    size_t
      inst_sub__per_inst__id;  ///< Rate ID for: Per-instruction rate to apply instruction substitutions.
    size_t
      inst_ins__per_inst__id;  ///< Rate ID for: Per-instruction rate to apply instruction insertions.
    size_t
      inst_del__per_inst__id;  ///< Rate ID for: Per-instruction rate to apply instruction deletions.

    size_t slip__per_func__id;  ///< Rate ID for: Per-function rate to apply slip-mutations.
    size_t
      func_dup__per_func__id;  ///< Rate ID for: Per-function rate to apply function duplications.
    ...
    MutatorDef & GetMutator(size_t id) { return mutator_lib[id]; }

    /// Get a const reference to the mutator specified by id.
    const MutatorDef & GetConstMutator(size_t id) const { return mutator_lib[id]; }
  ...
  public:
    SignalGPMutator(size_t _PROG_MIN_FUNC_CNT  = 1,
      ...
      // Setup default rates.
      arg_sub__per_arg__id = AddParam("ARG_SUB__PER_ARG",
                                      0.005,
                                      "Rate to apply substitutions to instruction arguments.");
      inst_sub__per_inst__id =
        AddParam("INST_SUB__PER_INST",
                 0.005,
                 "Per-instruction rate to apply instruction substitutions. ");
      inst_ins__per_inst__id = AddParam("INST_INS__PER_INST",
                                        0.005,
                                        "Per-instruction rate to apply instruction insertions.");
      inst_del__per_inst__id = AddParam("INST_DEL__PER_INST",
                                        0.005,
                                        "Per-instruction rate to apply instruction deletions.");
      slip__per_func__id =
        AddParam("SLIP__PER_FUNC", 0.05, "Per-function rate to apply slip-mutations.");
      func_dup__per_func__id =
        AddParam("FUNC_DUP__PER_FUNC", 0.05, "Per-function rate to apply function duplications.");
      func_del__per_func__id =
        AddParam("FUNC_DEL__PER_FUNC", 0.05, "Per-function rate to apply function deletions.");
      tag_bit_flip__per_bit__id =
        AddParam("TAG_BIT_FLIP__PER_BIT", 0.005, "Per-bit rate to apply tag bit flips. ");
    ...
    double GetParam(const std::string & param_name) const {
      return GetParam(GetParamID(param_name));
    }
    ...
    void AddMutator(const std::string & name,
                    const mutator_fun_t & mut_fun,
    ...
    void ResetMutators() {
      ...
      AddMutator(
        "FuncDup",
        [this](program_t & p, emp::Random & r) { return this->DefaultMutator_FuncDup(p, r); },
        "Default mutator. Whole-function duplications applied at a per-function rate.");
      AddMutator(
        "FuncDel",
        [this](program_t & p, emp::Random & r) { return this->DefaultMutator_FuncDel(p, r); },
        "Default mutator. Whole-function deletions applied at a per-function rate.");
      AddMutator(
        "FuncTag",
        [this](program_t & p, emp::Random & r) { return this->DefaultMutator_FuncTag(p, r); },
        "Default mutator. Function tag mutations applied at a per-bit rate.");
      AddMutator(
        "Slip",
        [this](program_t & p, emp::Random & r) { return this->DefaultMutator_Slip(p, r); },
        "Default mutator. Slip mutations (multi-instruction sequence duplication/deletions) applied at a per-function rate.");
      AddMutator(
        "Subs",
        [this](program_t & p, emp::Random & r) { return this->DefaultMutator_Subs(p, r); },
        "Default mutator. Single-instruction substitutions applied at a per-instruction rate, argument substitutions applied at a per-argument rate, and instruction tag mutations applied at a per-bit rate.");
      AddMutator(
        "InstInDels",
        [this](program_t & p, emp::Random & r) { return this->DefaultMutator_InstInDels(p, r); },
        "Default mutator. Single-instruction insertions and deletions applied at a per-instruction rate.");
    }

    /// Return the description associated with the mutator associated with name.
    const std::string & GetMutatorDesc(const std::string & name) const {
      return GetConstMutator(GetMutatorID(name)).desc;
    }

    /// Return the last mutation count from the mutator associated with name.
    size_t GetLastMutationCnt(const std::string & name) const {
      return GetConstMutator(GetMutatorID(name)).last_mut_cnt;
    }
    ...
    size_t DefaultMutator_FuncDup(program_t & program, emp::Random & rnd) {
      ...
      for (size_t fID = 0; fID < orig_func_wall; ++fID) {
        ...
        if (fID < orig_func_wall && rnd.P(FUNC_DUP__PER_FUNC()) &&
    ...
    size_t DefaultMutator_FuncDel(program_t & program, emp::Random & rnd) {
      ...
      for (int fID = 0; fID < (int) program.GetSize(); ++fID) {
        ...
        if (rnd.P(FUNC_DEL__PER_FUNC()) && program.GetSize() > GetProgMinFuncCnt()) {
    ...
    size_t DefaultMutator_Slip(program_t & program, emp::Random & rnd) {
      ...
      for (size_t fID = 0; fID < program.GetSize(); ++fID) {
        if (rnd.P(SLIP__PER_FUNC())) {
          ...
          if (dup && (expected_prog_len + (size_t) dup_size <= GetProgMaxTotalLen()) &&
            ...
            for (size_t i = 0; i < new_size; ++i) {
              if (i < end) {
                ...
                new_fun.PushInst(program[fID][i - (size_t) dup_size]);
              }
            }
            program[fID] = new_fun;
            ++mut_cnt;
            expected_prog_len += (size_t) dup_size;
          } else if (del && (program[fID].GetSize() - (size_t) del_size) >= GetProgMinFuncLen()) {
            // Delete end:begin!
            fun_t new_fun(program[fID].GetAffinity());
            for (size_t i = 0; i < end; ++i) { new_fun.PushInst(program[fID][i]); }
    ...
    size_t DefaultMutator_InstInDels(program_t & program, emp::Random & rnd) {
      ...
      for (size_t fID = 0; fID < program.GetSize(); ++fID) {
        ...
        while (rhead < program[fID].GetSize()) {
          // Should we insert?
          if (ins_locs.size() > 0) {
            if (rhead >= ins_locs.back() && expected_func_len < GetProgMaxFuncLen() &&
                expected_prog_len < GetProgMaxTotalLen()) {
              // Insert a random instruction.
              new_fun.PushInst(GenRandSignalGPInst(rnd,
                                                   *(program.GetInstLib()),
                                                   GetProgMinArgVal(),
                                                   GetProgMaxArgVal()));
              ++mut_cnt;
          ...
          if (rnd.P(INST_DEL__PER_INST()) && expected_func_len > GetProgMinFuncLen()) {
  ...
  class SignalGPMutatorFacade
    : public SignalGPMutator<Hardware::affinity_width,
                             typename Hardware::trait_t,
                             typename Hardware::matchbin_t> {};
}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_HARDWARE_SIGNALGP_UTILS_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: rnd uset signalgp dels locs amlalejini cnt rhead
