...
namespace emp {
  ...
  public:
    ...
    bool operator!=(const BitSorter & other) const { return compare_set != other.compare_set; }
    ...
    bits_t & operator[](size_t idx) { return compare_set[idx]; }

    bits_t GetBits(size_t idx) { return compare_set[idx]; }
    ...
    bool AddCompare(size_t id1, size_t id2) {
      ...
      if (id1 == id2) {
        return false;  // If ids are the same, don't add comparator!
      }
    ...
    static bits_t RunCompare(bits_t values, bits_t comparator) {
      ...
      if (((values & comparator) == 0)      // Bits are both 0... no change!
          ...
          || ((values & comparator) <
              (~values & comparator))  // Bits are already in correct (0 - 1) order!
      ) {
        return values;
      }
    ...
    size_t CountSortable(size_t num_bits = 16) const {
      ...
      for (bits_t vals = 0; vals < limit; vals++) {
        ...
        if ((svals & (svals + 1)) == 0) {
          count++;  // Sorted!  (a sorted network should have all one bits to the right)
        }
      }
      return count;
    }
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_BIT_SORTER_HPP_GUARD
