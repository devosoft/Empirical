...
namespace emp {
  ...
  struct VirtualCPU_InstLib : public InstLib<HARDWARE_T, ARG_T, ARG_COUNT> {
    ...
    static void Inst_Swap_Stack(hardware_t & hw, const inst_t & /*inst*/) { hw.StackSwap(); }
    ...
    static void Inst_H_Search(hardware_t & hw, const inst_t & inst) {
      ...
      if (inst.nop_vec.size() == 0 || res == hw.inst_ptr) {
        ...
        hw.regs[1] = (res - hw.inst_ptr) > 0 ? res - hw.inst_ptr
                                             : res + hw.genome_working.size() - res + hw.inst_ptr;
    ...
    static void Inst_Mov_Head(hardware_t & hw, const inst_t & inst) {
      if (hw.expanded_nop_args) {
        ...
        if (!inst.nop_vec.empty()) {
          hw.SetModdedHead(inst.nop_vec[0], dest_idx);
        } else {
          hw.SetIP(dest_idx);
        }
      } else {
        if (!inst.nop_vec.empty()) {
          ...
          if (inst.nop_vec[0] % 4 == 0) {
            ...
            hw.SetModdedHead(inst.nop_vec[0], hw.flow_head);
          }
        } else {
          hw.SetIP(hw.flow_head - 1);
        }
    ...
    static void Inst_Get_Head(hardware_t & hw, const inst_t & inst) {
      if (hw.expanded_nop_args) {
        ...
        if (inst.nop_vec.size() < 2) {
          hw.regs[2] = head_val;
        } else {
          hw.regs[inst.nop_vec[1]] = head_val;
        }
      } else {
        if (inst.nop_vec.empty()) {
          hw.regs[2] = hw.inst_ptr;
        } else {
          hw.regs[2] = hw.GetModdedHead(inst.nop_vec[0]);
        }
      }
    }
    ...
    static const this_t & DefaultInstLib() {
      ...
      if (inst_lib.GetSize() == 0) {
        ...
        inst_lib.AddInst("IfNEq",
                         Inst_If_Not_Equal,
                         1,
                         "Skip next inst unless register values match");
        inst_lib.AddInst("IfLess",
                         Inst_If_Less,
                         1,
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_VIRTUAL_CPU_INST_LIB_HPP_GUARD
