 ...
 *
 * @file include/emp/hardware/Genome.hpp
...
#ifndef INCLUDE_EMP_HARDWARE_GENOME_HPP_GUARD
#define INCLUDE_EMP_HARDWARE_GENOME_HPP_GUARD
...
namespace emp {
  ...
  public:
    ...
    bool operator<(const Genome & other) const {
      ...
      while (it1 != SEQUENCE_T::end() && it2 != other.end()) {
        if (*it1 != *it2) {
          return *it1 < *it2;  // The first difference determines result
        }
        ++it1;
        ++it2;
      }
    ...
    size_t Hash() const {
      ...
      for (auto & i : *this) { seed ^= i.id + 0x9e3779b9 + (seed << 6) + (seed >> 2); }
      return seed;
    }
...
#endif  // #ifndef INCLUDE_EMP_HARDWARE_GENOME_HPP_GUARD
