...
namespace emp {
  ...
  public:
    ...
    void PrintSmall(const std::string & title = "", std::ostream & os = std::cout) {
      ...
      os << "  ";
      for (size_t i = 5; i < 6; i--) { os << boardB[i] << " "; }
      os << std::endl;
      os << boardB[6] << "              " << boardA[6] << std::endl;
      os << "  ";
      for (size_t i = 0; i < 6; i++) { os << boardA[i] << " "; }
      os << std::endl;
    }
    ...
    void Print(std::ostream & os = std::cout) {
      ...
      os << "\n"
         << "|                                                 | \n"
         << "|       (" << std::setw(2) << boardB[5] << " ) (" << std::setw(2) << boardB[4]
         << " ) (" << std::setw(2) << boardB[3] << " ) (" << std::setw(2) << boardB[2] << " ) ("
         << std::setw(2) << boardB[1] << " ) (" << std::setw(2) << boardB[0] << " )       |\n";
      os << "v [" << std::setw(2) << boardB[6] << " ]                                     ["
         << std::setw(2) << boardA[6] << " ] ^\n";
      os << "|       (" << std::setw(2) << boardA[0] << " ) (" << std::setw(2) << boardA[1]
         << " ) (" << std::setw(2) << boardA[2] << " ) (" << std::setw(2) << boardA[3] << " ) ("
         << std::setw(2) << boardA[4] << " ) (" << std::setw(2) << boardA[5] << " )       |\n";
      os << "|                                                 |\n"
    ...
    size_t ScoreA() const {
      size_t score = 0;
      for (size_t i = 0; i < 7; i++) { score += boardA[i]; }
      return score;
    }

    size_t ScoreB() const {
      size_t score = 0;
      for (size_t i = 0; i < 7; i++) { score += boardB[i]; }
      return score;
    }

    double GetScore(size_t player) {
      if (player == 0) {
        return (double) ScoreA();
      } else if (player == 1) {
        return (double) ScoreB();
      }
...
#endif  // #ifndef INCLUDE_EMP_GAMES_MANCALA_HPP_GUARD
