 ...
 *
 * @file include/emp/games/Othello.hpp
...
#ifndef INCLUDE_EMP_GAMES_OTHELLO_HPP_GUARD
#define INCLUDE_EMP_GAMES_OTHELLO_HPP_GUARD
...
namespace emp {
  ...
  public:
    ...
    struct Index {
      ...
      void Set(size_t x, size_t y) {
        pos = (x < BOARD_SIZE && y < BOARD_SIZE) ? (x + y * BOARD_SIZE) : NUM_CELLS;
      }
  ...
  public:
    ...
    bool IsValidMove(Player player, Index pos) {
      ...
      if (GetPosOwner(pos) != Player::NONE) {
    ...
    emp::vector<Index> GetFlipList(Player player, Index pos) {
      ...
      for (Facing dir : ALL_DIRECTIONS) {
        ...
        if (!neighbor_pos.IsValid() || GetPosOwner(neighbor_pos) == Player::NONE) {
...
#endif  // #ifndef INCLUDE_EMP_GAMES_OTHELLO_HPP_GUARD
