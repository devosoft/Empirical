...
namespace emp {
  ...
  private:
    ...
    static emp::array<grid_bits_t, NUM_REGIONS> BuildRegionMap() {
      ...
      bit_regions[18] =
        "{  0,  1,  2,  9, 10, 11, 18, 19, 20 }";  // Overlap with 0, 1, 2,  9, 10, 11
      bit_regions[19] =
        "{  3,  4,  5, 12, 13, 14, 21, 22, 23 }";  // Overlap with 0, 1, 2, 12, 13, 14
      bit_regions[20] =
        "{  6,  7,  8, 15, 16, 17, 24, 25, 26 }";  // Overlap with 0, 1, 2, 15, 16, 17
      bit_regions[21] =
        "{ 27, 28, 29, 36, 37, 38, 45, 46, 47 }";  // Overlap with 3, 4, 5,  9, 10, 11
      bit_regions[22] =
        "{ 30, 31, 32, 39, 40, 41, 48, 49, 50 }";  // Overlap with 3, 4, 5, 12, 13, 14
      bit_regions[23] =
        "{ 33, 34, 35, 42, 43, 44, 51, 52, 53 }";  // Overlap with 3, 4, 5, 15, 16, 17
      bit_regions[24] =
        "{ 54, 55, 56, 63, 64, 65, 72, 73, 74 }";  // Overlap with 6, 7, 8,  9, 10, 11
      bit_regions[25] =
        "{ 57, 58, 59, 66, 67, 68, 75, 76, 77 }";  // Overlap with 6, 7, 8, 12, 13, 14
      bit_regions[26] =
        "{ 60, 61, 62, 69, 70, 71, 78, 79, 80 }";  // Overlap with 6, 7, 8, 15, 16, 17
    ...
    static inline const grid_bits_t & BoxMap(size_t id) {
      return RegionMap()[id + NUM_ROWS + NUM_COLS];
    }
    ...
    static emp::array<region_bits_t, NUM_CELLS> BuildCellMemberships() {
      ...
      for (size_t reg_id = 0; reg_id < NUM_REGIONS; ++reg_id) {
        ...
        region.ForEach(
          [&cell_regions, reg_id](size_t cell_id) { cell_regions[cell_id].Set(reg_id); });
      }
    ...
    static inline const grid_bits_t & CellLinks(size_t cell_id) { return CellLinks()[cell_id]; }
  ...
  public:
    ...
    bool ForceSolve(uint8_t cur_state = 1) {
      ...
      while (cur_state <= NUM_STATES) {
        ...
        if (solved) {
          return true;  // If solved, we're done!
        }
    ...
    emp::vector<PuzzleMove> Solve_FindRegionOverlap() {
      ...
      for (auto [r1, r2] : RegionOverlaps()) {
        ...
        if ((overlap & ~is_set).CountOnes() < 2) {
          continue;  // Enough free cells to matter?
        }
        for (uint8_t state = 1; state <= NUM_STATES; ++state) {
          ...
          if (o1 == o2) {
            continue;  // Neither region can limit the other.
          }
          if (o1) {  // o1 limits o2
        ...
        for (uint8_t state1 = 1; state1 < NUM_STATES - 1; ++state1) {
          ...
          for (uint8_t state2 = state1 + 1; state2 < NUM_STATES; ++state2) {
            ...
            for (uint8_t state3 = state2 + 1; state3 <= NUM_STATES; ++state3) {
              ...
              // Do either of those have OTHER states to block?
              for (uint8_t block_state = 1; block_state <= NUM_STATES; ++block_state) {
                if (block_state == state1 || block_state == state2 || block_state == state3) {
                  continue;
                }
                if (bit_options[block_state].Has(pos1)) {
      ...
      for (uint8_t state = 1; state <= NUM_STATES; ++state) {
        for (size_t region1_id = 0; region1_id < NUM_ROWS + NUM_COLS; ++region1_id) {
          ...
          for (size_t region2_id = region1_id + 1; region2_id % NUM_ROWS; ++region2_id) {
            ...
            grid_bits_t target_cells =
              bit_options[state] & ~region1 & ~region2 & ComboRegion(a_regions | b_regions);
      ...
      for (uint8_t state = 1; state <= NUM_STATES; ++state) {
        for (size_t box_id = NUM_ROWS + NUM_COLS; box_id < NUM_REGIONS; ++box_id) {
          ...
          for (size_t region2_id = 0; region2_id < NUM_ROWS + NUM_COLS; ++region2_id) {
            auto region2 = RegionMap(region2_id) & bit_options[state];
            if (region2.CountOnes() != 2) { continue; }
            if ((region1 & region2).Any()) {
              continue;  // No double-counting of cells.
            }
            ...
            if (aa_regions.Any() && bb_regions.Any()) {
              // For each cross region, remove additional "state" options.
              grid_bits_t target_cells =
                bit_options[state] & ~region1 & ~region2 & ComboRegion(aa_regions | bb_regions);
            ...
            if (ab_regions.Any() && ba_regions.Any()) {
              // For each cross region, remove additional "state" options.
              grid_bits_t target_cells =
                bit_options[state] & ~region1 & ~region2 & ComboRegion(ab_regions | ba_regions);
        ...
        for (size_t region1_id = 0; region1_id < NUM_ROWS + NUM_COLS; ++region1_id) {
          ...
          for (size_t region2_id = region1_id + 1; region2_id % NUM_ROWS; ++region2_id) {
            ...
            for (size_t region3_id = region2_id + 1; region3_id % NUM_ROWS; ++region3_id) {
              ...
              region_bits_t a_regions =
                CellMemberships(cell1a) & CellMemberships(cell2a) & CellMemberships(cell3a);
              region_bits_t b_regions =
                CellMemberships(cell1b) & CellMemberships(cell2b) & CellMemberships(cell3b);
              region_bits_t c_regions =
                CellMemberships(cell1c) & CellMemberships(cell2c) & CellMemberships(cell3c);
          ...
          for (size_t region2_id = region1_id + 1; region2_id % NUM_ROWS; ++region2_id) {
            ...
            for (size_t region3_id = region2_id + 1; region3_id % NUM_ROWS; ++region3_id) {
              ...
              for (size_t region4_id = region3_id + 1; region4_id % NUM_ROWS; ++region4_id) {
                ...
                // If all cell groups don't share at least one region, we can't do a swordfish
                if (a_regions.None() || b_regions.None() || c_regions.None() || d_regions.None()) {
                  continue;
                }

                // For each cross region, remove additional "state" options.
                grid_bits_t target_cells =
                  bit_options[state] & ~region1 & ~region2 & ~region3 & ~region4 &
...
#endif  // #ifndef INCLUDE_EMP_GAMES_SUDOKU_ANALYZER_HPP_GUARD
