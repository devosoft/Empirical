...
namespace emp {
  ...
  struct PuzzleProfile {
    ...
    void AddMoves(size_t level, size_t count) { slices.push_back(Slice{level, count}); }
    ...
    double CalcScore() const {
      double score = 0.0;
      for (Slice slice : slices) { score += slice.level + 1; }
      return score;
    }
    ...
    void Print(std::ostream & out = std::cout) const { out << ToString() << std::endl; }
  };
  ...
  struct GridPuzzleAnalyzer {
    ...
    emp::array<char, NUM_STATES + 1>
      symbols;  // sudoku_symbols = {'-', '1', '2', '3', '4', '5', '6', '7', '8', '9'};
    ...
    uint8_t SymbolToState(char symbol) const { return symbol_map[symbol]; }
    ...
    values_t LoadToArray(std::istream & is, emp::CharSet empty = "-.") {
      ...
      for (size_t cell_id = 0; cell_id < NUM_CELLS; ++cell_id) {
        is >> cur_char;
        if (emp::is_whitespace(cur_char)) {
          --cell_id;
          continue;
        }
        uint8_t state_id = SymbolToState(cur_char);
        if (state_id == UNKNOWN_STATE) {
          if (empty.Has(cur_char)) {
            state_id = UNSET_STATE;
          } else {
            emp::notify::Warning("Unknown puzzle symbol '", cur_char, "'.  Ignoring.");
          }
        }
    ...
    bool Load(std::span<size_t> board) {
      notify::TestError(board.size() != NUM_CELLS,
                        "Attempting to load a board of size ",
                        board.size(),
                        ", but ",
                        NUM_CELLS,
                        " required.");
      ...
      for (size_t i = 0; i < NUM_CELLS; ++i) {
        notify::TestError(board[i] > NUM_STATES,
                          "Attempting to set board state to ",
                          board[i],
                          ", but max state is ",
                          NUM_STATES);
        if (board[i]) {
    ...
    void Clear() {
      ...
      for (auto & val_options : bit_options) { val_options.SetAll(); }
      is_set.Clear();
    }
    ...
    virtual bool Set(size_t cell, uint8_t state) {
      ...
      if (values[cell] == state) {
        return false;  // If state is already set, SKIP!
      }
    ...
    void Block(size_t cell, uint8_t state) { bit_options[state].Clear(cell); }
    ...
    bool MoveProgress(const PuzzleMove & move) const {
      ...
      if (move.type == PuzzleMove::SET_STATE) { return GetValue(move.pos_id) != move.state; }
    ...
    bool Move(const PuzzleMove & move) {
      ...
      switch (move.type) {
        ...
        default:                      emp_assert(false);  // One of the previous move options should have been triggered!
      }
    ...
    bool Move(const emp::vector<PuzzleMove> & moves) {
      for (const auto & move : moves) {
        if (!Move(move)) {
          return false;  // If any moves are invalid, return false.
        }
      }
      return true;
    }
    ...
    emp::String Symbol(uint8_t id) const { return emp::MakeString(symbols[id]); }
    ...
    bool IsUnsolvable() {
      ...
      for (uint8_t state = 1; state <= NUM_STATES; ++state) { has_options |= bit_options[state]; }
    ...
    PuzzleProfile CalcProfile() {
      ...
      while (fun_id < solve_funs.size()) {
        ...
        if (moves.size() > 0 && MoveProgress(moves)) {
          if (!Move(moves)) {
            break;  // Trigger the full set moves; stop if any move fails.
          }
...
#endif  // #ifndef INCLUDE_EMP_GAMES_PUZZLE_HPP_GUARD
