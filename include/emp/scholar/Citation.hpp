...
namespace emp {
  ...
  public:
    ...
    bool HasKeyword(const std::string & kw) const { return keywords.find(kw) != keywords.end(); }
    ...
    Citation & SetTitle(T && val) {
      return SetVal("title", val);
    }

    template <class T>
    Citation & SetJournal(T && val) {
      return SetVal("journal", val);
    }  // If in journal

    template <class T>
    Citation & SetBookTitle(T && val) {
      return SetVal("book_title", val);
    }  // If in book

    template <class T>
    Citation & SetVolume(T && val) {
      return SetVal("volume", val);
    }

    template <class T>
    Citation & SetYear(T && val) {
      return SetVal("year", val);
    }

    template <class T>
    Citation & SetNumber(T && val) {
      return SetVal("number", val);
    }

    template <class T>
    Citation & SetMonth(T && val) {
      return SetVal("month", val);
    }

    template <class T>
    Citation & SetStartPage(T && val) {
      return SetVal("start_page", val);
    }

    template <class T>
    Citation & SetEndPage(T && val) {
      return SetVal("end_page", val);
    }

    template <class T>
    Citation & SetPages(T && p1, T && p2) {
      SetStartPage(p1);
      return SetEndPage(p2);
    }
...
#endif  // #ifndef INCLUDE_EMP_SCHOLAR_CITATION_HPP_GUARD
