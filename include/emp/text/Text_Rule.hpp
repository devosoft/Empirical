...
namespace emp {
  ...
  public:
    ...
    Text_Rule(String open_tag_start,
              char open_tag_end,
              String close_tag,
              String base_type,
              char placeholder = ' ')
      : main_tag(open_tag_start, open_tag_end)
      , base_type(base_type)
      , close_tag(close_tag)
    ...
    void SetID(size_t in_id) {
      emp_assert(in_id < 1'000'000'000, in_id, "Invalid rule ID");
      id = in_id;
    }
    ...
    void PrintDebug(std::ostream & os = std::cout) const {
      ...
      os << " : base_type='" << base_type << "' ; placeholder='" << placeholder << std::endl;
    }
  };

}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_TEXT_TEXT_RULE_HPP_GUARD
