...
namespace emp {
  ...
  protected:
    ...
    void Cleanup() {
      ...
      for (const String & style : unused_styles) { style_map.erase(style); }
    }
  ...
  public:
    ...
    Text(const Text & in)
      : text(in.text), style_map(in.style_map), symbol_layout(in.symbol_layout) {
      CloneEncodings(in);
    }
    ...
    ~Text() {
      for (auto & [e_name, ptr] : encodings) { ptr.Delete(); }
    }
    ...
    [[nodiscard]] std::string_view ViewString(size_t start    = 0,
                                              size_t out_size = String::npos) const {
    ...
    [[nodiscard]] const std::map<size_t, String> & GetSymbols() const { return symbol_layout; }
    ...
    [[nodiscard]] bool HasEncoding(const String & name) const { return emp::Has(encodings, name); }
    ...
    String AddEncoding(EXTRA_Ts &&... args) {
      ...
      emp_assert(!HasEncoding(encoding_name),
                 encoding_name,
    ...
    String ActivateEncoding(const String & name, EXTRA_Ts &&... args) {
      if (HasEncoding(name)) {
        ...
        encoding_ptr = NewPtr<ENCODING_T>(std::forward<EXTRA_Ts>(args)...);
        emp_assert(encoding_ptr->GetName() == name,
                   encoding_ptr->GetName(),
                   name,
    ...
    Text & Append(T && in, EXTRA_Ts &&... in_extra) {
      ...
      if constexpr (std::is_base_of_v<emp::Text, CleanT>) {
        ...
        for (auto [pos, symbol] : in.symbol_layout) { symbol_layout[start_size + pos] = symbol; }
      }
      ...
      if constexpr (sizeof...(EXTRA_Ts) > 0) { Append(std::forward<EXTRA_Ts>(in_extra)...); }
    ...
    Text & AppendAs(const String & encode_name, IN_T && in) {
      if (!HasEncoding(encode_name)) {
        AddEncoding<ENCODING_T>();
      } else {
        SetEncoding(encode_name);
      }
    ...
    template <typename T>
    Text & operator<<(T && in) {
      return Append(std::forward<T>(in));
    }
    ...
    void Resize(size_t new_size) {
      ...
      if (new_size < size()) {
        // Remove styles that go beyond the new size.
        for (auto & [tag, bits] : style_map) {
    ...
    Text & Set(size_t pos, TextCharRef<IS_CONST> in) {
      ...
      for (const String & style : styles) { SetStyle(style, pos); }
      return *this;
    }
    ...
    template <typename... Ts>
    [[nodiscard]] size_t find(Ts &&... in) const {
      return text.find(std::forward<Ts>(in)...);
    }

    template <typename... Ts>
    [[nodiscard]] size_t rfind(Ts &&... in) const {
      return text.rfind(std::forward<Ts>(in)...);
    }

    template <typename... Ts>
    [[nodiscard]] size_t find_first_of(Ts &&... in) const {
      return text.find_first_of(std::forward<Ts>(in)...);
    }

    template <typename... Ts>
    [[nodiscard]] size_t find_first_not_of(Ts &&... in) const {
      return text.find_first_not_of(std::forward<Ts>(in)...);
    }

    template <typename... Ts>
    [[nodiscard]] size_t find_last_of(Ts &&... in) const {
      return text.find_last_of(std::forward<Ts>(in)...);
    }

    template <typename... Ts>
    [[nodiscard]] size_t find_last_not_of(Ts &&... in) const {
      return text.find_last_not_of(std::forward<Ts>(in)...);
    }
    ...
    [[nodiscard]] emp::vector<String> GetStyles(size_t pos = MAX_SIZE_T) const {
      ...
      for (const auto & [name, bits] : style_map) {
        if (pos == MAX_SIZE_T || bits.Has(pos)) { styles.push_back(name); }
      }
      return styles;
    }
    ...
    Text & Clear(const String & style, size_t start, size_t end) {
      ...
      if (it != style_map.end() && it->second.size() > start) {  // If style bits exist...
        if (end > it->second.size()) {
          end = it->second.size();  // ...don't pass text end
        }
    ...
    [[nodiscard]] BitVector GetSymbolPositions() const {
      ...
      for (auto entry : symbol_layout) { out_bits.Set(entry.first); }
      return out_bits;
    }
    ...
    [[nodiscard]] String ToDebugString() {
      ...
      for (auto [name, bits] : style_map) { out += MakeString("      ", bits, " : ", name, "\n"); }
      out += MakeString("      ", GetSymbolPositions(), " : SYMBOLS\n");
      return out;
    }
  ...
  void TextEncoding_None::PrintDebug(std::ostream & os) const { os << "TextEncoding None."; }

}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_TEXT_TEXT_HPP_GUARD
