 ...
 *
 * @file include/emp/deprecated/attrs.hpp
 * @brief TODO.
 */

#pragma once

#ifndef INCLUDE_EMP_DEPRECATED_ATTRS_HPP_GUARD
#define INCLUDE_EMP_DEPRECATED_ATTRS_HPP_GUARD
...
namespace emp { namespace tools {

  struct value_tag {};

  template <typename V1, typename V2>
  struct is_same_attribute : std::is_same<typename V1::attribute_t, typename V2::attribute_t> {};
  ...
  namespace __impl_attr_base {
    ...
    private:
      ...
      static constexpr auto __impl_CallOrGetAttribute(const std::true_type & isCallable,
                                                      T && target,
                                                      U &&... args) {
      ...
      template <class T, class... U>
      static constexpr auto __impl_CallOrGetAttribute(const std::false_type & isCallable,
                                                      T && target,
                                                      U &&... args) {
      ...
      static constexpr struct call_or_get_attribute_t {
        ...
        constexpr decltype(auto) operator()(V && target, U &&... args) const {
          ...
          return __impl_CallOrGetAttribute(
            ...
            std::is_invocable<ValueOfTargetType, decltype(std::forward<U>(args))...>{},
            std::forward<V>(target),
            std::forward<U>(args)...);
        }
      } CallOrGetAttribute{};
    ...
    private:
      ...
      __impl_MapOrElse(const std::true_type &, Pack && pack, M && map, D &&, U &&...) {
        return std::forward<M>(map)(GetAttribute(std::forward<Pack>(pack)));
      }

      template <typename Pack, typename M, typename D, typename... U>
      static constexpr decltype(auto)
      __impl_MapOrElse(const std::false_type &, Pack &&, M &&, D && defaultFunction, U &&... args) {
    ...
    public:
      static constexpr struct map_or_else_t {
        ...
        constexpr decltype(auto) operator()(Pack && pack,
                                            ...
                                            D && defaultFunction,
                                            U &&... args) const {
          return __impl_MapOrElse(has_attribute<std::decay_t<Pack>, attribute_t>{},
      ...
      static constexpr struct get_attribute_or_else_t {
        ...
        constexpr decltype(auto) operator()(Pack && pack,
                                            D && defaultFunction,
                                            U &&... args) const {
          return MapOrElse(std::forward<Pack>(pack),
                           GetAttribute,
                           std::forward<D>(defaultFunction),
                           std::forward<U>(args)...);
        }
    ...
    private:
      ...
      struct Default {
        ...
        constexpr decltype(auto) operator()() const { return std::forward<D>(value); }
      };

    public:
      static constexpr struct get_or_t {
        template <class Pack, class D>
        constexpr decltype(auto) operator()(Pack && pack, D && defaultValue) const {
          return GetOrElse(std::forward<Pack>(pack), Default<D &&>{std::forward<D>(defaultValue)});
        }
      } GetOr{};
    ...
    private:
      ...
      struct FallbackHandler<Fallback> {
        ...
        constexpr FallbackHandler(Fallback fallback) : fallback(std::forward<Fallback>(fallback)) {}
      ...
      struct FallbackHandler<Fallback0, Fallback1, Fallbacks...> {
        ...
        constexpr FallbackHandler(Fallback0 && fallback0,
          ...
          , fallbacks(std::forward<Fallback1>(fallback1), std::forward<Fallbacks>(fallbacks)...) {}

        constexpr decltype(auto) operator()() && {
          return GetAttributeOrElse(std::forward<Fallback0>(fallback), std::move(fallbacks));
        }
      };
    ...
    public:
      static constexpr struct get_attribute_or_get_attribute_in_t {
        ...
        constexpr decltype(auto) operator()(Fallbacks &&... fallbacks) const {
          return FallbackHandler<Fallbacks &&...>{std::forward<Fallbacks>(fallbacks)...}();
        }
      ...
      static constexpr struct get_or_get_in_t {
        template <class... Fallbacks>
        constexpr decltype(auto) operator()(Fallbacks &&... fallbacks) const {
          return Get(FallbackHandler<Fallbacks &&...>{std::forward<Fallbacks>(fallbacks)...}());
        }
      } GetOrGetIn{};
    ...
    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_attribute_t AttrBase<Attr, Value>::GetAttribute;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_t AttrBase<Attr, Value>::Get;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::call_or_get_attribute_t AttrBase<Attr, Value>::
      CallOrGetAttribute;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::map_or_else_t AttrBase<Attr, Value>::MapOrElse;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_attribute_or_else_t AttrBase<Attr, Value>::
      GetAttributeOrElse;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_or_else_t AttrBase<Attr, Value>::GetOrElse;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_or_t AttrBase<Attr, Value>::GetOr;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_attribute_or_get_attribute_in_t
      AttrBase<Attr, Value>::GetAttributeOrGetAttributeIn;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::get_or_get_in_t AttrBase<Attr, Value>::GetOrGetIn;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::set_t AttrBase<Attr, Value>::Set;

    template <typename Attr, template <typename> class Value>
    constexpr typename AttrBase<Attr, Value>::make_t AttrBase<Attr, Value>::Make;
...
#define DEFINE_ATTR(NAME)                                                      \
  ...
  namespace __attrs_impl {
    ...
    protected:
      ...
      template <class... U>
      constexpr AttrsParent(const args_tag &, U &&... args) : T{std::forward<U>(args)}... {}
  ...
  namespace __impl_attrs_reduce {
    ...
    struct AttrsIter<__impl_variadics_type_traits::pack<U>> {
      ...
      static constexpr auto MergeReduce(I && init, R && reducer, A &&... attrs) {
        return std::forward<R>(
          reducer)(std::forward<I>(init),
                   U::attribute_t::GetAttributeOrGetAttributeIn(std::forward<A>(attrs)...));
      }

      template <typename I, typename R, typename... A>
      static constexpr auto Reduce(I && init, R && reducer, A &&... attrs) {
        return std::forward<R>(reducer)(std::forward<I>(init),
      ...
      template <typename F, typename... A>
      static constexpr void Foreach(F && callback, A &&... attrs) {
        std::forward<F>(callback)(U::attribute_t::GetAttribute(std::forward<A>(attrs))...);
      }
    };
    ...
    struct AttrsIter<__impl_variadics_type_traits::pack<U0, U1, U...>> {
      template <typename I, typename R, typename... A>
      static constexpr auto MergeReduce(I && init, R && reducer, A &&... attrs) {
        using new_init_t = decltype(reducer(std::forward<I>(init),
                                            U0::attribute_t::GetAttributeOrGetAttributeIn(
                                              std::forward<A>(attrs)...)));

        return AttrsIter<__impl_variadics_type_traits::pack<U1, U...>>::MergeReduce(
          std::forward<new_init_t>(
            reducer(std::forward<I>(init),
                    U0::attribute_t::GetAttributeOrGetAttributeIn(std::forward<A>(attrs)...))),
      ...
      template <typename F, typename... A>
      static constexpr void MergeForeach(F && callback, A &&... attrs) {
        callback(U0::attribute_t::GetAttributeOrGetAttributeIn(std::forward<A>(attrs)...));
  ...
  constexpr auto MergeReduce(I && init, R && reducer, A &&... attrs) {
    return __impl_attrs_reduce::AttrsIter<variadic_union_t<
      is_same_attribute,
      __impl_variadics_type_traits::ToPackType<Attrs, typename std::decay_t<A>::attributes_t>...>>::
      MergeReduce(std::forward<I>(init), std::forward<R>(reducer), std::forward<A>(attrs)...);
  }
  ...
  constexpr auto Reduce(I && init, R && reducer, A &&... attrs) {
    return __impl_attrs_reduce::AttrsIter<variadic_intersection_t<
      is_same_attribute,
      __impl_variadics_type_traits::ToPackType<Attrs, typename std::decay_t<A>::attributes_t>...>>::
      Reduce(std::forward<I>(init), std::forward<R>(reducer), std::forward<A>(attrs)...);
  }
  ...
  public:
    ...
    constexpr Attrs(U0 && arg, U &&... args)
      : __attrs_impl::AttrsParent<T...>(
          ...
          typename __attrs_impl::AttrsParent<T...>::template constructor_detector<U0>{},
  ...
  public:
    ...
    Attrs & operator=(F && from) {
      emp::tools::Foreach(__impl_AssignOp_assigner, *this, std ::forward<F>(from));
  ...
  private:
    ...
    struct GetAttr<false, A> {
      ...
      static constexpr decltype(auto) Get(S && Self, U &&... args) {
        ...
        return attribute_t::CallOrGetAttribute(std::forward<S>(Self), args...);
      }
    };
    ...
    struct GetAttr<true, A> {
      ...
      static constexpr decltype(auto) Get(S && Self, U &&... args) {
        using attribute_t = typename A::attribute_t;
        return attribute_t::CallOrGetAttribute(std::forward<S>(Self), std::forward<U>(args)...);
      }
    };
    ...
    static constexpr auto Call(S && Self, U &&... args) {
      ...
      return MakeAttrs(GetAttr < variadic_index_of_v<T, Attrs> == last,
  ...
  private:
    ...
    struct __impl_reduce_from_reduce_value {
      ...
      constexpr __impl_reduce_from_reduce_value(F callback) : callback(std::forward<F>(callback)) {}

      template <typename I, typename U>
      constexpr auto operator()(I && init, U && value) {
        return std::forward<F>(
          callback)(value.name, std::forward<I>(init), *std::forward<U>(value));
      }
    };
  ...
  public:
    ...
    constexpr auto AttributeReduce(I && init, F && callback) & {
      return emp::tools::Reduce(std::forward<I>(init), std::forward<F>(callback), *this);
    }
    ...
    template <typename I, typename F>
    constexpr auto AttributeReduce(I && init, F && callback) && {
      return emp::tools::Reduce(std::forward<I>(init), std::forward<F>(callback), std::move(*this));
    }
    ...
    constexpr auto Reduce(I && init, F && callback) {
      return AttributeReduce(std::forward<I>(init),
  ...
  public:
    ...
    constexpr void Foreach(F && callback) {
      AttributeForeach(__impl_foreach_from_foreach_value<decltype(std::forward<F>(callback))>{
        std::forward<F>(callback)});
    }
  ...
  private:
    template <typename S, typename U>
    constexpr static Attrs<T..., std::decay_t<U>>
    __impl_SetAttribute(const std::false_type & has_attr, S && self, U && attribute) {
    ...
    constexpr static auto __impl_SetAttribute(const std::true_type & has_attr,
                                              S && self,
                                              U && attribute) {
      return MakeAttrs(T::attribute_t::GetAttributeOrGetAttributeIn(std::forward<S>(self),
                                                                    std::forward<U>(attribute))...);
    }

    template <typename S, typename U>
    constexpr static auto __impl_SetAttribute(S && self, U && attribute) {
      return __impl_SetAttribute(has_attribute<S, typename std::decay_t<U>::attribute_t>{},
                                 std::forward<S>(self),
                                 std::forward<U>(attribute));
    }
  ...
  public:
    ...
    template <typename U>
    constexpr auto SetAttribute(U && attribute) && {
      return __impl_SetAttribute(std::move(*this), std::forward<U>(attribute));
    }
  ...
  template <typename... T>
  constexpr typename Attrs<T...>::__impl_AssignOp_assigner_t Attrs<T...>::__impl_AssignOp_assigner;
  ...
  constexpr auto Merge(U &&... packs)
    ...
    -> decltype(MergeReduce(Attrs<>{}, __impl_attrs_merge::attrs_merge, std::forward<U>(packs)...)) {
    return MergeReduce(Attrs<>{}, __impl_attrs_merge::attrs_merge, std::forward<U>(packs)...);
  }
#else  // Doxygen is getting tripped up by this : #ifndef DOXYGEN_SHOULD_SKIP_THIS
  template <typename... U>
  constexpr auto Merge(U &&... packs) {
    ;
  }
#endif /*DOXYGEN_SHOULD_SKIP_THIS*/  // Doxygen is getting tripped up by this : #ifndef DOXYGEN_SHOULD_SKIP_THIS : #else
  ...
  namespace __attrs_impl {
    ...
    void PrintAttrs(std::ostream & out,
                    const Attrs<T...> & attrs,
  ...
  std::ostream & operator<<(std::ostream & out, const Attrs<H, T...> & attrs) {
    out << "{ ";
    __attrs_impl::PrintAttrs(out, attrs, __attrs_impl::print_attrs_tag<H, T...>{});
    return out << " }";
  }

  std::ostream & operator<<(std::ostream & out, const Attrs<> & attrs) { return out << "{ }"; }
}}  // namespace emp::tools

#endif  // #ifndef INCLUDE_EMP_DEPRECATED_ATTRS_HPP_GUARD
