...
namespace emp {
  ...
  /// This class uses a combination of a hash (std::unordered_map) and emp::vector to
  ...
  public:
    ...
    size_t insert(const value_type & v) {
      ...
      if (pos_it != id_map.end()) {
        return pos_it->second;  // Already in map.
      }
    ...
    T & operator[](key_type key) {
      ...
      if (key_it == id_map.end()) { return NewEntry(key); }
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_RA_MAP_HPP_GUARD
