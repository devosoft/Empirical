...
namespace emp {
  ...
  private:
    ...
    bool RefillQueue() {
      ...
      for (size_t i = 0; i < item_buffer.size(); i++) {
        if (item_buffer[i].timing <= last_time) {
          item_queue.push_back(item_buffer[i]);
        } else {
          item_buffer[keep_count++] = item_buffer[i];
        }
      }
    ...
    void UpdateMinimum(double new_min) {
      ...
      for (size_t i = pos; i < item_queue.size(); i++) {
        ...
        if (item_queue[i].timing > time_limit) {
          item_buffer.push_back(item_queue[i]);
        }

        // Otherwise shift it into position.
        else {
          item_queue[shift_pos++] = item_queue[i];
        }
      }
  ...
  public:
    ...
    void Insert(T in_item, double trigger_time) {
      emp_assert(trigger_time > cur_time,
                 trigger_time,
                 cur_time);  // Triggers must be in the future
    ...
    T Front() {
      ...
      if (pos == item_queue.size()) {
        RefillQueue();  // Move over from buffer if needed.
      }
    ...
    T Next() {
      emp_assert(pos <= item_queue.size(),
                 pos,
                 item_queue.size(),
                 "Pos should never be more than one past end.");
      if (pos == item_queue.size()) {
        RefillQueue();  // Refill from buffer if needed.
      }
      emp_assert(item_queue.size() > pos,
                 item_queue.size(),
                 pos);  // Must have an element to return!
    ...
    bool Next(T & out_item) {
      ...
      if (item_queue.size() == 0) {
        return false;  // Return false if no items left.
      }
      cur_time = item_queue[pos].timing;
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_TIME_QUEUE_HPP_GUARD
