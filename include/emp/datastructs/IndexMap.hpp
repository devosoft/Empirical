...
namespace emp {
  ...
  private:
    ...
    size_t ToInternalID(size_t id) const { return (id + zero_offset) % num_items + num_items - 1; }
    ...
    size_t ToExternalID(size_t id) const { return (id + 1 - zero_offset) % num_items; }
    ...
    void RawAdjust(size_t id, const double new_weight) {
      ...
      if (needs_refresh) {
        return;  // If we already need a refresh don't update tree weights!
      }
    ...
    public:
      ...
      Proxy & operator=(double new_weight) {
        index_map.RawAdjust(id, new_weight);
        return *this;
      }
    };
  ...
  public:
    ...
    void Resize(size_t new_size, double def_value = 0.0) {
      if (new_size == num_items) {
        return;  // Already the right size?  Stop!
      }

      const size_t min_size =
        std::min(num_items, new_size);                    // Min size determines how much to copy.
      ...
      for (size_t i = min_size; i < new_size; i++) { weights[ToInternalID(i)] = def_value; }
    }
    ...
    void ResizeClear(size_t new_size) {
      if (new_size == 0) {
        weights.resize(0);  // If there are no items, zero-out weights array.
      } else {
        weights.resize(2 * new_size - 1);  // Else size for N values and N-1 internal nodes.
      }
      num_items = new_size;
      Clear();
    }
    ...
    [[deprecated("Use Set(id,weight) instead")]] void Adjust(size_t id, const double new_weight) {
      Set(id, new_weight);
    }

    [[deprecated("Use Set(new_weights) instead")]] void Adjust(
      const emp::vector<double> & new_weights) {
      Set(new_weights);
    }

    [[deprecated("Use SetAll(new_weight) instead")]] void AdjustAll(double new_weight) {
      SetAll(new_weight);
    }
    ...
    void DeferRefresh() { needs_refresh = true; }
  };
}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_INDEX_MAP_HPP_GUARD
