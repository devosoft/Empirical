 ...
 *
 * @file include/emp/datastructs/hash_utils.hpp
...
#ifndef INCLUDE_EMP_DATASTRUCTS_HASH_UTILS_HPP_GUARD
#define INCLUDE_EMP_DATASTRUCTS_HASH_UTILS_HPP_GUARD
...
namespace emp {
  ...
  namespace internal {
    ...
    template <typename T>
    auto Hash_impl(const T & x, bool) noexcept -> decltype(x.Hash()) {
      return x.Hash();
    }

    // By default, use std::hash if nothing else exists.
    template <typename T>
    auto Hash_impl(const T & x, int) noexcept -> decltype(std::hash<T>()(x)) {
      return std::hash<T>()(x);
    }
  ...
  std::size_t Hash(const T & x) noexcept {
    return internal::Hash_impl(x, true);
  }
  ...
  inline std::size_t hash_combine(emp::Ptr<const std::size_t> hashes, size_t num_hashes) noexcept {
    ...
    if (num_hashes == 1) {
      return hashes[0];  // If we have exactly one, just return it.
    }
...
// helper functions for murmur hash
  ...
  namespace internal {
    ...
    constexpr inline void fmix64(uint64_t & k) noexcept {
      k ^= k >> 33;
      k *= 0xff51'afd7'ed55'8ccd;
      k ^= k >> 33;
      k *= 0xc4ce'b9fe'1a85'ec53;
      k ^= k >> 33;
    }
  }  // namespace internal
  ...
  /// @return Hash of key.
  constexpr inline uint64_t murmur_hash(const std::span<const std::byte> key,
                                        const uint64_t seed = 0) noexcept {
    // define constants
    const uint64_t numbytes = key.size();
    const uint64_t nblocks  = numbytes / 16;
    const uint64_t c1       = 0x87c3'7b91'1142'53d5LLU;
    const uint64_t c2       = 0x4cf5'ad43'2745'937fLLU;
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_HASH_UTILS_HPP_GUARD
