...
// helpers for AlignedCharArrayUnion
  ...
  struct AlignedCharArrayUnion {
    alignas(
      ::emp::detail::AlignerImpl<T, Ts...>) char buffer[sizeof(emp::detail::SizerImpl<T, Ts...>)];
  };
  ...
  void SmallVectorTemplateBase<T, TriviallyCopyable>::grow(size_t MinSize) {
    ...
    size_t NewCapacity = size_t(emp::NextPowerOf2(this->capacity() + 2));
  ...
  public:
    ...
    iterator erase(const_iterator CI) {
      ...
      emp_assert(I >= this->begin() && "Iterator to erase is out of bounds.");
      emp_assert(I < this->end() && "Erasing at past-the-end iterator.");
    ...
    iterator erase(const_iterator CS, const_iterator CE) {
      ...
      emp_assert(S >= this->begin() && "Range to erase is out of bounds.");
      emp_assert(S <= E && "Trying to erase invalid range.");
      emp_assert(E <= this->end() && "Trying to erase past the end.");
    ...
    iterator insert(iterator I, T && Elt) {
      ...
      emp_assert(I >= this->begin() && "Insertion iterator is out of bounds.");
      emp_assert(I <= this->end() && "Inserting past the end of the vector.");
    ...
    template <typename ItTy,
              typename = typename std::
                enable_if<std::is_convertible<typename std::iterator_traits<ItTy>::iterator_category,
                                              std::input_iterator_tag>::value>::type>
  ...
  struct SmallVectorStorage {
    emp::AlignedCharArrayUnion<T> InlineElts[N];
  };
...
namespace std {
  ...
  template <typename T>
  inline void swap(emp::SmallVectorImpl<T> & LHS, emp::SmallVectorImpl<T> & RHS) {
    LHS.swap(RHS);
  }

  /// Implement std::swap in terms of SmallVector swap.
  template <typename T, unsigned N>
  inline void swap(emp::SmallVector<T, N> & LHS, emp::SmallVector<T, N> & RHS) {
    LHS.swap(RHS);
  }
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_SMALL_VECTOR_HPP_GUARD
