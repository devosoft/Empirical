...
namespace emp {
  ...
  public:
    ...
    UnorderedIndexMap(size_t _items = 0, double init_weight = 0.0)
      : num_items(_items)
      , num_nodes(_items - 1)
      , needs_refresh(_items && (init_weight > 0.0))
      , weights(0) {
    ...
    void Resize(size_t new_size, double def_value = 0.0) {
      ...
      for (size_t i = min_size; i < new_size; i++) { new_weights[new_size - 1 + i] = def_value; }
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_UNORDERED_INDEX_MAP_HPP_GUARD
