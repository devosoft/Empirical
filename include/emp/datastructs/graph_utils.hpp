 ...
 *
 * @file include/emp/datastructs/graph_utils.hpp
...
#ifndef INCLUDE_EMP_DATASTRUCTS_GRAPH_UTILS_HPP_GUARD
#define INCLUDE_EMP_DATASTRUCTS_GRAPH_UTILS_HPP_GUARD
...
namespace emp {
  ...
  Graph shuffle_graph(const Graph & in_graph, Random & random) {
    ...
    for (size_t from = 0; from < N; from++) {
      for (size_t to = 0; to < N; to++) {
        if (in_graph.HasEdge(from, to)) { out_graph.AddEdge(v_map[from], v_map[to]); }
      }
    }
  ...
  Graph build_graph_random(size_t v_count, size_t e_count, Random & random, bool connected = true) {
    ...
    emp_assert(e_count <= max_edges,
               e_count,
               max_edges);  // Shouldn't have more edges than can fit!
  ...
  Graph build_graph_clique_set(size_t clique_size,
                               size_t clique_count,
                               Random & random,
      ...
      for (size_t start2 = start1 + clique_size; start2 < v_count; start2 += clique_size) {
        ...
        for (size_t node1 = start1; node1 < end1; node1++) {
          for (size_t node2 = start2; node2 < end2; node2++) {
            if (node1 == start1 && node2 == start2) {
              continue;  // Both part of IS.
            }
  ...
  Graph build_graph_dag(size_t v_count, size_t e_count, Random & random, bool connected = true) {
    ...
    if (connected) {
      ...
      for (size_t i = 1; i < v_count; i++) {
        ...
        if (from > to) {
          std::swap(from, to);  // Make sure lower number is first.
        }
  ...
  WeightedGraph build_weighted_graph_tree(size_t v_count,
                                          size_t min_weight,
                                          size_t max_weight,
                                          Random & random) {
  ...
  WeightedGraph build_weighted_graph_random(size_t v_count,
                                            size_t e_count,
                                            size_t min_weight,
                                            size_t max_weight,
                                            Random & random,
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_GRAPH_UTILS_HPP_GUARD
