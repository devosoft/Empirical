 ...
 * Copyright (C) 2020-2023 Michigan State University
 * MIT Software license; see doc/LICENSE.md
 *
 * @file include/emp/datastructs/QueueCache.hpp
...
#ifndef INCLUDE_EMP_DATASTRUCTS_QUEUE_CACHE_HPP_GUARD
#define INCLUDE_EMP_DATASTRUCTS_QUEUE_CACHE_HPP_GUARD
...
namespace emp {
  ...
  private:
    ...
    static Value DefaultFun(const Key &) { throw std::invalid_argument("Key not in cache."); }
  ...
  public:
    ...
    bool Contains(const Key & key) const { return cache_map.count(key); }
    ...
    typename cache_list_t::iterator Put(const Key & key, const Value & val) {
      ...
      if (found != cache_map.end()) { Delete(found); }
    ...
    const typename cache_list_t::const_iterator cbegin() const { return cache_list.begin(); }
    ...
    const typename cache_list_t::const_iterator cend() const { return cache_list.end(); }
    ...
    Value & operator[](const Key & index) {
      ...
      if (!cache_map.count(index)) { Put(index, Value()); }
...
#endif  // #ifndef INCLUDE_EMP_DATASTRUCTS_QUEUE_CACHE_HPP_GUARD
