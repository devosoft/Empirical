    ...
    public:
      ...
      ConfigPanel(
        ...
        for (auto group : config.GetGroupSet()) {
          ...
          std::string group_base = emp::to_string(GetID(), "_", group_name, "_outer");
          ...
          for (size_t i = 0; i < group->GetSize(); i++) {
            ...
            std::string setting_base = emp::to_string(GetID(), "_", name);

            auto handleChange = [name, reload=reload_button, &config, &handleChange = onChangeRef](const std::string & val) {
              config.Set(name, val);
