...
namespace prefab {
  ...
  class ConfigPanel : public web::Div {
    ...
    public:
      /**
       * @param config config object used to construct
       * @param open whether all panels start in the open or closed state
       */
      ConfigPanel(
        Config & config,
        bool open = true,
        ...
        for (auto group : config.GetGroupSet()) {
          ...
          // Setting groups have IDs generated by "<main id>_<group name>"
          std::string group_base = emp::to_string(this_id, "_", group_name, "_outer");
          ...
          Div settings(emp::to_string(this_id, "_", group_name));
        ...
        // A div at the end for controls that might be added
      ...
      void ExcludeGroup(const std::string & setting_group) {
        ((Div)this->Find(emp::to_string(GetID(), "_", setting_group, "_outer"))).AddAttr("class", "excluded");
      }
