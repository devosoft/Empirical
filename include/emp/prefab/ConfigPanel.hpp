...
namespace emp { namespace prefab {
  ...
  namespace internal {
    ...
    public:
       ...
       */
      ConfigPanelInfo(const std::string & in_id = "") : web::internal::DivInfo(in_id) { ; }
       ...
       */
      void SetOnChangeFun(const on_change_fun_t & fun) { on_change_fun = fun; }
    };
  ...
  private:
     ...
     * @return cast pointer
     */
    INFO_TYPE * Info() { return dynamic_cast<INFO_TYPE *>(info); }
     ...
     * @return cast pointer
     */
    const INFO_TYPE * Info() const { return dynamic_cast<INFO_TYPE *>(info); }
    ...
    inline static std::function<std::string(const std::string &)> format_label =
      [](const std::string & name) { return to_titlecase(join(slice(name, '_'), " ")); };
     ...
     * @return current callback function handle
     */
    on_change_fun_t & GetOnChangeFun() { return Info()->GetOnChangeFun(); }
  ...
  public:
     ...
     */
    ConfigPanel(Config & config, bool open = true, const std::string & div_name = "") {
      ...
      for (auto & group : config.GetGroupSet()) {
        ...
        for (size_t i = 0; i < group->GetSize(); ++i) {
          ...
          const auto handleChange = [name,
                                     reload = reload_button,
                                     &config,
                                     &handleChange = onChangeRef](const std::string & val) {
            config.Set(name, val);
          ...
          if (Has(numeric_types, type)) {
            settings
              << NumericValueControl(pretty_name, desc, value, type, handleChange, setting_base);
          } else if (type == "std::string") {
            settings << TextValueControl(pretty_name, desc, value, handleChange, setting_base);
          } else if (type == "bool") {
            settings << BoolValueControl(pretty_name,
                                         desc,
     ...
     * @param fun callback function handle
     */
    void SetOnChangeFun(const on_change_fun_t & fun) { Info()->SetOnChangeFun(fun); }
     ...
     * to leave unchanged)
     */
    void SetRange(const std::string & setting,
      ...
      if (this->HasChild(target_id)) {
        ...
        if (target.Children()[0].IsInput()) {
          ...
          if (slider.GetType() == "range") {
            if (min != "DEFAULT") { slider.Min(min); }
            if (max != "DEFAULT") { slider.Max(max); }
            if (step != "DEFAULT") { slider.Step(step); }
          }
        }
      }
    }
     ...
     */
    [[deprecated("Use 'ExcludeSetting' to remove a single config parameter.")]] void ExcludeConfig(
     ...
     */
    [[deprecated("Prefer construction of ConfigPanel after config values have been set.")]] void
    Setup(Config & config, bool open = true, const std::string & id_prefix = "") {
     ...
     */
    [[deprecated("Can directly stream this component into another.")]] web::Div &
    GetConfigPanelDiv() {
      return (*this);
    }
  };
...
#endif  // #ifndef INCLUDE_EMP_PREFAB_CONFIG_PANEL_HPP_GUARD
