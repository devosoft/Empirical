...
namespace emp { namespace prefab {
  ...
  public:
     ...
     */
    ToggleSwitch(const std::function<void(std::string)> & cb,
      ...
      *this << checkbox;
      *this << label_element;
      if (label != "") { label_element << label; }
      checkbox.SetAttr("class", "custom-control-input");
      this->SetAttr("class", "custom-control custom-switch");
      this->SetCSS("clear", "none", "display", "inline");
      label_element.SetAttr("class", "custom-control-label", "for", checkbox.GetID());
    }
...
#endif  // #ifndef INCLUDE_EMP_PREFAB_TOGGLE_SWITCH_HPP_GUARD
