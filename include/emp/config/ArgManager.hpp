...
namespace emp {
  ...
  struct ArgSpec {
    ...
    ArgSpec(const size_t most_quota_,
  ...
  bool operator==(const ArgSpec & lhs, const ArgSpec & rhs) {
    return lhs.most_quota == rhs.most_quota && lhs.least_quota == rhs.least_quota &&
  ...
  public:
    ...
    /// to argument packs.
    static pack_map_t parse(pack_t args, const spec_map_t & specs = spec_map_t()) {
      auto res = pack_map_t();

      const auto alias_map = std::accumulate(std::begin(specs),
                                             std::end(specs),
                                             std::unordered_map<std::string, std::string>(),
                                             [](std::unordered_map<std::string, std::string> l,
                                                const std::pair<std::string, ArgSpec> & r) {
                                               l.insert({r.first, r.first});
                                               for (const auto & p : r.second.aliases) {
                                                 l.insert({p, r.first});
                                               }
                                               return l;
                                             });
      ...
      emp_assert(alias_map.size() ==
                   std::accumulate(std::begin(specs),
                                   std::end(specs),
                                   specs.size(),
      ...
      auto parse_alias = [deflagged, args, alias_map, specs](const size_t i) {
        ...
        for (const char ch : deflag) {
          if (alias_map.count(std::string{ch})) {
            ...
            if (specs.find(alias_map.find(std::string{ch})->second)->second.most_quota != 0) {
              // put strung-together commands that take arguments
              // into _invalid
              return pack_t{"_invalid"};
            };

            commands.push_back(alias_map.find(std::string{ch})->second);

          } else {
            // found a bad letter! abort and return positional or unknown
            if (alias_map.count("_positional") &&
      ...
      for (size_t i = 1; i < args.size(); ++i) {
        ...
        for (const auto & command : commands) {
          ...
          size_t j;
          for (j = i;
               j < args.size() && j - i < spec.most_quota &&
               (spec.gobble_flags || !(j + 1 < args.size()) || deflagged[j + 1] == args[j + 1]);
               ++j);

          // store the argument pack
          bool is_special =
            command == "_positional" || command == "_unknown" || command == "_invalid";
          res.insert({command,
                      pack_t(std::next(std::begin(args), (int) (is_special ? i : i + 1)),
                             j + 1 < args.size() ? std::next(std::begin(args), (int) j + 1)
                                                 : std::end(args))});
          i = j;
        }
      }
    ...
    static spec_map_t make_builtin_specs(const emp::Ptr<Config> config = nullptr) {
      spec_map_t res({
        {"_positional",
         ArgSpec(std::numeric_limits<size_t>::max(),
                 1,
      ...
      if (config) {
        for (const auto & e : *config) {
          ...
          res.insert({entry->GetName(),
                      ArgSpec(1,
                              emp::to_string(entry->GetDescription(),
                                             " (type=",
                                             entry->GetType(),
                                             "; default=",
                                             entry->GetDefault(),
    ...
    ArgManager(int argc, char * argv[], const spec_map_t & specs_ = make_builtin_specs())
      : ArgManager(ArgManager::args_to_strings(argc, argv), specs_) {
      ;
    }
    ...
    ArgManager(const pack_t args, const spec_map_t & specs_ = make_builtin_specs())
      : ArgManager(ArgManager::parse(args, DealiasSpecs(specs_)), specs_) {
      ;
    }
    ...
    ArgManager(const pack_map_t & packs_, const spec_map_t & specs_ = make_builtin_specs())
      ...
      for (auto & [n, s] : specs) {
        if (s.flatten && packs.count(n)) {
          pack_t flat =
            std::accumulate(packs.equal_range(n).first,
                            packs.equal_range(n).second,
                            pack_t(),
                            [](pack_t l, const std::pair<std::string, pack_t> & r) {
    ...
    /// packs are consumed.
    void UseCallbacks() {
      for (const auto & [name, spec] : specs) { while (CallbackArg(name)); }
    }
    ...
    bool ProcessBuiltin(const emp::Ptr<Config> config = nullptr, std::ostream & os = std::cout) {
      UseArg("_command");
    ...
    void PrintDiagnostic(std::ostream & os = std::cout) const {
      for (const auto & [name, pack] : packs) {
        if (name == "_unknown") {
          os << "UNKNOWN | ";
        } else if (specs.count(name) && specs.find(name)->second.least_quota <= pack.size() &&
    ...
    void PrintHelp(std::ostream & os = std::cerr) const {
      ...
      for (const auto & [name, spec] :
        ...
        if (spec.least_quota == spec.most_quota) {
          ...
          os << " [ " << spec.least_quota << " <= quota <= " << spec.most_quota << " ]";
        }

        os << std::endl << "   | " << spec.description << std::endl;
      }
    }
    ...
    static spec_map_t DealiasSpecs(spec_map_t inSpecsMap) {
      ...
      for (auto iSpec : inSpecsMap) {
        ...
        for (auto iAlias : aliases) { oSpec.aliases.insert(iAlias); }
  ...
  namespace cl {
    ...
    public:
      ...
      /// config object.
      int UseArg(const std::string & name,
                 Config & config,
      ...
      void PrintHelp(std::ostream & os) const {
        ...
        for (size_t i = 0; i < arg_names.size(); i++) {
          os << arg_names[i] << std::string(max_name_size + 1 - arg_names[i].size(), ' ')
             << arg_descs[i] << std::endl;
        }
      }
      ...
      /// Return bool for "should program proceed" (i.e., true=continue, false=exit).
      bool ProcessConfigOptions(Config & config,
                                std::ostream & os,
        ...
        bool const_macros =
          macro_file.size() && UseFlag("--make-const", "Generate const version of macros file.");
...
#endif  // #ifndef INCLUDE_EMP_CONFIG_ARG_MANAGER_HPP_GUARD
