 ...
 *  @brief A tool for collecting settings, including from files and the command
 * line.
...
/// Class to take a set of value for each "setting" and then step through all
/// combinations of those values for a factorial analysis.
...
private:
  ...
  struct SettingBase {
    ...
    SettingBase(const std::string &_name, const std::string &_desc,
        ...
        : name(_name), desc(_desc), flag(_flag),
          option(emp::to_string("--", _name)), args_label(_args_label) {}
    virtual ~SettingBase() {}
    ...
    virtual std::string
    AsString() const = 0; ///< All values, as a single string.
    virtual std::string
        AsString(size_t) const = 0; ///< A specified value as a string.
    virtual bool FromString(
        const std::string_view &) = 0; ///< Convert string to set of settings.
    virtual bool SetValueID(size_t) {
      return false;
    } ///< Setup cur value in linked variable
    virtual bool IsComboSetting() {
      return false;
    ...
    bool IsOptionMatch(const std::string &test_option) const {
      return test_option == option;
    }
    bool IsFlagMatch(const char test_flag) const { return test_flag == flag; }
  };

  /// Full details about a single setting, including type information and
  /// values.
  template <typename T> struct SettingInfo : public SettingBase {
    T value;
    emp::Ptr<T> var_ptr = nullptr;

    SettingInfo(
        const std::string &_name, ///< Unique name for this setting.
    ...
    emp::Ptr<SettingBase> Clone() const override {
      ...
      emp::Ptr<SettingInfo<T>> setting_info_ptr = emp::NewPtr<SettingInfo<T>>(
          name, desc, flag, args_label, new_var_ptr);
  ...
  /// Allow a single setting to have multiple values specified that should be
  /// stepped through.
  template <typename T> struct ComboSettingInfo : public SettingBase {
    emp::vector<T> values; ///< Set of values to use for this setting.
    emp::Ptr<T> var_ptr =
        nullptr; ///< Pointer to variable to set as combos change.
    ...
    emp::Ptr<SettingBase> Clone() const override {
      ...
      auto csi_ptr = emp::NewPtr<ComboSettingInfo<T>>(name, desc, flag,
                                                      args_label, new_var_ptr);
    ...
    bool FromString(const std::string_view &input) override {
      ...
      // Clear out the values to set, one at a time (in most cases, aleady
      // clear)
      values.resize(0);
      ...
      for (auto &cur_str : slices) {
        ...
        if constexpr (std::is_arithmetic<T>::value) {
          ...
          if (r_slices.size() > 3)
            return false; // Error!  Too many slices!!!
          T start = emp::from_string<T>(r_slices[0]);
          T end = emp::from_string<T>(
              r_slices.back()); // Same as start if one value.
          T step =
              (r_slices.size() == 3) ? emp::from_string<T>(r_slices[1]) : 1;
          while (start <= end) {
      ...
      if (!var_ptr.IsNull() && values.size())
        *var_ptr = values[0];
      return values.size(); // Must result in at least one value.
    }

    bool SetValueID(size_t id) override {
      if (var_ptr)
        *var_ptr = values[id];
      return true;
    }
  ...
  /// A setting that is just a flag with an action function to run if it's
  /// called.
  struct ActionFlag {
  ...
  std::map<std::string, emp::Ptr<SettingBase>>
      setting_map;                              ///< All settings by name
  std::map<std::string, ActionFlag> action_map; ///< Action flags

  emp::vector<emp::Ptr<SettingBase>>
      combo_settings; ///< Multi-value settings (in order)
  emp::vector<size_t>
      cur_combo;       ///< Which combo settings are we currently using?
  size_t combo_id = 0; ///< Unique value indicating which combination we are on.

  emp::vector<std::string>
      unused_args; // Arguments that we were unable to process.
  std::string errors;
...
public:
  ...
  template <typename T> T MaxValue(const std::string &name) const {
    emp_assert(emp::Has(setting_map, name), name);
    emp::Ptr<SettingBase> base_ptr = setting_map.find(name)->second;
  ...
  /// Add a new setting of a specified type.  Returns the (initially empty)
  /// vector of values to allow easy setting. Example:
  ///   config.AddSetting("num_runs") = 200;

  template <typename T>
  T &AddSetting(const std::string &name, const std::string &desc,
                const char option_flag, T &var,
                const std::string &args_label = "Value") {
    emp_assert(!emp::Has(setting_map, name));
    auto new_ptr =
        emp::NewPtr<SettingInfo<T>>(name, desc, option_flag, args_label, &var);
  ...
  T &AddSetting(const std::string &name, const std::string &desc = "",
    ...
    emp_assert(!emp::Has(setting_map, name));
    auto new_ptr =
        emp::NewPtr<SettingInfo<T>>(name, desc, option_flag, "Value");
    setting_map[name] = new_ptr;
    return new_ptr->value;
  }
  ...
  emp::vector<T> &AddComboSetting(const std::string &name,
    ...
    emp_assert(!emp::Has(setting_map, name));
    auto new_ptr =
        emp::NewPtr<ComboSettingInfo<T>>(name, desc, option_flag, "Values...");
  ...
  emp::vector<T> &AddComboSetting(const std::string &name,
    ...
    emp_assert(!emp::Has(setting_map, name));
    auto new_ptr = emp::NewPtr<ComboSettingInfo<T>>(name, desc, option_flag,
                                                    args_label, &var);
    new_ptr->id = combo_settings.size();
    combo_settings.push_back(new_ptr);
    setting_map[name] = new_ptr;
  ...
  void AddAction(const std::string &name, const std::string &desc,
                 const char flag, std::function<void()> fun) {
  ...
  template <typename T> emp::vector<T> &ComboValues(const std::string &name) {
    emp_assert(emp::Has(setting_map, name), name);
  ...
  void ResetCombos() {
    // Setup as base combo.
    for (size_t &x : cur_combo)
      x = 0;
    combo_id = 0;

    // Setup all linked values.
    for (auto x : combo_settings)
      x->SetValueID(0);
  }

  /// Add a single new value to the specified setting.
  template <typename T> void AddComboValue(const std::string &name, T &&val) {
    emp_assert(emp::Has(setting_map, name), name);
    emp_assert(setting_map[name]->IsComboSetting());
  ...
  size_t CountCombos() {
    size_t result = 1;
    for (auto ptr : combo_settings)
      result *= ptr->GetSize();
    return result;
  }

  /// Set the next combination of settings to be active.  Return true if
  /// successful or false if we ran through all combinations and reset.
  bool NextCombo() {
    combo_id++;
    for (size_t i = 0; i < cur_combo.size(); i++) {
      ...
      if (cur_combo[i] < combo_settings[i]->GetSize()) {
        combo_settings[i]->SetValueID(
            cur_combo[i]); // Set value in linked variable.
        return true;
      }
  ...
  std::string CurSettings(const std::string &separator = ",") const {
    ...
    for (auto [name, ptr] : setting_map) {
      if (ptr) {
        if (out_str.size())
          out_str += separator;
        out_str += ptr->IsComboSetting()
                       ? ptr->AsString(cur_combo[ptr->GetID()])
                       : ptr->AsString();
      }
    }
    return out_str;
  ...
  /// Convert all of the current values into a comma-separated string.
  std::string
  CurComboString(const std::string &separator = ",",
                 bool use_labels = false, ///< Print name with each value?
                 bool multi_only = false ///< Only print values that can change?
  ) const {
    std::string out_str;
    for (size_t i = 0; i < cur_combo.size(); i++) {
      if (multi_only && combo_settings[i]->GetSize() == 1)
        continue;
      if (out_str.size())
        out_str += separator;
      if (use_labels)
        out_str += emp::to_string(combo_settings[i]->name, '=');
  ...
  emp::Ptr<SettingBase> FindOptionMatch(const std::string &option_name) {
    for (const auto &[name, ptr] : setting_map) {
      if (ptr->IsOptionMatch(option_name))
        return ptr;
    }
    return nullptr;
  }
  ...
  emp::Ptr<SettingBase> FindFlagMatch(const char symbol) {
    for (const auto &[name, ptr] : setting_map) {
      if (ptr->IsFlagMatch(symbol))
        return ptr;
    }
    return nullptr;
  }

  /// Take an input set of config options, process them, and track set of
  /// unprocessed ones.
  ...
  template <typename... Ts> void PrintHelp(const Ts &...examples) const {
    ...
    for (auto [name, action] : action_map) {
      if (name.size() == 2)
        continue; // Skip flag entries.
      std::cout << " -" << action.flag << " : " << action.desc << " (" << name
                << ")\n";
    }
