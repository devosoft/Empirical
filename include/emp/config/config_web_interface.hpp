 ...
 *
 * @file include/emp/config/config_web_interface.hpp
 * @brief TODO.
 */

#pragma once

#ifndef INCLUDE_EMP_CONFIG_CONFIG_WEB_INTERFACE_HPP_GUARD
#define INCLUDE_EMP_CONFIG_CONFIG_WEB_INTERFACE_HPP_GUARD
...
namespace emp {
  ...
  class ConfigWebUI {
  private:
    inline static std::set<std::string> numeric_types =
      {"int", "double", "float", "uint32_t", "uint64_t", "size_t"};
    Config & config;
  ...
  public:
    ...
    void ExcludeConfig(std::string setting) { exclude.insert(setting); }
...
#endif  // #ifndef INCLUDE_EMP_CONFIG_CONFIG_WEB_INTERFACE_HPP_GUARD
