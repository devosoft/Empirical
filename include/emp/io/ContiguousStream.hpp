...
namespace emp {
  ...
  public:
    ...
    ContiguousBuffer(const size_t init_size = 1024)
      : buffer(std::max(static_cast<size_t>(1), init_size)  // 0 init size breaks!
        ) {
      Reset();
    }
    ...
    inline size_t GetSize() const { return std::distance<const char *>(GetData(), this->pptr()); }
    ...
    inline const_iterator cbegin() const { return std::cbegin(buffer); }

    /// End iterator on stored data
    inline const_iterator cend() const { return std::cend(buffer); }
  ...
  private:
    ...
    int_type overflow(int_type c) override {
      ...
      buffer.resize(
        prev_capacity + std::max(prev_capacity / 2, static_cast<size_t>(1))  // grow at least one
      );

      // update the 3 streambuf pointers to point to the fresh write area
      this->setp(GetData() + prev_capacity,  // pbase & pptr
                 GetData() + GetCapacity()   // epptr
      );
...
#endif  // #ifndef INCLUDE_EMP_IO_CONTIGUOUS_STREAM_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: streambufs andreoffringa sputc nateriz pbase isi huy pbump uvector onlinedocs ujv cplusplus epptr setp pptr
