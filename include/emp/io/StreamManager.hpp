...
namespace emp {
  ...
  protected:
    ...
    public:
      ...
      TypedStreamInfo(const std::string & name) : StreamInfo(name, true) { ResetStream(); }
      ...
      void ResetStream() {
        ...
        if constexpr (TYPE == Type::FILE) {
          if constexpr (ACCESS == Access::INPUT) {
            ptr = NewPtr<std::ifstream>(name);
          } else if constexpr (ACCESS == Access::OUTPUT) {
            ptr = NewPtr<std::ofstream>(name);
  ...
  public:
    ...
    bool HasOutputFileStream(const std::string & name) const {
      return GetInfo(name).IsOutputFile();
    }
    ...
    bool HasStdOutputStream(const std::string & name) const {
      return GetInfo(name).IsOtherOutput();
    }
    ...
    std::ostream & get_ostream(const std::string & name        = "cout",
                               const std::string & stdout_name = "cout") {
...
#endif  // #ifndef INCLUDE_EMP_IO_STREAM_MANAGER_HPP_GUARD
