 ...
 *
 * @file include/emp/io/io_utils.hpp
...
#ifndef INCLUDE_EMP_IO_IO_UTILS_HPP_GUARD
#define INCLUDE_EMP_IO_IO_UTILS_HPP_GUARD
...
namespace emp { static constexpr bool WINDOWS_IO = true; }
#else  // #ifdef _WIN32
#include <termios.h>
#include <unistd.h>

namespace emp { static constexpr bool WINDOWS_IO = false; }
...
namespace emp {
  ...
  void Print(ARG_Ts &&... args) {
    PrintTo(std::cout, std::forward<ARG_Ts>(args)...);
  }

  template <typename... ARG_Ts>
  void PrintLn(ARG_Ts &&... args) {
    Print(std::forward<ARG_Ts>(args)..., '\n');
  }
  ...
  void PrintRepeat(T arg, size_t count) {
    for (size_t i = 0; i < count; ++i) { Print(arg); }
  }

  template <typename... ARG_Ts>
  void PrintRepeatLn(ARG_Ts &&... args) {
    PrintRepeat(std::forward<ARG_Ts>(args)...), PrintLn();
  }
  ...
  [[nodiscard]] std::optional<std_fs::path> FindFolderInPath(std::string folder_name,
                                                             std_fs::path test_path) {
    ...
    while (!test_path.empty() && test_path != "/") {
      ...
      if (std_fs::exists(candidate) && std_fs::is_directory(candidate)) { return candidate; }
      std_fs::path parent_path = test_path.parent_path();
      if (parent_path == test_path) {
        break;  // If we are no longer descending, stop!
      }
  ...
  void ReduceToCommonPath(split_path_t & path1, const split_path_t & path2) {
    ...
    while (match_size < path1.size() && match_size < path2.size() &&
           path1[match_size] == path2[match_size]) {
      ++match_size;
    }
  ...
  [[nodiscard]] std_fs::path FindCommonPath(const CONTAINER_T & filenames) {
    ...
    while (++it != filenames.end()) { ReduceToCommonPath(common, SplitPath(*it)); }
...
#endif  // #ifndef INCLUDE_EMP_IO_IO_UTILS_HPP_GUARD
