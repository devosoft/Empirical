...
namespace emp {
  ...
  template <typename C>
  typename emp::remove_ptr_type<typename C::value_type>::type Sum(C & elements) {
  ...
  auto SumScalarResults(C & elements, FUN && fun) {
    ...
    return_t total = 0;
    for (auto element : elements) { total += fun(emp::remove_ptr_value(element)); }
    return total;
  }
  ...
  auto Variance(C & elements) {
    ...
    auto sum =
      SumScalarResults(elements, [mean](auto x) { return emp::Pow(mean - (double) x, 2); });
  ...
  template <typename C>
  typename std::enable_if<!emp::is_ptr_type<typename C::value_type>::value, int>::type UniqueCount(
    C & elements) {
    // Converting to a set will remove duplicates leaving only unique values
    std::set<typename C::value_type> unique_elements(elements.begin(), elements.end());
  ...
  typename std::enable_if<emp::is_ptr_type<typename C::value_type>::value, int>::type UniqueCount(
    ...
    for (auto element : elements) { unique_elements.insert(*element); }
  ...
  emp::vector<RET_TYPE> ApplyFunction(std::function<RET_TYPE(ARG_TYPE)> & fun, C & elements) {
    ...
    for (auto element : elements) { results.push_back(fun(element)); }
    return results;
  }
...
#endif  // #ifndef INCLUDE_EMP_MATH_STATS_HPP_GUARD
