 ...
 *
 * @file include/emp/math/Random.hpp
...
#ifndef INCLUDE_EMP_MATH_RANDOM_HPP_GUARD
#define INCLUDE_EMP_MATH_RANDOM_HPP_GUARD
...
namespace emp {
  ...
  private:
    ...
    uint64_t Get64() noexcept {
      ...
      if constexpr (requires { engine.Get64(); }) {
        return engine.Get64();
      }

      // If the regular Get() is already 64 bits, use it.
      else if constexpr (NATIVE64) {
        return Get();
      }

      // Otherwise stitch two 32-bit random values together.
      else {
        return (static_cast<uint64_t>(Get32()) << 32) + Get32();
      }
    }
  ...
  public:
    ...
    inline uint64_t GetUInt64(const uint64_t max) noexcept {
      if (max <= VAL32_CAP) {
        return GetUInt(max);  // Don't need extra precision.
      }
    ...
    template <Prob PROB>
    void RandFillP(mem_ptr_t dest,
                   [[maybe_unused]] const size_t num_bytes,
                   size_t start_bit,
                   size_t stop_bit) noexcept {
      ...
      constexpr double p =
        static_cast<double>(PROB) / 1000.0;  // Determine actual probability of a 1
      ...
      if (end_bit_id) {
        ...
        for (size_t i = 0; i < end_bit_id; i++) {                // Step through bits to flip.
          if (P(p)) {
            end_byte |= ((uint8_t) 1 << i);  // Set appropriate bits.
          }
        }
      }
    }
    ...
    void RandFill12_5(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_12_5>(dest, bytes);
    }

    void RandFill25(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_25>(dest, bytes);
    }

    void RandFill37_5(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_37_5>(dest, bytes);
    }

    void RandFill50(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_50>(dest, bytes);
    }

    void RandFill62_5(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_62_5>(dest, bytes);
    }

    void RandFill75(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_75>(dest, bytes);
    }

    void RandFill87_5(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_87_5>(dest, bytes);
    }

    void RandFill100(mem_ptr_t dest, const size_t bytes) noexcept {
      RandFillP<PROB_100>(dest, bytes);
    }
    ...
    void RandFill12_5(mem_ptr_t dest,
                      const size_t bytes,
                      size_t start_bit,
                      size_t stop_bit) noexcept {
    ...
    void RandFill37_5(mem_ptr_t dest,
                      const size_t bytes,
                      size_t start_bit,
                      size_t stop_bit) noexcept {
    ...
    void RandFill62_5(mem_ptr_t dest,
                      const size_t bytes,
                      size_t start_bit,
                      size_t stop_bit) noexcept {
    ...
    void RandFill87_5(mem_ptr_t dest,
                      const size_t bytes,
                      size_t start_bit,
                      size_t stop_bit) noexcept {
    ...
    void RandFill100(mem_ptr_t dest,
                     const size_t bytes,
                     size_t start_bit,
                     size_t stop_bit) noexcept {
    ...
    void RandFill(mem_ptr_t dest, const size_t num_bytes, const double p) noexcept {
      ...
      if (p == 0.0) {
        return RandFill0(dest, num_bytes);
      } else if (p == 0.125) {
        return RandFill12_5(dest, num_bytes);
      } else if (p == 0.25) {
        return RandFill25(dest, num_bytes);
      } else if (p == 0.375) {
        return RandFill37_5(dest, num_bytes);
      } else if (p == 0.5) {
        return RandFill50(dest, num_bytes);
      } else if (p == 0.625) {
        return RandFill62_5(dest, num_bytes);
      } else if (p == 0.75) {
        return RandFill75(dest, num_bytes);
      } else if (p == 0.875) {
        return RandFill87_5(dest, num_bytes);
      } else if (p == 1.0) {
        return RandFill100(dest, num_bytes);
      }
    ...
    /// Randomize a contiguous segment of memory with a given probability of ones.
    void RandFill(mem_ptr_t dest,
                  const size_t num_bytes,
                  const double p,
                  const size_t start_bit,
                  const size_t stop_bit) noexcept {
      emp_assert((stop_bit >> 3) <= num_bytes);

      // Try to find a shortcut if p allows....
      if (p == 0.0) {
        return RandFill0(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.125) {
        return RandFill12_5(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.25) {
        return RandFill25(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.375) {
        return RandFill37_5(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.5) {
        return RandFill50(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.625) {
        return RandFill62_5(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.75) {
        return RandFill75(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 0.875) {
        return RandFill87_5(dest, num_bytes, start_bit, stop_bit);
      } else if (p == 1.0) {
        return RandFill100(dest, num_bytes, start_bit, stop_bit);
      }
      ...
      for (size_t i = start_bit; i < stop_bit; i++) {
        if (P(p)) {
          dest[cur_byte] |= cur_mask;  // Set the target bit.
        } else {
          dest[cur_byte] &= ~cur_mask;  // Clear out the target bit.
        }
    ...
    [[nodiscard]] double GetNormal(const double mean, const double std) {
      return mean + GetNormal() * std;
    }
    ...
    [[nodiscard]] uint64_t GetPoisson(const double n, const double p) {
      ...
      if (p > .5) {
        return static_cast<uint64_t>(n) - GetPoisson(n * (1 - p));
      } else {
        return GetPoisson(n * p);
      }
    }
    ...
    [[nodiscard]] uint64_t GetPoisson(const double mean) {
      ...
      if (a <= 0) {
        return MAX_8BYTE;  // cannot calculate, so return max
      }
      uint64_t k = 0;
      double u   = GetDouble();
      while (u >= a) {
...
#endif  // #ifndef INCLUDE_EMP_MATH_RANDOM_HPP_GUARD
