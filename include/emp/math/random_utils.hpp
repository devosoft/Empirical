...
namespace emp {
  ...
  inline void Choose(auto & random, size_t N, size_t K, std::vector<size_t> & choices) {
    ...
    choices.resize(K);
    while (K) {
      if (N == K || random.P(((double) K) / ((double) N))) {
        choices[--K] = --N;
      } else {
        --N;
      }
  ...
  public:
    LowProbability(double p) : scale(CalcScale(p)) { emp_assert(p > 0.0 && p < 1.0); }
  ...
  public:
    HighProbability(double p) : scale(CalcScale(p)) { emp_assert(p > 0.0 && p < 1.0); }
  ...
  private:
    ...
    void ResetBuffer(RAND_T & random) {
      for (uint8_t & buffer_entry : buffer) { buffer_entry = random.P(p); }
      next = 0;
    }
  public:
    BufferedProbability(double p) : p(p) { emp_assert(p > 0.0 && p < 1.0); }
...
#endif  // #ifndef INCLUDE_EMP_MATH_RANDOM_UTILS_HPP_GUARD
