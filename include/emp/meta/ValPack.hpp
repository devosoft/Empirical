...
// Anonymous implementations of ValPack interface.
  ...
  namespace internal {
    ...
    struct vp_loop {
      ...
      template <auto V, bool D = false>
      using pnext = vp_loop<in_pop, out_pbin<V>, D, VERSION>;  // Prune
    ...
    struct vp_loop<T_IN, T_OUT, true, VERSION> {
      template <auto V>
      using pop_val = typename T_OUT::template append<T_IN>;  // Pop done!
    };

    template <typename T_OUT, auto VERSION>
    struct vp_loop<ValPack<>, T_OUT, false, VERSION> {
      template <auto V>
      using pop_val = T_OUT;  // Nothing to pop! (error?)
      template <auto V>
      using remove = T_OUT;  // Nothing left to remove!
      template <auto V>
      using uniq = T_OUT;  // Nothing left to check!
    };
    ...
    struct vp_reverse<ValPack<V1, Vs...>> {
      using result = typename vp_reverse<ValPack<Vs...>>::result::template push_back<V1>;
    };

    template <>
    struct vp_reverse<ValPack<>> {
      using result = ValPack<>;
    };
    ...
    template <auto V1, auto... Vs>
    struct vp_uniq<ValPack<V1, Vs...>> {
      using result =
        typename vp_loop<ValPack<V1, Vs...>, ValPack<>, false, 1>::template uniq<V1 + 1>;
    };

    template <>
    struct vp_uniq<ValPack<>> {
      using result = ValPack<>;
    };
    ...
    struct vp_sort_impl {
      ...
      template <auto V>
      using snext = vp_sort_impl<spop<V>, typename T_OUT::template push_back<V>>;
      template <auto V>
    ...
    struct vp_sort_impl<ValPack<>, T_OUT> {
      template <auto V>
      using sort = T_OUT;  // Nothing left to sort!
    };
  ...
  struct ValPack<V1, Vs...> {
    ...
    template <typename T>
    constexpr static bool Has(T val) {
      return (val == V1) | pop::Has(val);
    }

    /// Count the number of occurrences of value V in ValPack.
    template <typename T>
    constexpr static size_t Count(T val) {
      return pop::Count(val) + (val == V1);
    }

    /// Determine the position at which V appears in ValPack.
    template <typename T>
    constexpr static int GetID(T val) {
      return (val == V1) ? 0 : (1 + pop::GetID(val));
    }
    ...
    template <typename T>
    constexpr static auto Min(T cap) {
      return cap < pop::Min(V1) ? cap : pop::Min(V1);
    }
    ...
    template <typename T>
    constexpr static auto Max(T floor) {
      return floor > pop::Max(V1) ? floor : pop::Max(V1);
    }
    ...
    static std::string ToString() { return std::string(1, (char) V1) + pop::ToString(); }
  ...
  struct ValPack<> {
    ...
    template <auto V>
    using pop_val = ValPack<>;  // No value to pop!  Failure?
    template <auto V>
    ...
    template <typename T>
    constexpr static bool Has(T) {
      return false;
    }

    template <typename T>
    constexpr static size_t Count(T) {
      return 0;
    }

    template <typename T>
    constexpr static int GetID(T) {
      return -100000;
    }
...
#endif  // #ifndef INCLUDE_EMP_META_VAL_PACK_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: usort snext spop rsort pbin uniq pnext
