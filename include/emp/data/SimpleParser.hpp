...
namespace emp {
  ...
  private:
    ...
    struct ValueType {
      ...
      ValueType & operator=(emp::Datum in_val) {
        type  = VALUE;
        value = in_val;
        return *this;
      }

      ValueType & operator=(double in_val) {
        type  = VALUE;
        value = in_val;
        return *this;
      }

      ValueType & operator=(const emp::String & in_val) {
        type  = VALUE;
        value = in_val;
        return *this;
      }

      ValueType & operator=(fun_t in_fun) {
        type = FUNCTION;
        fun  = in_fun;
        return *this;
      }
    ...
    struct SymbolTable<emp::ra_map<emp::String, VALUE_T>, DUMMY_T> {
      ...
      SymbolTable(const emp::ra_map<emp::String, VALUE_T> & in_map) : layout(in_map.GetLayout()) {}
    ...
    public:
      ...
      bool IsExternal(const emp::Token & token) const noexcept {
        return token.id == token_external;
      }
    ...
    struct BinaryOperator {
      ...
      void Set(size_t in_prec, fun_t in_fun) {
        prec = in_prec;
        fun  = in_fun;
      }
    };
    ...
    struct Function {
      ...
      void Set0(fun0_t in_fun) {
        num_args = 0;
        fun0     = in_fun;
      }

      void Set1(fun1_t in_fun) {
        num_args = 1;
        fun1     = in_fun;
      }

      void Set2(fun2_t in_fun) {
        num_args = 2;
        fun2     = in_fun;
      }

      void Set3(fun3_t in_fun) {
        num_args = 3;
        fun3     = in_fun;
      }
    };
  ...
  public:
    ...
    const std::set<emp::String> & GetNamesUsed(const emp::String & expression) {
      ...
      emp::TokenStream tokens =
        lexer.Tokenize(expression, emp::String("Expression: ") + expression);
    ...
    void AddOp(const emp::String & op,
               size_t prec,
    ...
    void AddDefaultFunctions() {
      ...
      functions["IF"].Set3(
        [](emp::Datum x, emp::Datum y, emp::Datum z) { return (x != 0.0) ? y : z; });
      functions["CLAMP"].Set3([](emp::Datum x, emp::Datum y, emp::Datum z) {
        return (x < y) ? y : (x > z) ? z : x;
      });
      functions["TO_SCALE"].Set3(
        [](emp::Datum x, emp::Datum y, emp::Datum z) { return (z - y) * x + y; });
      functions["FROM_SCALE"].Set3(
        [](emp::Datum x, emp::Datum y, emp::Datum z) { return (x - y) / (z - y); });
    }
    ...
    void AddRandomFunctions(Random & random) {
      ...
      functions["RAND"].Set2(
        [&random](emp::Datum x, emp::Datum y) { return random.GetDouble(x, y); });
    }
    ...
    typename SYMBOLS_T::value_t ParseValue(const SYMBOLS_T & symbols, pos_t & pos) {
      if constexpr (verbose) {
        std::cout << "ParseValue at position " << pos.GetIndex() << " : " << pos->lexeme
                  << std::endl;
      }
      ...
      if (pos->lexeme == "(") {
        ...
        if (pos->lexeme != ")") {
          return ParseError("Expected ')', but found '", pos->lexeme, "'.");
        }
        ++pos;
        return val;
      }
      ...
      if (is_fun) {
        if (!emp::Has(functions, name)) {
          return ParseError("Call to unknown function '", name, "'.");
        }
        ++pos;
        ...
        switch (args.size()) {
          case 0:
            ...
            break;
          case 1:
            if (!functions[name].fun1) {
              ParseError("Function '", name, "' cannot have 1 arguments.");
            }
            ...
            break;
          case 2:
            if (!functions[name].fun2) {
              ParseError("Function '", name, "' cannot have 2 arguments.");
            }
            ...
            break;
          case 3:
            if (!functions[name].fun3) {
              ParseError("Function '", name, "' cannot have 3 arguments.");
            }
            ...
            break;
          default: ParseError("Too many arguments (", args.size(), ") for function '", name, "'.");
        }
        return out_fun;
      }
    ...
    template <typename SYMBOLS_T>
    typename SYMBOLS_T::value_t ParseMath(const SYMBOLS_T & symbols,
                                          pos_t & pos,
                                          size_t prec_limit = 0) {
      using value_t = typename SYMBOLS_T::value_t;
      using arg_t   = typename SYMBOLS_T::arg_t;
      ...
      while (pos.IsValid() && pos->lexeme != ")" && pos->lexeme != ",") {
        if constexpr (verbose) {
          std::cout << "...Scanning for op... [" << pos->lexeme << "]" << std::endl;
        }

        // If we have an operator, act on it!
        if (Has(binary_ops, pos->lexeme)) {
          const BinaryOperator & op = binary_ops[pos->lexeme];
          if (prec_limit >= op.prec) {
            return val1;  // Precedence not allowed; return current value.
          }
          ++pos;
          value_t val2 = ParseMath(symbols, pos, op.prec);
          if (val1.type == value_t::VALUE) {
            if (val2.type == value_t::VALUE) {
              val1 = op.fun(val1.value, val2.value);
            } else {
              val1 = [val1_num = val1.value, val2_fun = val2.fun, op_fun = op.fun](
                       arg_t symbol_vals) { return op_fun(val1_num, val2_fun(symbol_vals)); };
            }
          } else {
            if (val2.type == value_t::VALUE) {
              val1 = [val1_fun = val1.fun, val2_num = val2.value, op_fun = op.fun](
                       arg_t symbol_vals) { return op_fun(val1_fun(symbol_vals), val2_num); };
            } else {
              val1 =
                [val1_fun = val1.fun, val2_fun = val2.fun, op_fun = op.fun](arg_t symbol_vals) {
                  return op_fun(val1_fun(symbol_vals), val2_fun(symbol_vals));
                };
            }
          }
        } else {
          ParseError("Operator '", pos->lexeme, "' NOT found!");
        }
      }
    ...
    void SetupStaticValues(T1 arg1, Ts... args) {
      ...
      if constexpr (sizeof...(Ts) == 0 && (emp::is_emp_vector<T1>() || emp::is_std_vector<T1>())) {
        ...
        external_vals =
          emp::vector<emp::Datum>{static_cast<emp::Datum>(arg1), static_cast<emp::Datum>(args)...};
      }
    }
...
#endif  // #ifndef INCLUDE_EMP_DATA_SIMPLE_PARSER_HPP_GUARD
