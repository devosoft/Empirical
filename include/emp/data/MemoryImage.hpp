...
namespace emp {
  ...
  private:
    ...
    void Fuzz() {
      for (size_t i = init_to; i < size; i++) { image[i] = std::byte{85}; }
    }
  public:
    ...
    template <typename T = void>
    emp::Ptr<const T> GetPtr(size_t pos) const {
      if constexpr (!std::is_same_v<T, void>) {
        emp_assert(pos + sizeof(T) <= GetSize(), pos, sizeof(T), GetSize());
      }
    ...
    void RawResize(size_t new_size) {
      ...
      if (size) {
        ...
        image = nullptr;  // Or set to null if now empty.
      }
    }
...
#endif  // #ifndef INCLUDE_EMP_DATA_MEMORY_IMAGE_HPP_GUARD
