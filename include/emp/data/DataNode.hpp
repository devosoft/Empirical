...
namespace emp {
  ...
  using ModPack = emp::ValPack<(int) MODS...>;
  ...
  public:
    ...
    double GetMin() const {
      emp_assert(false, "Calculating min requires a DataNode with the Range or FullRange modifier");
      return 0;
    }

    double GetMax() const {
      emp_assert(false, "Calculating max requires a DataNode with the Range or FullRange modifier");
      return 0;
    }
    ...
    double GetStandardDeviation() const {
      emp_assert(
        false,
        "Calculating standard deviation requires a DataNode with the Stats or FullStats modifier");
      return 0;
    }
    ...
    double GetMedian() const {
      emp_assert(false, "Calculating median requires a DataNode with the Log modifier");
      return 0;
    }
    ...
    void PrintDebug(std::ostream & os = std::cout) { os << "BASE DataNodeModule.\n"; }
  };
...
/// This module lets you track the current (i.e. most recently added) value
  ...
  class DataNodeModule<VAL_TYPE, data::Current, MODS...>
    : public DataNodeModule<VAL_TYPE, MODS...> {
...
/// This module lets you log all of the values that have been added since the last re-set
  ...
  public:
    ...
    /// Calculate the median of observed values
    double GetMedian() const { return GetPercentile(50); }
    ...
    double GetPercentile(const double pct) const {
      ...
      return dup[idx] == dup[std::ceil(idx)]
             ? dup[idx]
             : (dup[idx] * (1.0 - emp::Mod(idx, 1.0)) + dup[std::ceil(idx)] * emp::Mod(idx, 1.0));
    }
...
#endif  // #ifndef DOXYGEN_SHOULD_SKIP_THIS : #else
  protected:
    emp::vector < emp::vector < VAL_TYPE >>
      archive;  ///< Data archived from before most recent reset.
...
/// (except mean, which is calculated from total and count).
  ...
  public:
    DataNodeModule() : total_vals(), num_vals(), min_vals(), max_vals() { ; }
...
/// specified by the other modules in this node.
  ...
  protected:
    ...
    void PullData_impl() {
      ...
      for (const auto & x : pull_sets) { in_vals.insert(in_vals.end(), x.begin(), x.end()); }
    }
#endif /*DOXYGEN_SHOULD_SKIP_THIS*/  // #ifndef DOXYGEN_SHOULD_SKIP_THIS
  ...
  template <typename VAL_TYPE, typename MOD_PACK>
  ...
  public:
    ...
    void PullData() {
      ...
      for (const VAL_TYPE & val : in_vals) {
        parent_t::AddDatum(val);  // Actually add the data.
      }
    }
...
#endif  // #ifndef INCLUDE_EMP_DATA_DATA_NODE_HPP_GUARD
