...
namespace emp {
  ...
  public:
    ...
    bool IsType() {
      return (bool) dynamic_cast<Trait<TARGET_T, VALUE_T>>(this);
    }
  };
  ...
  public:
    Trait(const std::string & _n, const fun_t & _f) : name(_n), desc(""), fun(_f), range() { ; }
    ...
    Ptr<BaseTrait<TARGET_T>> Clone() const { return NewPtr<this_t>(*this); }
  };
  ...
  public:
    ...
    void Clear() {
      for (auto & ptr : traits) { ptr.Delete(); }
      traits.resize(0);
    }
...
#endif  // #ifndef INCLUDE_EMP_DATA_TRAIT_HPP_GUARD
