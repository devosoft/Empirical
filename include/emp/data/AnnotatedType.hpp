...
namespace emp {
  ...
  public:
    ...
    template <typename T>
    bool TestTraitType(size_t id) const {
      return data_map.IsType<T>(id);
    }

    template <typename T>
    bool TestTraitType(const std::string & name) const {
      return data_map.IsType<T>(name);
    }
    ...
    template <typename T>
    T & SetTrait(size_t id, const T & val) {
      return data_map.Set<T>(id, val);
    }

    template <typename T>
    T & SetTrait(const std::string & name, const T & val) {
      return data_map.Set<T>(name, val);
    }
...
#endif  // #ifndef INCLUDE_EMP_DATA_ANNOTATED_TYPE_HPP_GUARD
