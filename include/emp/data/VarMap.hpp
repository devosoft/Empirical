...
namespace emp {
  ...
  private:
    ...
    struct VarInfo : public VarBase {
      ...
      VarInfo(const std::string & name, const T & in_value) : VarBase(name), value(in_value) { ; }
  ...
  public:
    ...
    VarMap(const VarMap & _in) : vars(_in.vars.size()), id_map(_in.id_map) {
      ...
      for (size_t i = 0; i < vars.size(); i++) { vars[i] = _in.vars[i]->Clone(); }
    }
    ...
    void Set(const std::string & name, const T & value) {
      if (auto it = id_map.find(name); it != id_map.end()) {
        Set(it->second, value);
      } else {
        Add(name, value);
      }
...
#endif  // #ifndef INCLUDE_EMP_DATA_VAR_MAP_HPP_GUARD
