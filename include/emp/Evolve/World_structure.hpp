...
namespace emp {
  ...
  public:
    ...
    WorldPosition(size_t _id, size_t _pop_id = 0)
      : index((uint32_t) _id), pop_id((uint32_t) _pop_id) {
  ...
  public:
    ...
    void MakeValid(WorldPosition pos) {
      ...
      const size_t id     = pos.GetIndex();
      emp_assert(pop_id < 2);
      if (id >= base_t::operator[](pop_id).size()) { base_t::operator[](pop_id).resize(id + 1); }
    }
  ...
  void SetPools(World<ORG> & world,
    ...
    if (synchronous_gen) {
      ...
      world.SetAddBirthFun([&world, pool_size]([[maybe_unused]] Ptr<ORG> new_org,
        ...
        WorldPosition pos =
          world.GetRandomNeighborPos(parent_pos);  // Placed near parent, in next pop.
  ...
  void SetMapElites(World<ORG> & world,
    ...
    world.SetAddInjectFun([&world, traits, trait_counts](Ptr<ORG> new_org) {
      ...
      if (cur_fitness > org_fitness) {
        return WorldPosition();  // Return invalid position!
      }
    ...
    world.SetAddBirthFun(
      [&world, traits, trait_counts](Ptr<ORG> new_org, WorldPosition parent_pos) {
  ...
  template <typename ORG>
  void SetMapElites(World<ORG> & world) {
    SetMapElites(world, world.GetPhenotypes());
  }
  ...
  struct World_MinDistInfo {
    ...
    void Refresh(size_t refresh_id, size_t start_id = 0) {
      ...
      for (size_t trait_id = start_id; trait_id < traits.GetSize(); trait_id++) {
        ...
        if (prev_bin_id < num_total_bins) { Refresh_AgainstBin(refresh_id, prev_bin_id); }
        size_t next_bin_id = bin_id + trait_offset;
        if (next_bin_id < num_total_bins) { Refresh_AgainstBin(refresh_id, next_bin_id); }
    ...
    size_t FindKill() {
      if (!is_setup) {
        Setup();  // The first time we run out of space and need to kill, setup structure!
      }
      ...
      emp_assert(nearest_id[min_id] >= 0 && nearest_id[min_id] < world.GetSize(),
                 ...
                 nearest_id[min_id],
                 distance.size());
      if (world.CalcFitnessID(min_id) < world.CalcFitnessID(nearest_id[min_id])) {
        return min_id;
      } else {
        return nearest_id[min_id];
      }
    }
    ...
    void Update(size_t pos) {
      ...
      if (update_chart == true) {
        ...
        for (size_t id = 0; id < world.GetSize(); id++) { Refresh(id); }
    ...
    bool OK() {
      ...
      if (is_setup) {
        ...
        for (size_t i = 0; i < num_orgs; i++) {
          emp_assert(org_bins[i] < num_total_bins,
                     i,
                     org_bins[i],
                     num_total_bins,
                     world.GetNumOrgs());
  ...
  void SetDiverseElites(World<ORG> & world, TraitSet<ORG> traits, size_t world_size) {
    ...
    // if a more fit organism is already in place; you must run clear first if you want to
    // ensure placement.
    world.SetAddInjectFun(
      [/*&world, traits,*/ world_size, info_ptr]([[maybe_unused]] Ptr<ORG> new_org) {
    ...
    // Birth is effectively the same as inject.
    world.SetAddBirthFun([/*&world, traits,*/ world_size,
                          info_ptr]([[maybe_unused]] Ptr<ORG> new_org, WorldPosition parent_pos) {
      (void) parent_pos;
      size_t pos = info_ptr->GetBirthPos(world_size);
      return WorldPosition(pos);
    });
...
#endif  // #ifndef INCLUDE_EMP_EVOLVE_WORLD_STRUCTURE_HPP_GUARD
