 ...
 *
 * @file include/emp/Evolve/World_output.hpp
 * @brief TODO.
 *
...
#ifndef INCLUDE_EMP_EVOLVE_WORLD_OUTPUT_HPP_GUARD
#define INCLUDE_EMP_EVOLVE_WORLD_OUTPUT_HPP_GUARD
...
namespace emp {
  ...
  DataFile & AddOEEFile(WORLD_TYPE & world,
                        OEE_TYPE & oee_tracker,
                        const std::string fpath = "oee_data.csv") {
  ...
  DataFile & AddPhylodiversityFile(WORLD_TYPE & world,
    ...
    file.AddFun(get_update, "update", "Update");
    file.AddStats(*sys->GetDataNode("evolutionary_distinctiveness"),
                  "evolutionary_distinctiveness",
                  "evolutionary distinctiveness for a single update",
                  true,
                  true);
    file.AddStats(*sys->GetDataNode("pairwise_distance"),
                  "pairwise_distance",
                  "pairwise distance for a single update",
                  true,
                  true);
    file.AddCurrent(*sys->GetDataNode("phylogenetic_diversity"),
  ...
  template <typename WORLD_TYPE>
  DataFile & AddLineageMutationFile(WORLD_TYPE & world,
                                    int systematics_id                 = 0,
    ...
    file.AddFun(get_update, "update", "Update");
    for (size_t i = 0; i < mut_types.size(); i++) {
      file.AddStats(*sys->GetDataNode(mut_types[i] + "_mut_count"),
                    mut_types[i] + "_mutations_on_lineage",
                    "counts of" + mut_types[i] + "mutations along each lineage",
                    true,
                    true);
    }

    file.AddStats(*sys->GetDataNode("deleterious_steps"),
                  "deleterious_steps",
                  "counts of deleterious steps along each lineage",
                  true,
                  true);
    file.AddStats(*sys->GetDataNode("volatility"),
                  "taxon_volatility",
                  "counts of changes in taxon along each lineage",
                  true,
                  true);
    file.AddStats(*sys->GetDataNode("unique_taxa"),
                  "unique_taxa",
...
#endif  // #ifndef INCLUDE_EMP_EVOLVE_WORLD_OUTPUT_HPP_GUARD
