...
namespace emp {
  ...
  public:
    ...
    Resource(double amt, double in, double out) : amount(amt), inflow(in), outflow(out) {}
  ...
  void ResourceSelect(World<ORG> & world,
                      emp::vector<std::function < double(ORG &)> > &extra_funs,
...
#endif  // #ifndef INCLUDE_EMP_EVOLVE_RESOURCE_HPP_GUARD
