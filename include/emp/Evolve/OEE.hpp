 ...
 *
 * @file include/emp/Evolve/OEE.hpp
 * @brief TODO.
 */

#pragma once

#ifndef INCLUDE_EMP_EVOLVE_OEE_HPP_GUARD
#define INCLUDE_EMP_EVOLVE_OEE_HPP_GUARD
...
namespace emp {
  ...
  template <typename SYSTEMATICS_TYPE,
            typename SKEL_TYPE = typename SYSTEMATICS_TYPE::info_t,
            typename SEEN_TYPE = SeenSet<SKEL_TYPE>>
  class OEETracker {
  private:
    ...
    using fun_calc_data_t =
      std::function<SKEL_TYPE(Ptr<taxon_t> &)>;  // TODO: Allow other skeleton types
  ...
  public:
    OEETracker(Ptr<systematics_t> s,
               fun_calc_data_t d,
               fun_calc_complexity_t c,
               bool remove_top             = false,
               int bloom_count             = 200000,
               double bloom_false_positive = .0001)
      : systematics_manager(s)
      , skeleton_fun(d)
      , complexity_fun(c)
      , seen(bloom_count, bloom_false_positive)
      , prune_top(remove_top) {
      emp_assert(
        s->GetStoreAncestors(),
        "OEE tracker only works with systematics manager where store_ancestor is set to true");
    ...
    void Update(size_t gen, int ud = -1) {
      if (Mod((int) gen, resolution) == 0) {
        ...
        if ((int) snapshots.size() > generation_interval / resolution + 1) {
          if (prune_top) { systematics_manager->RemoveBefore(snapshot_times.front() - 1); }
    ...
    std::map<SKEL_TYPE, int> CoalescenceFilter(size_t /*ud*/) {
      emp_assert(emp::Mod(generation_interval, resolution) == 0,
                 "Generation interval must be a multiple of resolution",
                 generation_interval,
                 resolution);

      std::map<SKEL_TYPE, int> res;

      if ((int) snapshots.size() <= generation_interval / resolution) { return res; }

      std::set<Ptr<taxon_t>> extant_canopy_roots =
        systematics_manager->GetCanopyExtantRoots(snapshot_times.front());
  ...
  emp::vector<INST_TYPE> Skeletonize(ORG_TYPE & org,
                                     const INST_TYPE null_value,
                                     std::function<double(ORG_TYPE &)> fit_fun) {
    ...
    for (int i = 0; i < (int) org.size(); i++) {
      ...
      if (new_fitness < fitness) { skeleton.push_back(org[i]); }
      test_org[i] = org[i];
    }
...
#endif  // #ifndef INCLUDE_EMP_EVOLVE_OEE_HPP_GUARD
