 ...
 *
 * @file include/emp/Evolve/NK-const.hpp
...
#ifndef INCLUDE_EMP_EVOLVE_NK_CONST_HPP_GUARD
#define INCLUDE_EMP_EVOLVE_NK_CONST_HPP_GUARD
...
namespace emp::evo {
  ...
  /// "tunably rugged" - a useful feature, since the ruggedness of the fitness landscape is
  /// thought to be important to many evolutionary dynamics. For each possible value that a site
  /// and its K neighbors to the right can have, a random fitness contribution is chosen. These
  /// contributions are summed across the bitstring. So when K = 0, each site has a single optimal
  /// value, resulting in a single smooth fitness peak.
  ///
  ...
  /// Note: Overly large Ns and Ks currently trigger a seg-fault, caused by trying to build a
  /// table that is larger than will fit in memory. You can use larger values of N and K (for
  /// slightly reduced speed) of you use an NKLandscape object instead.
