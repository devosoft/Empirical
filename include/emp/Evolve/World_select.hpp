 ...
 *
 * @file include/emp/Evolve/World_select.hpp
 * @brief Functions for popular selection methods applied to worlds.
 */

#pragma once

#ifndef INCLUDE_EMP_EVOLVE_WORLD_SELECT_HPP_GUARD
#define INCLUDE_EMP_EVOLVE_WORLD_SELECT_HPP_GUARD
...
namespace emp {
  ...
  void TournamentSelect(World<ORG> & world, size_t t_size, size_t tourney_count = 1) {
    emp_assert(t_size > 0,
               "Cannot have a tournament with zero organisms.",
               t_size,
               world.GetNumOrgs());
    emp_assert(t_size <= world.GetNumOrgs(),
               "Tournament too big for world.",
               t_size,
               world.GetNumOrgs());
  ...
  void LocalTournamentSelect(World<ORG> & world, size_t t_size, size_t tourney_count = 1) {
    ...
    for (size_t T = 0; T < tourney_count; T++) {
      ...
      // Choose organisms for this tournament (with replacement!)
      for (size_t i = 1; i < t_size; i++) {
        entries.push_back(world.GetRandomNeighborPos(entries[0]).GetPopID());
      }
  ...
  void LexicaseSelect(World<ORG> & world,
    ...
    for (size_t repro = 0; repro < repro_count; ++repro) {
      ...
      for (size_t fit_id : order) {
        ...
        if (cur_orgs.size() == 1) {
          break;  // Stop if we're down to just one organism.
        }
      }
  ...
  void OptimizedLexicaseSelect(World<ORG> & world,
    ...
    for (size_t i = 0; i < genotype_lists.size(); i++) { all_gens[i] = i; }
    ...
    for (size_t repro = 0; repro < repro_count; ++repro) {
      ...
      size_t options = 0;
      for (size_t gen : cur_gens) { options += genotype_lists[gen].size(); }
  ...
  void EcoSelect(World<ORG> & world,
    ...
    if (world.IsCacheOn()) { world.ClearCache(); }
    ...
    for (size_t i = 0; i < extra_funs.size(); i++) { extra_fitnesses[i].resize(world.GetSize()); }
    ...
    for (size_t ex_id = 0; ex_id < extra_funs.size(); ex_id++) {
      if (max_count[ex_id] == 0) {
        continue;  // No one gets this reward...
      }
    ...
    for (size_t T = 0; T < tourney_count; T++) {
      ...
      for (size_t i = 0; i < t_size; i++) {
        entries.push_back(world.GetRandomOrgID());  // Allows replacement!
      }
...
#endif  // #ifndef INCLUDE_EMP_EVOLVE_WORLD_SELECT_HPP_GUARD
