...
namespace emp {
  ...
  private:
    ...
    inline std::string GenerateSignalName(const std::string & pre = "") {
      if (pre == "") {
        return emp::to_string(prefix, next_id++);  // Use default version.
      }
  ...
  public:
    ...
    SignalManager(const SignalManager & in) : signal_map(), next_id(in.next_id), prefix(in.prefix) {
      ...
      for (const auto & x : in.signal_map) { signal_map[x.first] = x.second->Clone(); }
    }

    ~SignalManager() {
      for (auto & x : signal_map) { delete x.second; }
    }
...
#endif  // #ifndef INCLUDE_EMP_CONTROL_SIGNAL_MANAGER_HPP_GUARD
