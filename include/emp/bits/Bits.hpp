...
namespace emp {
  ...
  class Bits {
    ...
    [[nodiscard]] static constexpr size_t FieldPos(const size_t index) {
      return index & (FIELD_BITS - 1);
    }

    // Identify which field a specified byte position would be in.
    [[nodiscard]] static constexpr size_t Byte2Field(const size_t index) {
      return index / sizeof(field_t);
    }

    // Convert a byte position in Bits to a byte position in the target field.
    [[nodiscard]] static constexpr size_t Byte2FieldPos(const size_t index) {
  ...
  public:
    ...
    template <typename T>
    Bits(const std::initializer_list<T> l);
    ...
    template <size_t NUM_BITS>
    [[nodiscard]] Bits<Bits_FixedData<NUM_BITS>, true> ExportArray(size_t start_bit = 0) const {
    ...
    template <typename T>
    Bits & SetValueAtIndex(const size_t index, T value);
    ...
    template <typename T>
    Bits & SetValueAtBit(const size_t index, T value);
    ...
    void ForEachPair(FUN_T && fun) const {
      for (size_t i = FindOne(); i < GetSize(); i = FindOne(i + 1)) {
        for (size_t j = FindOne(i + 1); j < GetSize(); j = FindOne(j + 1)) { fun(i, j); }
      }
    }
    ...
    Bits & SetFirstBitCombo(size_t k) {
      emp_assert(k < GetSize(),
                 "Can only search through combos smaller than total bit count.",
                 k,
                 GetSize());
      Clear();
    ...
    bool NextBitCombo() {
      emp_assert(!All() && !None(),
                 All(),
                 None(),
                 "Must have both zeros and ones to step through combos.");
    ...
    Bits & operator>>=(const size_t shift_size) { return SHIFT_SELF(static_cast<int>(shift_size)); }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  void Bits<DATA_T, ZERO_LEFT>::RawCopy(const Ptr<const bits_field_t> from, size_t num_fields) {
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  constexpr void Bits<DATA_T, ZERO_LEFT>::RawMove(const size_t from_start,
                                                  const size_t from_stop,
                                                  const size_t to) {
    ...
    // Calculate how far will the moved piece needs to shift.
    const int shift = static_cast<int>(from_start) - static_cast<int>(to);
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <typename FUN_T>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::ApplyRange(const FUN_T & fun,
                                                                size_t start,
                                                                size_t stop) {
  ...
  constexpr void Bits<DATA_T, ZERO_LEFT>::ShiftRight(const size_t shift_size, bool raw) {
    ...
    if (field_shift) {
      for (size_t i = 0; i < field_shift2; ++i) { _data.bits[i] = _data.bits[i + field_shift]; }
  ...
  constexpr void Bits<DATA_T, ZERO_LEFT>::ROTL_SELF(const size_t shift_size_raw) {
    if (GetSize() == 0) {
      return;  // Nothing to rotate if there are not bits.
    }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <typename DATA2_T, bool ZERO_LEFT2>
  Bits<DATA_T, ZERO_LEFT>::Bits(const Bits<DATA2_T, ZERO_LEFT2> & in) : _data(in.GetSize()) {
    emp_assert(in.OK());
    RawCopy(in);
  }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <size_t NUM_BITS>
  Bits<DATA_T, ZERO_LEFT>::Bits(const std::bitset<NUM_BITS> & bitset) : _data(NUM_BITS) {
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT>::Bits(std::integral auto num_bits,
                                Random & random,
                                const std::integral auto target_ones)
    : _data(num_bits) {
    Clear();
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <typename T>
  Bits<DATA_T, ZERO_LEFT>::Bits(const std::initializer_list<T> l) : _data(l.size()) {
    Clear();
    size_t idx = 0;
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <typename DATA2_T, bool ZERO_LEFT2>
  Bits<DATA_T, ZERO_LEFT>::Bits(const Bits<DATA2_T, ZERO_LEFT2> & in, size_t new_size)
    : Bits(new_size) {
    emp_assert(in.OK());
  ...
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::operator=(
    const Bits<DATA_T, ZERO_LEFT> & in) & {
    emp_assert(in.OK());
    if (&in != this) {
  ...
  template <typename DATA2_T, bool ZERO_LEFT2>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::operator=(
    const Bits<DATA2_T, ZERO_LEFT2> & in) & {
    emp_assert(in.OK());
    Resize(in.GetSize());
    RawCopy(in);
  ...
  /// Move operator.
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::operator=(Bits<DATA_T, ZERO_LEFT> && in) & {
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <typename DATA2_T, bool ZERO_LEFT2>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::Import(
    ...
    for (size_t i = 0; i < copy_size; ++i) { Set(i, from_bits[i + from_start_pos]); }
  ...
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::Set(emp::String str) {
    ...
    if (str.PopIf('{')) {
      ...
      if (_data.IsFixedSize() == false) { _data.RawResize(CountBits(str)); }
      ...
      for (char c : str) {
        ...
        if (c == '0') {
          ++pos;  // Leave position as zero and move to next pos.
        }
      }
    }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::ChooseRandom(
    Random & random,
    const std::integral auto target_ones,
    const size_t start_pos,
    size_t stop_pos) {
    ...
    if (p < 0.12) {
      if (target_size == GetSize()) { Clear(start_pos, stop_pos); }
    } else if (p < 0.2) {
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::FlipRandom(Random & random,
                                                                const double p,
                                                                const size_t start_pos,
                                                                size_t stop_pos) {
    if (stop_pos == MAX_SIZE_T) { stop_pos = GetSize(); }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::FlipRandomCount(Random & random,
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::SetRandomCount(Random & random,
                                                                    const size_t target_bits) {
    emp_assert(target_bits <= GetSize());
    Bits<DATA_T, ZERO_LEFT> choice(GetSize(), random, target_bits);
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::ClearRandomCount(Random & random,
                                                                      const size_t target_bits) {
    emp_assert(target_bits <= GetSize());
  ...
  bool Bits<DATA_T, ZERO_LEFT>::operator<(const Bits<DATA2_T, ZL2> & in) const {
    ...
    if constexpr (IsAutoResize() || DATA2_T::IsAutoResize()) {
      ...
      for (size_t i = MIN_FIELDS; i > 0; --i) {
        ...
        if (_data.bits[pos] == in_fields[pos]) {
  ...
  Bits<DATA_T, ZERO_LEFT>::operator emp::vector<T>() {
    ...
    for (size_t i = 0; i < GetSize(); i++) { out[i] = (T) Get(i); }
    return out;
  }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  template <typename T>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::SetValueAtIndex(const size_t index,
                                                                     T in_value) {
    // @CAO For the moment, must fit inside bounds; eventually should pad with zeros.
    emp_assert((index + 1) * sizeof(T) <= _data.TotalBytes());
  ...
  void Bits<DATA_T, ZERO_LEFT>::Insert(const size_t index, const bool val, const size_t num) {
    ...
    if (val) {
      SetRange(index, index + num);  // If new bits should be ones, make it so.
    }
  ...
  size_t Bits<DATA_T, ZERO_LEFT>::FindOne(const size_t start_pos) const {
    if (start_pos >= GetSize()) {
      return npos;  // If we are past the end, return fail.
    }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
    ...
    for (size_t start_pos = FindOne(), end_pos; start_pos < size();
         start_pos        = FindOne(end_pos + 1)) {
  ...
  /// Print a space between each field (or other provided spacer)
  template <typename DATA_T, bool ZERO_LEFT>
  void Bits<DATA_T, ZERO_LEFT>::PrintDebug(std::ostream & out, const emp::String & label) const {
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::AND_SELF(
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::XOR_SELF(
    const Bits<DATA_T, ZERO_LEFT> & bits2) {
    if constexpr (IsAutoResize()) {
      if (GetSize() < bits2.GetSize()) { Resize(bits2.GetSize()); }
    }
  ...
  template <typename DATA_T, bool ZERO_LEFT>
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::EQU_SELF(
    const Bits<DATA_T, ZERO_LEFT> & bits2) {
    if constexpr (IsAutoResize()) {
      if (GetSize() < bits2.GetSize()) { Resize(bits2.GetSize()); }
    }
  ...
  Bits<DATA_T, ZERO_LEFT> Bits<DATA_T, ZERO_LEFT>::SHIFT(const int shift_size) const {
    Bits<DATA_T, ZERO_LEFT> out_bits(*this);
    if (shift_size > 0) {
      out_bits.ShiftRight(static_cast<size_t>(shift_size));
    } else if (shift_size < 0) {
      out_bits.ShiftLeft(static_cast<size_t>(-shift_size));
    }
    return out_bits;
  }
  ...
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::SHIFT_SELF(const int shift_size) {
    if (shift_size > 0) {
      ShiftRight(static_cast<size_t>(shift_size));
    } else if (shift_size < 0) {
      ShiftLeft(static_cast<size_t>(-shift_size));
    }
    return *this;
  }
  ...
  Bits<DATA_T, ZERO_LEFT> Bits<DATA_T, ZERO_LEFT>::ROTATE(const int rotate_size) const {
    Bits<DATA_T, ZERO_LEFT> out_set(*this);
    if (rotate_size > 0) {
      out_set.ROTR_SELF((field_t) rotate_size);
    } else if (rotate_size < 0) {
      out_set.ROTL_SELF((field_t) (-rotate_size));
    }
    return out_set;
  }
  ...
  Bits<DATA_T, ZERO_LEFT> & Bits<DATA_T, ZERO_LEFT>::ROTATE_SELF(const int rotate_size) {
    if (rotate_size > 0) {
      ROTR_SELF((field_t) rotate_size);
    } else if (rotate_size < 0) {
      ROTL_SELF((field_t) -rotate_size);
    }
    return *this;
  }
...
namespace std {
  ...
  struct hash<emp::Bits<DATA_T, ZERO_LEFT>> {
    std::size_t operator()(const emp::Bits<DATA_T, ZERO_LEFT> & bits) const { return bits.Hash(); }
  };
}  // namespace std

#endif  // #ifndef INCLUDE_EMP_BITS_BITS_HPP_GUARD
