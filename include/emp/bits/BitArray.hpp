...
namespace emp::old {
  ...
  class BitArray {
    ...
    template <size_t FRIEND_BITS, bool FRIEND_LEFT>
  ...
  private:
    ...
    void ClearExcessBits() noexcept {
      if constexpr (NUM_END_BITS > 0) { bits[LAST_FIELD] &= END_MASK; }
    }
  ...
  public:
    ...
    explicit BitArray(bool init_val = false) noexcept {
      if (init_val) {
        SetAll();
      } else {
        Clear();
    ...
    BitArray(Random & random) {
      Clear();
      Randomize(random);
    }

    /// Constructor to generate a random BitArray with provided PROBABILITY of 1's.
    BitArray(Random & random, double p1) {
      Clear();
      Randomize(random, p1);
    }

    /// Constructor to generate a random BitArray with provided NUMBER of 1's.
    BitArray(Random & random, size_t num_ones) {
      Clear();
      ChooseRandom(random, num_ones);
    }

    /// Constructor to generate a random BitArray with provided NUMBER of 1's.
    BitArray(Random & random, int num_ones) {
      Clear();
      ChooseRandom(random, num_ones);
    }
    ...
    BitArray & operator=(const this_t & in_bits) & noexcept {
      return Copy<NUM_FIELDS>(in_bits.bits);
    }
    ...
    BitArray & Clear() noexcept {
      for (field_t & x : bits) { x = FIELD_0; }
      return *this;
    }
    ...
    [[nodiscard]] std::size_t GetSizeT(size_t index) const {
      return GetValueAtIndex<size_t>(index);
    }
    ...
    [[nodiscard]] uint16_t GetUInt16(size_t index) const {
      return GetValueAtIndex<uint16_t>(index);
    }

    /// Retrieve the 32-bit uint from the specified uint index.
    [[nodiscard]] uint32_t GetUInt32(size_t index) const {
      return GetValueAtIndex<uint32_t>(index);
    }

    /// Retrieve the 64-bit uint from the specified uint index.
    [[nodiscard]] uint64_t GetUInt64(size_t index) const {
      return GetValueAtIndex<uint64_t>(index);
    }
    ...
    /// Retrieve the 8-bit uint from the specified uint index.
    [[nodiscard]] uint8_t GetUInt8AtBit(size_t index) const {
      return GetValueAtBit<uint8_t>(index);
    }

    /// Retrieve the 16-bit uint from the specified uint index.
    [[nodiscard]] uint16_t GetUInt16AtBit(size_t index) const {
      return GetValueAtBit<uint16_t>(index);
    }

    /// Retrieve the 32-bit uint from the specified uint index.
    [[nodiscard]] uint32_t GetUInt32AtBit(size_t index) const {
      return GetValueAtBit<uint32_t>(index);
    }

    /// Retrieve the 64-bit uint from the specified uint index.
    [[nodiscard]] uint64_t GetUInt64AtBit(size_t index) const {
      return GetValueAtBit<uint64_t>(index);
    }
    ...
    [[deprecated("Renamed to more accurate FindOne(start_pos)")]] [[nodiscard]] int FindBit(
    ...
    /// Operator shift left...
    [[nodiscard]] BitArray operator<<(const size_t shift_size) const {
      return SHIFT(-(int) shift_size);
    }
  ...
  template <typename FUN_T>
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::ApplyRange(const FUN_T & fun,
                                                                            size_t start,
                                                                            size_t stop) {
    if (start == stop) {
    ...
    const size_t stop_field =
      FieldID(stop - 1);  // Identify the last field where we actually make a change.
    ...
    if (start_field == stop_field) {
      ...
      const field_t mask      = MaskLow<field_t>(apply_bits)
                        << start_pos;        // Target change bits with a mask.
    ...
    // Otherwise mask the ends and fully modify the chunks in between.
      ...
      if (start_pos != 0) {
        ...
        const field_t mask      = MaskLow<field_t>(start_bits)
                          << start_pos;        // Target start bits with a mask.
        field_t & target = bits[start_field];  // Isolate the field to change.
        target           = (target & ~mask) | (fun(target) & mask);  // Update targeted bits!
  ...
  void BitArray<NUM_BITS, ZERO_LEFT>::ShiftLeft(const size_t shift_size) {
    ...
    if (field_shift) {
      for (size_t i = LAST_FIELD; i >= field_shift; --i) { bits[i] = bits[i - field_shift]; }
  ...
  void BitArray<NUM_BITS, ZERO_LEFT>::ShiftRight(const size_t shift_size) {
    ...
    if (field_shift) {
      for (size_t i = 0; i < (NUM_FIELDS - field_shift); ++i) { bits[i] = bits[i + field_shift]; }
  ...
  void BitArray<NUM_BITS, ZERO_LEFT>::RotateLeft(const size_t shift_size_raw) {
    ...
    if constexpr (NUM_FIELDS == 1) {
      ...
      const int field_shift = NUM_END_BITS ? ((shift_size + FIELD_BITS - NUM_END_BITS) / FIELD_BITS)
      ...
      std::rotate(std::rbegin(bits), std::rbegin(bits) + field_shift, std::rend(bits));
      ...
      // account for bit_shift
      if (bit_shift) {
        const field_t keystone = NUM_END_BITS ? ((bits[LAST_FIELD] << (FIELD_BITS - NUM_END_BITS)) |
  ...
  void BitArray<NUM_BITS, ZERO_LEFT>::RotateRight(const size_t shift_size_raw) {
    ...
    if constexpr (NUM_FIELDS == 1) {
      ...
      // if rotating more than field capacity, we need to rotate fields
      std::rotate(std::begin(bits), std::begin(bits) + field_shift, std::end(bits));
      ...
      // account for bit_shift
      if (bit_shift) {
        const field_t keystone =
          NUM_END_BITS ? (bits[0] >> (FIELD_BITS - NUM_END_BITS)) : (bits[0]);

        if constexpr (NUM_END_BITS > 0) { bits[NUM_FIELDS - 1] |= bits[0] << NUM_END_BITS; }
  ...
  // Assignment operator from a std::bitset.
  template <size_t NUM_BITS, bool ZERO_LEFT>
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::operator=(
    ...
    if (from_bit % 8) {
      ...
      if (FROM_BYTES > COPY_BYTES) {
        reinterpret_cast<unsigned char *>(bits)[COPY_BYTES - 1] |=
          (reinterpret_cast<const unsigned char *>(from_array.bits)[from_bit / 8 + COPY_BYTES]
           << (8 - from_bit % 8));
      }
    }
  ...
  template <size_t NUM_BITS, bool ZERO_LEFT>
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::ChooseRandom(
    Random & random,
    const size_t target_ones,
    const size_t start_pos,
    const size_t stop_pos) {
    ...
    if (p < 0.12) {
      if (target_size == NUM_BITS) { Clear(start_pos, stop_pos); }
    } else if (p < 0.2) {
      RandomizeP<Random::PROB_12_5>(random, start_pos, stop_pos);
    } else if (p < 0.35) {
      RandomizeP<Random::PROB_25>(random, start_pos, stop_pos);
    } else if (p < 0.42) {
      RandomizeP<Random::PROB_37_5>(random, start_pos, stop_pos);
    } else if (p < 0.58) {
      RandomizeP<Random::PROB_50>(random, start_pos, stop_pos);
    } else if (p < 0.65) {
      RandomizeP<Random::PROB_62_5>(random, start_pos, stop_pos);
    } else if (p < 0.8) {
      RandomizeP<Random::PROB_75>(random, start_pos, stop_pos);
    } else if (p < 0.88) {
      RandomizeP<Random::PROB_87_5>(random, start_pos, stop_pos);
    } else {
      SetRange(start_pos, stop_pos);
    }
  ...
  // Unset random bits with a given probability (does not check if already zero.)
  template <size_t NUM_BITS, bool ZERO_LEFT>
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::
    ClearRandom(Random & random, const double p, const size_t start_pos, const size_t stop_pos) {
    emp_assert(start_pos <= stop_pos);
    emp_assert(stop_pos <= NUM_BITS);
    emp_assert(p >= 0.0 && p <= 1.0, p);

    for (size_t i = start_pos; i < stop_pos; ++i) {
      if (random.P(p)) { Clear(i); }
    }
  ...
  BitArray<NUM_BITS, ZERO_LEFT>::FlipRandomCount(Random & random, const size_t num_bits) {
  ...
  template <size_t NUM_BITS, bool ZERO_LEFT>
  BitArray<NUM_BITS, ZERO_LEFT> &
  BitArray<NUM_BITS, ZERO_LEFT>::SetRandomCount(Random & random, const size_t num_bits) {
    emp_assert(num_bits <= NUM_BITS);
    this_t target_bits(random, num_bits);
    return *this |= target_bits;
  }
  ...
  template <size_t NUM_BITS, bool ZERO_LEFT>
  BitArray<NUM_BITS, ZERO_LEFT> &
  BitArray<NUM_BITS, ZERO_LEFT>::ClearRandomCount(Random & random, const size_t num_bits) {
    emp_assert(num_bits <= NUM_BITS);
  ...
  bool BitArray<NUM_BITS, ZERO_LEFT>::operator<(const BitArray<SIZE2, LEFT2> & in_bits) const {
    ...
    for (int i = NUM_FIELDS - 1; i >= 0; --i) {  // Start loop at the largest field.
      if (bits[i] == in_bits.bits[i]) {
        continue;  // If same, keep looking!
      }
  ...
  std::span<const std::byte> BitArray<NUM_BITS, ZERO_LEFT>::GetBytes() const {
    return std::span<const std::byte>(reinterpret_cast<const std::byte *>(bits), TOTAL_BYTES);
  }
  ...
  std::size_t BitArray<NUM_BITS, ZERO_LEFT>::Hash() const noexcept {
    ...
    if constexpr (std::is_same_v<field_t, size_t>) { return hash_combine(bits, NUM_FIELDS); }
  ...
  int BitArray<NUM_BITS, ZERO_LEFT>::FindOne() const {
    ...
    return (field_id < NUM_FIELDS) ? (int) (find_bit(bits[field_id]) + (field_id << FIELD_LOG2))
                                   : -1;
  }
  ...
  int BitArray<NUM_BITS, ZERO_LEFT>::FindOne(const size_t start_pos) const {
    if (start_pos >= NUM_BITS) {
      return -1;  // If we're past the end, return fail.
    }
    ...
    while (field_id < NUM_FIELDS && bits[field_id] == 0) { field_id++; }
    return (field_id < NUM_FIELDS) ? (int) (find_bit(bits[field_id]) + (field_id * FIELD_BITS))
                                   : -1;
  }
  ...
  // Print a space between each field (or other provided spacer)
  template <size_t NUM_BITS, bool ZERO_LEFT>
  void BitArray<NUM_BITS, ZERO_LEFT>::PrintFields(std::ostream & out,
  ...
  // Print the locations of all one bits, using the provided spacer (default is a single space)
  template <size_t NUM_BITS, bool ZERO_LEFT>
  void BitArray<NUM_BITS, ZERO_LEFT>::PrintOneIDs(std::ostream & out,
                                                  const std::string & spacer) const {
    bool started = false;
    for (size_t i = 0; i < NUM_BITS; i++) {
      if (Get(i)) {
  ...
  BitArray<NUM_BITS, ZERO_LEFT> BitArray<NUM_BITS, ZERO_LEFT>::SHIFT(const int shift_size) const {
    ...
    if (shift_size > 0) {
      out_array.ShiftRight((field_t) shift_size);
    } else if (shift_size < 0) {
      out_array.ShiftLeft((field_t) (-shift_size));
    }
    return out_array;
  }
  ...
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::SHIFT_SELF(const int shift_size) {
    if (shift_size > 0) {
      ShiftRight((field_t) shift_size);
    } else if (shift_size < 0) {
      ShiftLeft((field_t) -shift_size);
    }
    return *this;
  }
  ...
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::REVERSE_SELF() {
    ...
    if constexpr (filler_bits != 0) { this->ShiftRight(8 - filler_bits); }
  ...
  BitArray<NUM_BITS, ZERO_LEFT> BitArray<NUM_BITS, ZERO_LEFT>::ROTATE(const int rotate_size) const {
    this_t out_array(*this);
    if (rotate_size > 0) {
      out_array.RotateRight((field_t) rotate_size);
    } else if (rotate_size < 0) {
      out_array.RotateLeft((field_t) (-rotate_size));
    }
    return out_array;
  }
  ...
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::ROTATE_SELF(const int rotate_size) {
    if (rotate_size > 0) {
      RotateRight((field_t) rotate_size);
    } else if (rotate_size < 0) {
      RotateLeft((field_t) -rotate_size);
    }
    return *this;
  }
  ...
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::ROTR_SELF() {
    ...
    if constexpr (NUM_FIELDS == 1) {
      ...
      if (bit_shift) {
        ...
        if constexpr ((bool) NUM_END_BITS) { bits[NUM_FIELDS - 1] |= bits[0] << NUM_END_BITS; }
  ...
  BitArray<NUM_BITS, ZERO_LEFT> & BitArray<NUM_BITS, ZERO_LEFT>::ADD_SELF(const this_t & array2) {
    ...
    if constexpr (static_cast<bool>(NUM_END_BITS)) {
      bits[NUM_BITS / FIELD_BITS] =
        (bits[NUM_BITS / FIELD_BITS] + array2.bits[NUM_BITS / FIELD_BITS] +
  ...
  BitArray<NUM_BITS1 + NUM_BITS2> join(const BitArray<NUM_BITS1> & in1,
                                       const BitArray<NUM_BITS2> & in2) {
...
#endif  // #ifndef INCLUDE_EMP_BITS_BIT_ARRAY_HPP_GUARD
