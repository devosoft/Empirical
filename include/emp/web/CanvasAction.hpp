...
namespace emp { namespace web {
  ...
  protected:
    ...
    void Fill(const std::string & style = "") {
      if (style != "") {
        EM_ASM({ emp_i.ctx.fillStyle = UTF8ToString($0); }, style.c_str());
      }
    ...
    void Stroke(const std::string & style = "") {
      if (style != "") {
        EM_ASM({ emp_i.ctx.strokeStyle = UTF8ToString($0); }, style.c_str());
      }
  ...
  protected:
    ...
    double height;
  public:
    CanvasImage(const RawImage & raw_image,
                double _x = 0.0,
                double _y = 0.0,
                double _w = 0.0,
                double _h = 0.0)
    ...
    CanvasImage(const std::string & url,
                double _x = 0.0,
                double _y = 0.0,
                double _w = 0.0,
                double _h = 0.0)
...
#endif  // #ifndef INCLUDE_EMP_WEB_CANVAS_ACTION_HPP_GUARD
