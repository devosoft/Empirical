...
#define AlertVar(VAR) emp::Alert(std::string(#VAR) + std::string("=") + std::to_string(VAR))
    ...
    MAIN_THREAD_ASYNC_EM_ASM(
      {
        window.addEventListener(
          "resize",
          function() { emp.Callback($0, window.innerWidth, window.innerHeight); });
      },
      callback_id);
  }
  ...
  inline double GetTime() {
    return MAIN_THREAD_EM_ASM_DOUBLE({ return (new Date()).getTime(); });
  }

  /// Determine with width of the current window.
  inline int GetWindowInnerWidth() {
    return MAIN_THREAD_EM_ASM_INT({ return window.innerWidth; });
  }

  /// Determine with height of the current window.
  inline int GetWindowInnerHeight() {
    return MAIN_THREAD_EM_ASM_INT({ return window.innerHeight; });
  }
  ...
  emp::String MakeHTMLTrigger(emp::String text,
    ...
    if (style.size()) { style = emp::MakeString(" style=\"", style, "\""); }
    uint32_t callback_id = JSWrap(in_fun);
    return MakeString("<a href=\"#\" onclick=\"emp.Callback(",
                      callback_id,
                      "); return false;\"",
                      style,
                      ">",
...
#endif  // #ifndef INCLUDE_EMP_WEB_EMFUNCTIONS_HPP_GUARD
