 ...
 *
 * @file include/emp/web/d3/scales.hpp
 * @brief Tools for scaling graph axes in D3.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_SCALES_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_SCALES_HPP_GUARD
...
namespace D3 {
  ...
  public:
    ...
    Scale Copy() {
      ...
      MAIN_THREAD_EM_ASM({ js.objects[$1] = js.objects[$0].copy(); }, this->id, new_id);
      return Scale(new_id);
    }
  ...
  public:
    QuantizeScale() : Scale(true) {
      MAIN_THREAD_EM_ASM({js.objects[$0] = d3.scaleQuantize()}, this->id);
    }
    ...
    double InvertExtent(T y) {
      return EM_ASM_DOUBLE({ return js.objects[$0].invertExtent($1); }, this->id, y);
    }
  };

  class QuantileScale : public QuantizeScale {
  public:
    QuantileScale() : QuantizeScale(true) {
      MAIN_THREAD_EM_ASM({ js.objects[$0] = d3.scaleQuantile(); }, this->id);
    }
  ...
  public:
    ...
    IdentityScale & SetTickFormat(int count, std::string format) {
      ...
      // function without it?
      MAIN_THREAD_EM_ASM(
        { js.objects[$0].tick($1, UTF8ToString($2)); },
        this->id,
        count,
        format.c_str());
      return *this;
  ...
  PowScale SqrtScale() { return PowScale().Exponent(.5); }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_SCALES_HPP_GUARD
