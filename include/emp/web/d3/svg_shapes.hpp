 ...
 *
 * @file include/emp/web/d3/svg_shapes.hpp
 * @brief Tools to build common SVG shapes.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_SVG_SHAPES_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_SVG_SHAPES_HPP_GUARD
...
namespace D3 {
  ...
  public:
    ...
    void SetType(std::string type) {
      emp_assert(EM_ASM_INT(
        {
          return d3.symbolTypes.includes(UTF8ToString($0)) || window[UTF8ToString($0)] ==
               = "function" || window["d3"][UTF8ToString($0)] ==
               = "function" || window["emp"][UTF8ToString($0)] == = "function";
        },
        type.c_str()));
    ...
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetType(T type) {
    ...
    void SetSize(int size) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].size($1); }, this->id, size);
    }

    /// @cond TEMPLATES
    // Otherwise it's a function
    void SetSize(std::string size) { D3_CALLBACK_METHOD_1_ARG(size, size.c_str()) }

    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetSize(T type) {
  ...
  public:
    ...
    void SetCurve(std::string curve) { D3_CALLBACK_METHOD_1_ARG(curve, curve.c_str()) }

    /// If interpolation is "bundle", "cardinal", "cardinal-open", or "cardinal-closed", a tension
    /// parameter is used.
    void SetTension(float tension) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].tension($1); }, this->id, tension);
    }
    ...
    void SetDefined(std::string defined) { D3_CALLBACK_METHOD_1_ARG(defined, defined.c_str()) }

    /// @cond TEMPLATES
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetDefined(T defined) {
  ...
  public:
    ...
    void SetX(std::string x) { D3_CALLBACK_METHOD_1_ARG(x, x.c_str()) }

    /// @cond TEMPLATES
    // Handles setting x to a constant
    template <typename T>
    typename std::enable_if<std::is_fundamental<T>::value, void>::type SetX(T x) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].x($1); }, this->id, x);
    }

    // handles C++ functions
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetX(T x) {
    ...
    ///  };
    // Handles setting x accessor to a function or string
    void SetY(std::string y) { D3_CALLBACK_METHOD_1_ARG(y, y.c_str()) }

    /// @cond TEMPLATES

    // Handles constants
    template <typename T>
    typename std::enable_if<std::is_fundamental<T>::value, void>::type SetY(T y) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].y($1); }, this->id, y);
    }

    // handles C++ functions
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetY(T y) {
  ...
  public:
    ...
    void SetSource(std::string source) { D3_CALLBACK_METHOD_1_ARG(source, source.c_str()) }

    /// @cond TEMPLATES
    // Handles setting source to a constant
    template <typename T>
    typename std::enable_if<std::is_fundamental<T>::value, void>::type SetSource(T source) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].source($1); }, this->id, source);
    }

    // handles C++ functions
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetSource(T source) {
    ...
    void SetTarget(std::string target) { D3_CALLBACK_METHOD_1_ARG(target, target.c_str()) }

    /// @cond TEMPLATES

    // Handles constants
    template <typename T>
    typename std::enable_if<std::is_fundamental<T>::value, void>::type SetY(T target) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].target($1); }, this->id, target);
    }

    // handles C++ functions
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetY(T target) {
  ...
  public:
    ...
    void SetX0(std::string x) { D3_CALLBACK_METHOD_1_ARG(x0, x.c_str()) }

    // Handles setting y0 accessor to a function or string
    void SetY0(std::string y) { D3_CALLBACK_METHOD_1_ARG(y0, y.c_str()) }
    ...
    void SetX1(std::string x) { D3_CALLBACK_METHOD_1_ARG(x1, x.c_str()) }

    // Handles setting y1 accessor to a function or string
    void SetY1(std::string y) { D3_CALLBACK_METHOD_1_ARG(y1, y.c_str()) }
  };
  ...
  public:
    ...
    void SetRadius(std::string radius) { D3_CALLBACK_METHOD_1_ARG(radius, radius.c_str()) }

    void SetAngle(float angle) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].angle($1); }, this->id, angle);
    }

    void SetAngle(std::string angle) { D3_CALLBACK_METHOD_1_ARG(angle, angle.c_str()) }
  };
  ...
  public:
    ...
    void SetStartAngle(std::string angle) { D3_CALLBACK_METHOD_1_ARG(startAngle, angle.c_str()) }

    void SetEndAngle(float angle) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].endAngle($1); }, this->id, angle);
    }

    void SetEndAngle(std::string angle) { D3_CALLBACK_METHOD_1_ARG(endAngle, angle.c_str()) }
  };
  ...
  public:
    ...
    void SetPadRadius(std::string radius) { D3_CALLBACK_METHOD_1_ARG(padRadius, radius.c_str()) }

    void SetPadAngle(float angle) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].padAngle($1); }, this->id, angle);
    }

    void SetPadAngle(std::string angle) { D3_CALLBACK_METHOD_1_ARG(padAngle, angle.c_str()) }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_SVG_SHAPES_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: svg javascript sel api
