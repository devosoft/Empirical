 ...
 *
 * @file include/emp/web/d3/selection.hpp
 * @brief TODO.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_SELECTION_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_SELECTION_HPP_GUARD
...
namespace D3 {
  ...
  public:
    ...
    template <typename T>
    emp::sfinae_decoy<DERIVED &, decltype(&T::operator())> Call(T function) {
      uint32_t fun_id = emp::JSWrap(function, "", false);
      MAIN_THREAD_EM_ASM(
        {
          emp.__new_object =
            js.objects[$0].call(function(selection) { return emp.Callback($1, $0); });
        },
        this->id,
        fun_id);
      emp::JSDelete(fun_id);
    ...
    template <typename T>
    emp::sfinae_decoy<DERIVED, decltype(&T::operator())> Filter(T selector) const {
      int new_id = NextD3ID();
    ...
    template <typename T>
    emp::sfinae_decoy<DERIVED &, decltype(&T::operator())> Each(T function) {
    ...
    void Remove() { MAIN_THREAD_EM_ASM({js.objects[$0].remove()}, this->id); }
    ...
    */
    ...
    template <typename T>
    typename std::enable_if<std::is_fundamental<T>::value, DERIVED &>::type SetAttr(std::string name,
                                                                                    T value) {
      MAIN_THREAD_EM_ASM({js.objects[$0].attr(UTF8ToString($1), $2)}, this->id, name.c_str(), value);
      return *(static_cast<DERIVED *>(this));
    }
    ...
    template <typename T>
    emp::sfinae_decoy<DERIVED &, decltype(&T::operator())> SetAttr(std::string name, T value) {
      ...
      MAIN_THREAD_EM_ASM(
        {
          js.objects[$0].attr(
            UTF8ToString($1),
            function(d, i, k) { return emp.Callback($2, d, i, k); });
        },
        this->id,
        name.c_str(),
        fun_id);
    ...
    */
    ...
    template <typename T>
    typename std::enable_if<T::value_type != "", DERIVED &>::type SetAttr(std::string name,
                                                                          T value) {
    ...
    */
    ...
    // once and then pass the name as a string.
    template <typename T>
    emp::sfinae_decoy<DERIVED &, decltype(&T::operator())> SetStyle(std::string name, T value) {
    ...
    template <typename T>
    emp::sfinae_decoy<DERIVED &, decltype(&T::operator())> SetText(T func) {
...
// but we want it to all show up together in the docs
    ...
    int GetAttrInt(std::string name) const {
      return EM_ASM_INT({ return js.objects[$0].attr(UTF8ToString($1)); }, this->id, name.c_str());
    }

    /// Get the value of this object's [name] attribute when it's a double
    double GetAttrDouble(std::string name) const {
      return EM_ASM_DOUBLE({ return js.objects[$0].attr(UTF8ToString($1)); }, this->id, name.c_str());
    }
    ...
    int GetStyleInt(std::string name) const {
      return EM_ASM_INT({ return js.objects[$0].style(UTF8ToString($1)); }, this->id, name.c_str());
    }
    ...
    bool Empty() const {
      int empty = EM_ASM_INT({return Number(js.objects[$0].empty())}, this->id);
      return (bool) empty;
    }

    /// Returns number of elements in this selection
    int Size() const { return EM_ASM_INT({return js.objects[$0].size()}, this->id); }
  ...
  public:
      ...
      emp_assert(EM_ASM_INT(
                   {
                     ...
                     if (func_string == "null") { return true; }
                     if (typeof window[func_string] == = "function") {
                       func_string = window[func_string];
                     }
                     for (name in{d3: "d3", emp: "emp"}) {
                   ...
                   listener.c_str()) &&
                   "String passed to On is not s Javascript function or null",
                 listener);
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          if (typeof func_string == = "function") {
            js.objects[$0].on(UTF8ToString($1), func_string, $3);
          } else {
    ...
    template <typename T>
    emp::sfinae_decoy<Transition &, decltype(&T::operator())> On(std::string type,
                                                                 T listener,
                                                                 bool capture = false) {
    ...
    Transition & SetDuration(double time) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].duration($1); }, this->id, time);
      return (*this);
    }
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          js.objects[$0].each("end", function() { d3.select(this).property(arg1, func_string); });
        },
        name.c_str(),
        value.c_str());
      return *this;
    ...
    SetProperty(std::string name, T value) {
      MAIN_THREAD_EM_ASM(
        {
          js.objects[$0].each("end", function() { d3.select(this).property(UTF8ToString($1), $2); });
        },
        this->id,
        name.c_str());
      return *this;
    ...
    // once and then pass the name as a string.
    template <typename T>
    emp::sfinae_decoy<Transition &, decltype(&T::operator())> SetProperty(std::string name,
                                                                          T value) {
      uint32_t fun_id = emp::JSWrap(value, "", false);
      MAIN_THREAD_EM_ASM(
        {
          js.objects[$0].each(
            ...
            function() {
              d3.select(this).property(
                UTF8ToString($1),
                function(d, i, j) { return emp.Callback($2, d, i, j); });
            });
        },
        this->id,
        name.c_str(),
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          js.objects[$0].each("end", function() { d3.select(this).html(func_string); });
        },
        this->id,
        value.c_str());
      return *this;
    ...
    template <typename T>
    emp::sfinae_decoy<Transition &, decltype(&T::operator())> SetHtml(T func) {
      ...
      MAIN_THREAD_EM_ASM(
        {
          js.objects[$0].each(
            "end",
            function() {
              d3.select(this).html(function(d, i, j) { return emp.Callback($1, d, i, j); });
            });
        },
        this->id,
        fun_id);
    ...
    Transition & SetClassed(std::string classname, bool value) {
      MAIN_THREAD_EM_ASM(
        {
          js.objects[$0].each("end", function() { d3.select(this).classed(UTF8ToString($1), $2); });
        },
        this->id,
        classname.c_str(),
        value);
    ...
    template <typename T>
    emp::sfinae_decoy<Transition &, decltype(&T::operator())> SetClassed(std::string,
    ...
    Transition & SetClassed(std::string classname, std::string value) {
      emp_assert(EM_ASM_INT(
                   ...
                   value.c_str()) &&
                   "String passed to SetClassed is not a Javascript function",
                 value);
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          js.objects[$0].each("end", function() { d3.select(this).classed(arg1, func_string); });
        },
        classname.c_str(),
        value.c_str());
  ...
  public:
    ...
    Selection() {
      MAIN_THREAD_EM_ASM({ js.objects[$0] = d3.selection(); }, this->id);
    }
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          if (typeof fn == = "function") {
            ...
            var update_sel = js.objects[$0].data(js.objects[$2], window["d3"][in_string]);
          } else if (typeof window[in_string] == = "function") {
            var update_sel = js.objects[$0].data(js.objects[$2], window[in_string]);
          } else {
    ...
    template <typename T>
    emp::sfinae_decoy<Selection, decltype(&T::operator())> Data(Dataset & values, T key) {
      int update_id   = NextD3ID();
      ...
      MAIN_THREAD_EM_ASM(
        {
          var update_sel =
            js.objects[$0].data(js.objects[$2], function(d, i) { return emp.Callback($1, d, i); });
          js.objects[$3] = update_sel;
        },
        this->id,
        fun_id,
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          if (typeof fn == = "function") {
            ...
            var update_sel = js.objects[$0].data(emp_i.__incoming_array, window["d3"][in_string]);
          } else if (typeof window[in_string] == = "function") {
            var update_sel = js.objects[$0].data(emp_i.__incoming_array, window[in_string]);
          } else {
            var update_sel = js.objects[$0].data(emp_i.__incoming_array);
          }
    ...
    emp::sfinae_decoy<Selection, decltype(&T::operator())> Data(C values, T key) {
      int update_id = NextD3ID();
      emp::pass_array_to_javascript(values);
      uint32_t fun_id = emp::JSWrap(key, "", false);
      ...
      MAIN_THREAD_EM_ASM(
        {
          var update_sel = js.objects[$0].data(
            emp_i.__incoming_array,
            function(d, i, k) { return emp.Callback($1, d, i, k); });
          js.objects[$2] = update_sel;
        },
        this->id,
        fun_id,
    ...
    Dataset GetData() const {
      int new_id = NextD3ID();
      MAIN_THREAD_EM_ASM({ js.objects[$1] = [js.objects[$0].data()]; }, this->id, new_id);
    ...
    Selection EnterAppend(std::string type) {
      ...
      MAIN_THREAD_EM_ASM(
        {
          var append_selection = js.objects[$0].enter().append(UTF8ToString($1));
    ...
    Selection EnterInsert(std::string name, std::string before = NULL) {
      ...
      if (before.c_str()) {
        MAIN_THREAD_EM_ASM(
          {
            var new_sel    = js.objects[$0].enter().insert(UTF8ToString($1), UTF8ToString($2));
    ...
    SetProperty(std::string name, T value) {
      MAIN_THREAD_EM_ASM({js.objects[$0].property(UTF8ToString($1), $2)}, this->id, name.c_str());
      return *this;
    }
    ...
    // once and then pass the name as a string.
    template <typename T>
    emp::sfinae_decoy<Selection &, decltype(&T::operator())> SetProperty(std::string name,
                                                                         T value) {
    ...
    template <typename T>
    emp::sfinae_decoy<Selection &, decltype(&T::operator())> SetHtml(T func) {
    ...
    // Version for C++ function
    template <typename T>
    emp::sfinae_decoy<Selection &, decltype(&T::operator())> SetClassed(std::string,
                                                                        std::string classname,
                                                                        T func) {
    ...
    Selection Insert(std::string name, std::string before = NULL) {
      ...
      if (before.c_str()) {
        MAIN_THREAD_EM_ASM(
          {
            var new_sel    = js.objects[$0].insert(UTF8ToString($1), UTF8ToString($2));
            js.objects[$3] = new_sel;
          },
          this->id,
          name.c_str(),
    ...
    Selection & Interrupt(std::string name = "") {
      MAIN_THREAD_EM_ASM({ js.objects[$0].interrupt(UTF8ToString($1)); }, this->id, name.c_str());
      return *this;
    }
    ...
    Selection & Raise() {
      MAIN_THREAD_EM_ASM({ js.objects[$0].raise(); }, this->id);
      return *this;
    }

    Selection & Lower() {
      MAIN_THREAD_EM_ASM({ js.objects[$0].lower(); }, this->id);
      return *this;
    }
    ...
    /// Version for C++ functions
    template <typename T>
    emp::sfinae_decoy<Selection &, decltype(&T::operator())> On(std::string type,
                                                                T listener,
                                                                bool capture = false) {
      uint32_t fun_id = emp::JSWrap(listener, "", false);
      int new_id      = NextD3ID();
    ...
    */
    ...
    template <typename T>
    emp::sfinae_decoy<Selection &, decltype(&T::operator())> Sort(T comparator) {
    ...
    void SetupToolTip(ToolTip & tip) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].call(js.objects[$1]); }, this->id, tip.GetID());
    }
  ...
  Selection Select(std::string selector) { return Selection(selector); }

  /// Create a selection containing all DOM elements matching [selector]
  /// (convenience function to match D3 syntax - you can also just use the constructor)
  Selection SelectAll(std::string selector) { return Selection(selector, true); }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_SELECTION_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: javascript sel api bost ocks svg
