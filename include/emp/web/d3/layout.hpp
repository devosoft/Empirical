 ...
 *
 * @file include/emp/web/d3/layout.hpp
 * @brief Tools for laying out nodes in D3.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_LAYOUT_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_LAYOUT_HPP_GUARD
...
namespace D3 {
  ...
  struct JSONTreeNode {
    EMP_BUILD_INTROSPECTIVE_TUPLE(double, x, int, name, int, parent, double, y, int, depth)
  };
  ...
  public:
    ...
    void SetDataset(JSONDataset * dataset) { this->data = dataset; }
    ...
    emp::array<Selection, 4> GenerateNodesAndLinks(Selection svg) {
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          // Declare the nodes
          var node = js.objects[$3].selectAll("g.node").data(nodes, function(d) { return d.name; });
          ...
          var link =
            js.objects[$3].selectAll("path.link").data(links, function(d) { return d.name; });
      ...
      return emp::array<Selection, 4>{
        {Selection(node_enter), Selection(node_exit), Selection(link_enter), Selection(link_exit)}};
    }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_LAYOUT_HPP_GUARD
