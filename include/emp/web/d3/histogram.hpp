 ...
 *
 * @file include/emp/web/d3/histogram.hpp
 * @brief Tools to construct histograms on the web.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_HISTOGRAM_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_HISTOGRAM_HPP_GUARD
...
namespace D3 {
  ...
  struct HistogramBin {
    EMP_BUILD_INTROSPECTIVE_TUPLE(int, x0, int, x1, int, length)
  };
  ...
  public:
    ...
    ~Histogram() { emp::JSDelete(value_fun_id); }
    ...
    Histogram & Domain(double x, double y) { return SetDomain(x, y); }

    emp::array<double, 2> GetDomain() {
      MAIN_THREAD_EM_ASM({ emp_i.__outgoing_array = js.objects[$0].domain(); }, this->id);
      emp::array<double, 2> domain;
      emp::pass_array_to_cpp(domain);
      return domain;
    }

    emp::array<double, 2> Domain() { return GetDomain(); }
    ...
    /// @cond TEMPLATES
    template <typename T>
    emp::sfinae_decoy<Histogram, decltype(&T::operator())> SetValueAccessor(T func) {
      value_fun_id = JSWrap(func, emp::to_string(id) + "_return_value");
      MAIN_THREAD_EM_ASM({ js.objects[$0].value(window["emp"][$0 + "_return_value"]); }, this->id);
      return (*this);
    }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_HISTOGRAM_HPP_GUARD
