...
namespace D3 {
    ...
    public:
        Histogram(){
            MAIN_THREAD_EM_ASM({emp_d3.objects[$0] = d3.histogram();}, this->id);
        };
        ...
        Histogram& SetDomain(double x, double y) {
            MAIN_THREAD_EM_ASM({emp_d3.objects[$0].domain([$1, $2]);}, this->id, x, y);
            return (*this);
        }
        ...
        emp::array<double, 2> GetDomain() {
            MAIN_THREAD_EM_ASM({emp_i.__outgoing_array = emp_d3.objects[$0].domain();}
                        , this->id);
        ...
        Histogram& SetThresholds(int count) {
            MAIN_THREAD_EM_ASM({emp_d3.objects[$0].thresholds($1);}, this->id, count);
            return (*this);
        }

        Histogram& SetThresholds(std::string threshold_generator) {
            MAIN_THREAD_EM_ASM({
                emp_d3.objects[$0].thresholds(UTF8ToString($1));
        ...
        Dataset Call(emp::vector<DATA_TYPE> data) {
            ...
            MAIN_THREAD_EM_ASM({
                ...
                emp_d3.objects[$1] = emp_d3.objects[$0](emp_i.__incoming_array);
                console.log(emp_d3.objects[$1]);
            }, this->id, bins.GetID());
        ...
        SetValueAccessor(T func) {
          ...
          MAIN_THREAD_EM_ASM({
              emp_d3.objects[$0].value(window["emp"][$0+"_return_value"]);
          }, this->id);
          return (*this);
        }
        /// @endcond
