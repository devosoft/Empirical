 ...
 *
 * @file include/emp/web/d3/visualizations.hpp
 * @brief Tools to build D3 visualizations.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_VISUALIZATIONS_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_VISUALIZATIONS_HPP_GUARD
...
namespace emp {
  namespace web {
    ...
    protected:
      ...
      protected:
        ...
        virtual void GetHTML(std::stringstream & HTML) override {
          HTML.str("");
          HTML << "<svg id=\"" << id << "\" width=\"" << width << "\" height=\"" << height << "\">";
    ...
    public:
      D3Visualization(int w, int h, const std::string & in_id = "") : WidgetFacet(in_id) {
  ...
  /// @cond TEMPLATES
  template <typename T>
  emp::sfinae_decoy<void, decltype(&T::operator())> SetDrawCallback(T func) {
  ...
  void SetDrawCallback(std::string func) { draw_data_callback = func; }
...
public:
  ...
  void Setup() {
    ...
    y_scale->SetDomain(
      emp::array<double, 2>{{highest * value_growth_margin, lowest * value_loss_margin}});
...
public:
  ...
  virtual void Setup() { grid = D3::TileGrid<>(grid_width, grid_height, cell_size, *GetSVG()); }
};
...
public:
  HistogramChart(std::string x_var, int w = 800, int h = 400)
    : D3Visualization(w, h), x_ax("bottom", x_var), y_ax("left", "Frequency") {
    ;
  }
  ...
  void DrawData(emp::vector<DATA_TYPE> & data) {
    ...
    bins.Merge(enter)
      .MakeTransition(t)
      .SetAttr("x", [this, &x](D3::HistogramBin d) { return x.ApplyScale(d.x0()); })
      .SetAttr("y", [this, &y](D3::HistogramBin d) { return y.ApplyScale(d.length()); })
      .SetAttr("width",
               [this, &x](D3::HistogramBin d) {
...
public:
  ...
  void SetTooltipFunction(std::string func) { tip->SetHtml(func); }

  /// @cond TEMPLATES
  template <typename T>
  emp::sfinae_decoy<void, decltype(&T::operator())> SetTooltipFunction(T func) {
    tip->SetHtml(func);
  }
  ...
  /// @cond TEMPLATES
  template <typename T>
  emp::sfinae_decoy<void, decltype(&T::operator())> SetXAccessor(T func) {
    return_x = func;
  ...
  /// @cond TEMPLATES
  template <typename T>
  emp::sfinae_decoy<void, decltype(&T::operator())> SetYAccessor(T func) {
    return_y = func;
  ...
  void DrawPointsFromDataset() {
    ...
    D3::Selection update = GetSVG()->SelectAll(".data-point").Data(*dataset, GetID() + "return_x");
  ...
  void LoadDataFromFile(std::string filename) {
    ...
    if (this->init) {
      ...
      this->pending_funcs.Add(
        [this, filename]() { dataset->LoadDataFromFile(filename, "draw", false); });
    }
  }
  ...
  void AddDataPoint(DATA_TYPE data_point) {
    ...
    if (data_point[1] > y_max || data_point[1] < y_min || data_point[0] > x_max ||
        data_point[0] < x_min) {
    ...
    MAIN_THREAD_EM_ASM(
      {
        ...
        t =
          s.transition(js.objects[$2]).duration(300).attrTween("d", pathTween).ease(d3.easeLinear);
        ...
        js.objects[$3]
          ...
          .ease(d3.easeLinear)
          .attr("d",
                js.objects[$1](circle_data.slice(circle_data.length - 2, circle_data.length - 1)));
      },
      GetSVG()->GetID(),
      line_gen->GetID(),
      s.GetID(),
  ...
  void DrawData(bool backlog = false) {
    ...
    if ((!backlog && data.size() > 1) || data.size() == 0) { return; }
...
struct TreeNode {
  EMP_BUILD_INTROSPECTIVE_TUPLE(double, x, double, y, TreeDataNode, data, int, parent, int, depth)
};
...
#ifndef DOXYGEN_SHOULD_SKIP_THIS
  std::function<std::string(NODE, int)> color_fun_node = [](NODE d, int i) { return "black"; };

  std::function<std::string(NODE, int)> color_fun_link = [](NODE d, int i) { return "black"; };
#endif  // #ifndef DOXYGEN_SHOULD_SKIP_THIS
...
public:
  ...
  TreeVisualization(int width = 800, int height = 400) : D3Visualization(width, height) {
    variables.push_back("Persist");
  }
  ...
  void LoadDataFromFile(std::string filename) {
    if (this->init) {
      ...
      this->pending_funcs.Add(
        [this, filename]() { data->LoadDataFromFile(filename, [this]() { DrawTree(); }); });
    }
  }

  void AddDataPoint(int parent, int child) {
    std::string child_json = std::string(
      "{\"name\":" + to_string(child) + ", \"parent\":" + to_string(parent) + ", \"children\":[]}");
...
public:
  ...
  TreeVisualizationReplacement(int width, int height) : TreeVisualization<NODE>(width, height) {
    variables.push_back("Persist");
  }
  ...
  void AddDataPoint(int parent, int child) {
    ...
    MAIN_THREAD_EM_ASM(
      {
        while (js.objects[$0].length < $1 + 1) { js.objects[$0].push(-1); }
...
struct SpatialGridTreeNode {
  EMP_BUILD_INTROSPECTIVE_TUPLE(double, x, double, y, int, name, int, parent, int, depth, int, loc)
};

template <typename NODE                       = SpatialGridTreeNode,
          template <typename> class TREE_TYPE = TreeVisualization>
...
public:
  ...
  struct TreeEdge {
    EMP_BUILD_INTROSPECTIVE_TUPLE(NODE, source, NODE, target)
  };
  ...
  std::function<std::string(NODE, int)> color_fun_node = [this](NODE d, int i) {
    if (d.loc() < 0) { return std::string("black"); }

    double x = (d.loc() % grid_width) - grid_width / 2;
    double y = (d.loc() / grid_width) - grid_height / 2;

    double r = sqrt(std::pow((int) x, 2) + std::pow((int) y, 2)) /
               sqrt(std::pow(grid_width, 2) + std::pow(grid_height, 2));
    (void) r;
  ...
  std::function<void(LegendNode, int)> legend_mouseover = [this](LegendNode d, int il) {
    legend.SelectAll("rect")
      ...
      .SetClassed("faded", true);
    GetSVG()
      ->SelectAll(".node")
      .Filter([d](LegendNode in_data) { return d.loc() != in_data.loc(); })
      .SetClassed("faded", true);
    MAIN_THREAD_EM_ASM({emp.filter_fun = function(d){return d.source.loc != $0;
  }}, d.loc());
...
virtual void Setup() {
  ...
  tip->SetHtml([this](NODE d) {
    return "ID: " + to_string(d.name()) + ", Pos: (" + to_string(d.loc() % grid_width) + ", " +
           to_string(d.loc() / grid_width) + ")";
  });

  legend = D3::Select("body").Append("svg");

  legend.SetAttr("x", 1000).SetAttr(
    "y",
    0);  //.SetAttr("width", legend_cell_size*grid_width).SetAttr("height", legend_cell_size*grid_height);
  legend.SetStyle("position", "fixed")
    .SetStyle("right", "10px")
    .SetStyle("top", "10px");  //.SetStyle("width", "10%");//.SetStyle("height", "10%");
  ...
  GetSVG()->SelectAll(".node").On("click", [this](NODE d) {
    std::cout << emp::to_string(GetLocHistory(d.name())) << std::endl;
  });
}
}
;
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_VISUALIZATIONS_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: svg funcs hcl vis segs genpath loc
