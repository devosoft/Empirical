...
#ifndef __EMP_D3_UTILS_H__
#define __EMP_D3_UTILS_H__
...
#define D3_CALLBACK_METHOD_2_ARGS_IMPL(MACRO, FUNC, ARG1, ARG2)     \
  MAIN_THREAD_EM_ASM({                                                     \
      ...
      MACRO;                                                        \
      emp.__new_object = emp_d3.objects[$0].FUNC(arg1, func_string);    \
  }, this->id, ARG1, ARG2);
...
MAIN_THREAD_EM_ASM({                                               \
    var func_string = UTF8ToString($1);                \
    MACRO;                                                  \
    emp.__new_object = emp_d3.objects[$0].FUNC(func_string);    \
}, this->id, ARG1);
...
#define D3_CALLBACK_METHOD_CPP_FUNCTION_2_ARGS(FUNC, ARG1, CPP_FUN)        \
  ...
  MAIN_THREAD_EM_ASM({                                                            \
    emp.__new_object = emp_d3.objects[$0].FUNC(UTF8ToString($1),          \
                                    function(d, i, j) {                    \
  ...
  #define D3_CALLBACK_METHOD_CPP_FUNCTION_1_ARG(FUNC, CPP_FUN)               \
    uint32_t fun_id = emp::JSWrap(CPP_FUN, "", false);                       \
    MAIN_THREAD_EM_ASM({                                                            \
      emp.__new_object = emp_d3.objects[$0].FUNC(function(d, i, j) {             \
                                            return emp.Callback($1, d, i, j);\
                                            });                              \
    }, this->id, fun_id);                                                    \
    emp::JSDelete(fun_id);

//Store return of one of the above functions in emp_d3.objects
void StoreNewObject(int id){
    MAIN_THREAD_EM_ASM({
        emp_d3.objects[$0] = emp.__new_object;
