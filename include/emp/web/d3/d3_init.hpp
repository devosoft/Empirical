 ...
 *
 * @file include/emp/web/d3/d3_init.hpp
 * @brief TODO.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_D3_INIT_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_D3_INIT_HPP_GUARD
...
namespace D3 {
  ...
  protected:
    ...
    D3_Base() {
      ...
      EM_ASM_ARGS({ js.counts[$0] = 1; }, this->id);
    }
  ...
  public:
    ...
    ToolTip() {
      EM_ASM_ARGS(
        {
          js.objects[$0] =
            d3.tip().attr('class', 'd3-tip').offset([-10, 0]).html(function(d, i) { return d; });
        },
        this->id);
    }
    ...
    ToolTip(T func) {
      ...
      EM_ASM_ARGS(
        {
          js.objects[$0] =
            d3.tip().attr('class', 'd3-tip').offset([-10, 0]).html(emp[$0 + "_html_func"]);
        },
        id);
    }
    ...
    /// @cond TEMPLATES
    template <typename T>
    emp::sfinae_decoy<void, decltype(&T::operator())> SetHtml(T func) {
  ...
  public:
    FormatFunction(std::string format = "") {
      EM_ASM_ARGS({ js.objects[$1] = d3.format(UTF8ToString($0)); }, format.c_str(), this->id);
    }
  ...
  public:
    ...
    JSFunction(std::string name) {
      ...
      if (fail) { emp::NotifyWarning("Invalid function name passed to JSFunction"); }
    }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_D3_INIT_HPP_GUARD
