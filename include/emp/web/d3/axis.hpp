 ...
 *
 * @file include/emp/web/d3/axis.hpp
 * @brief Handle drawing of axes on D3 graphs.
 */

#pragma once

#ifndef INCLUDE_EMP_WEB_D3_AXIS_HPP_GUARD
#define INCLUDE_EMP_WEB_D3_AXIS_HPP_GUARD
  ...
  public:
    ...
    Axis(std::string type, std::string label = "") {
      ...
      if (type == "left") {
        ...
        MAIN_THREAD_EM_ASM({ js.objects[$0] = d3.axisTop(js.objects[$1]); }, this->id, scale.GetID());
      } else {
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          js.objects[$3].append("g").attr("id", UTF8ToString($2)).call(js.objects[$0]);
          ...
          if (UTF8ToString($5) != "") { dy = UTF8ToString($5); }
    ...
    Axis & ApplyAxis(const SelectionOrTransition<T> & selection) {
      MAIN_THREAD_EM_ASM({ js.objects[$1].call(js.objects[$0]); }, this->id, selection.GetID());
      return *this;
    }
    ...
    Axis & SetScale(SCALE_TYPE & scale) {
      ...
      MAIN_THREAD_EM_ASM({ js.objects[$0].scale(js.objects[$1]); }, this->id, scale.GetID());
      return *this;
    }

    /// Get a reference to this object's scale.
    SCALE_TYPE & GetScale() { return this->scale; }
    ...
    Axis & SetTickValues(emp::array<T, SIZE> values) {
      ...
      MAIN_THREAD_EM_ASM({ js.objects[$0].tickValues(emp_i.__incoming_array); }, this->id);
      return *this;
    }

    Axis & SetTickSize(float size) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].tickSize($1); }, this->id, size);
      return *this;
    }

    Axis & SetTickSizeInner(float size) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].tickSizeInner($1); }, this->id, size);
      return *this;
    }

    Axis & SetTickSizeOuter(float size) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].tickSizeOuter($1); }, this->id, size);
      return *this;
    }

    Axis & SetTickPadding(int padding) {
      MAIN_THREAD_EM_ASM({ js.objects[$0].tickPadding($1); }, this->id, padding);
      return *this;
    }
...
#endif  // #ifndef INCLUDE_EMP_WEB_D3_AXIS_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: svg percapitamortality api javascript
