...
namespace web {
  ...
  namespace internal {
    ...
    template <typename IN_TYPE, typename = std::enable_if_t<std::is_invocable<IN_TYPE>::value> >
...
namespace emp {
  ...
  static bool Initialize() {
    ...
    if (!init) {
      emp_assert_warning(
        false &&
        "Warning: you're using Empirical web features but not compiling with emcc. These features will not do anything unless you use emcc.");
    }
    init = true;
    return true;
  }
