 ...
 *
 * @file include/emp/web/_MochaTestRunner.hpp
...
#ifndef INCLUDE_EMP_WEB_MOCHA_TEST_RUNNER_HPP_impl_GUARD
#define INCLUDE_EMP_WEB_MOCHA_TEST_RUNNER_HPP_impl_GUARD
...
namespace emp { namespace web {
  ...
  private:
    std::unordered_map<std::string, emp::web::Document> documents;
  ...
  public:
    ...
    void Redraw() {
      for (auto & [id, doc] : documents) { doc.Redraw(); }
    }
    ...
    emp::web::Document & Doc(const std::string & id) {
      emp_assert(documents.count(id), "Bad request for unregistered document.");
  ...
  protected:
    ...
    emp::Signal<void()>
      after_each_test_sig;  ///< Is triggered after each test (after test marked 'done', but before test is deleted).
    std::deque<TestRunner>
      test_runners;  ///< Store test runners in a first-in-first-out (out=run) queue
    ...
    void NextTest() {
      ...
      test_runners.front()
        .describe();  // This will queue up describe clause for this test and either (1) queue up the next test or (2) queue up manager cleanup
    }
    ...
    void CleanupTestRunners() {
      ...
      std::for_each(test_runners.begin(), test_runners.end(), [](TestRunner & runner) {
    ...
    std::function<void()> MakeTestCreate() {
      return [this]() {
        auto & cur_runner =
          this->test_runners.front();  // Create the test in the front of the queue.
    ...
    std::function<void()> MakeTestDescribe() {
      return [this]() {
        emp_assert(test_runners.size());
        auto & cur_runner =
          this->test_runners.front();  // When a test_runner's describe function is run, it will be at
        ...
        MAIN_THREAD_EM_ASM(
          {
            ...
            // Queue cleanup for this test
            describe(`Cleanup $ { test_name }`,
                     function() {
                       it('should clean up the test ', function() { emp.PopTest(); });
                     });
          },
          test_name.c_str());
        ...
        if (test_runners.size() > 1) {
          ...
          MAIN_THREAD_EM_ASM(
            {
              ...
              // Queue up next test
              describe(`Queue $ { next_test_name }` ,
                       function() {
                         it("should queue the next test", function() { emp.NextTest(); });
                       });
            },
          ...
          MAIN_THREAD_EM_ASM({
            describe(
              ...
              function() {
                it("should cleanup test manager", function() { emp.CleanupTestRunners(); });
              });
          });
        }
      };
          ...
          MAIN_THREAD_EM_ASM(
            {
              ...
              describe(`${test_name} failed`, function() {
                it("failed at least one C++ unit test", function() { chai.assert(false); });
              });
            },
            test_name.c_str());
        }
  ...
  public:
    MochaTestRunner()
      ...
      , cleanup_all_js_func_id(
          emp::JSWrap([this]() { this->CleanupTestRunners(); }, "CleanupTestRunners")) {}
...
/// Tests are run in the order they were added.
    ...
    void OnBeforeEachTest(const std::function<void()> & fun) {
      before_each_test_sig.AddAction(fun);
    }
...
#endif  // #ifndef INCLUDE_EMP_WEB_MOCHA_TEST_RUNNER_HPP_impl_GUARD
