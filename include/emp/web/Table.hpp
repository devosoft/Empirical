...
namespace emp::web {
  ...
  namespace internal {
    ...
    struct TableColInfo {
      WidgetExtras extras;
    };  // Currently only need annotations.
    ...
    class TableInfo : public internal::WidgetInfo {
      ...
      friend TableRow;
      friend TableCol;
      friend TableRowGroup;
      friend TableColGroup;
    ...
    protected:
      ...
      void Resize(size_t new_rows, size_t new_cols) {
        ...
        // Clear removed rows.
        for (size_t row_id = new_rows; row_id < row_count; ++row_id) { ClearRowChildren(row_id); }
      ...
      void DoActivate(bool top_level = true) override {
        ...
        for (auto & row : rows) {
          for (auto & col : row.data) {
            for (auto & child : col.children) { child->DoActivate(false); }
          }
        }
      ...
      web::Text & GetTextWidget(size_t r, size_t c) {
        ...
        if (cell_children.size() == 0 || cell_children.back().IsText() == false ||
      ...
      virtual void GetHTML(std::stringstream & HTML) override {
        ...
        for (size_t r = 0; r < rows.size(); r++) {
          ...
          for (size_t c = 0; c < row.data.size(); c++) {
            ...
            if (datum.masked) {
              continue;  // If this cell is masked by another, skip it!
            }
            ...
            for (Widget & w : datum.children) { HTML << "<span id=\'" << w.GetID() << "'></span>"; }
      ...
      void ClearColGroup(size_t col_id) {
        ...
        for (size_t offset = 0; offset < col_groups[col_id].span; offset++) {
          ClearCol(col_id + offset);
        }
      }

      void ClearTableCells() {
        for (size_t r = 0; r < row_count; r++) { ClearRowCells(r); }
      }

      void ClearTableRows() {
        for (size_t r = 0; r < row_count; r++) { ClearRow(r); }
      }
      ...
      bool OK(std::stringstream & ss, bool verbose = false, const std::string & prefix = "") {
        ...
        // Basic info
        if (verbose) {
          ss << prefix << "Scanning: emp::TableInfo (rows=" << row_count << ", cols=" << col_count
             << ")." << std::endl;
        }

        // Make sure rows and columns are being sized correctly.
        if (row_count != rows.size()) {
          ss << prefix << "Error: row_count = " << row_count << ", but rows has " << rows.size()
             << " elements." << std::endl;
          ok = false;
        }

        if (cols.size() && col_count != cols.size()) {
          ss << prefix << "Error: col_count = " << col_count << ", but cols has " << cols.size()
             << " elements." << std::endl;
          ok = false;
        }
        ...
        if (col_groups.size() && col_count != col_groups.size()) {
          ss << prefix << "Error: col_count = " << col_count << ", but col_groups has "
             << col_groups.size() << " elements." << std::endl;
          ok = false;
        }

        if (row_groups.size() && row_count != row_groups.size()) {
          ss << prefix << "Error: row_count = " << row_count << ", but row_groups has "
             << row_groups.size() << " elements." << std::endl;
          ok = false;
        }
        ...
        for (size_t r = 0; r < row_count; r++) {
          ...
          if (col_count != rows[r].data.size()) {
            ss << prefix << "  Error: col_count = " << col_count << ", but row has "
               << rows[r].data.size() << " elements." << std::endl;
            ok = false;
          }
          for (size_t c = 0; c < col_count; c++) {
  ...
  public:
    ...
    TableColGroup GetColGroup(
      size_t c) const;       ///< Focus on a specific group of columns in the table.
  ...
  public:
    ...
    Table & ClearStyle() {
      Info()->extras.style.Clear();
      return *this;
    }

    Table & ClearAttr() {
      Info()->extras.attr.Clear();
      return *this;
    }

    Table & ClearListen() {
      Info()->extras.listen.Clear();
      return *this;
    }
    ...
    Table & SetColSpan(size_t new_span) {
      ...
      if (old_span != new_span) {
        for (size_t i = old_span; i < new_span; i++) {
          Info()->col_groups[cur_col + i].masked = true;
        }
        for (size_t i = new_span; i < old_span; i++) {
          Info()->col_groups[cur_col + i].masked = false;
        }
      }
    ...
    Table & RowCSS(size_t row_id, const std::string & setting, SETTING_TYPE && value) {
      ...
      if (IsActive()) {
        Info()->ReplaceHTML();  // @CAO only should replace row's CSS
      }
      return *this;
    }
    ...
    template <typename SETTING_TYPE>
    Table & CellCSS(size_t row_id,
                    [[maybe_unused]] size_t col_id,
...
namespace emp::web {
  ...
  TableRow TableWidget::operator[](size_t r) const { return GetRow(r); }

  TableRow TableWidget::GetLastRow() const { return TableRow(Info(), Info()->row_count - 1); }

  TableCol TableWidget::GetLastCol() const { return TableCol(Info(), Info()->col_count - 1); }
  ...
  Table TableWidget::GetTable() const { return Table(Info(), cur_row, cur_col); }
