...
namespace emp::web {
  ...
  namespace internal {
    ...
    protected:
      ...
      virtual ~DivInfo() {
        ...
        for (auto & p : anim_map) {
          delete p.second;  // Delete this document's animations.
        }
      }
      ...
      void Register_recurse(Widget & new_widget) override {
        ...
        if (parent) {
          parent->Register_recurse(new_widget);  // Also register in parent, if available
        }
      }
      ...
      void Unregister_recurse(Widget & old_widget) override {
        ...
        if (parent) {
          parent->Unregister_recurse(old_widget);  // Unregister in parent, if available
        }
      }
      ...
      void RemoveChild(Widget & child) override {
        ...
        emp_assert(1 == std::count(std::begin(m_children), std::end(m_children), child));
        // unregister and remove child
        Unregister(*std::find(std::begin(m_children), std::end(m_children), child));
        m_children.erase(std::remove(std::begin(m_children), std::end(m_children), child),
                         std::end(m_children));
        // render changes
        if (state == Widget::ACTIVE) { ReplaceHTML(); }
      }
      ...
      web::Text & GetTextWidget() {
        ...
        if (m_children.size() == 0 || m_children.back().IsText() == false ||
            m_children.back().AppendOK() == false || text_append == false) {
          AddChild(Text());
          text_append = true;
        }
  ...
  public:
    ...
    bool HasChild(const Widget & test_child) const {
      ...
      for (const Widget & c : Info()->m_children) {
        if (c == test_child) { return true; }
      }
      return false;
    }
    ...
    web::Animate & AddAnimation(const std::string & name, T &&... args) {
      ...
      emp_assert(Info()->anim_map.find(name) ==
                 Info()->anim_map.end());  // Make sure not in map already!
