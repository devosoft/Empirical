...
namespace emp {
  ...
  template <int ARG_ID, typename T>
  static void LoadArg(T & arg_var) {
  ...
  template <int ARG_ID, size_t SIZE, typename T>
  static void LoadArg(emp::array<T, SIZE> & arg_var) {
    MAIN_THREAD_EM_ASM({ emp_i.__outgoing_array = emp_i.cb_args[$0]; }, ARG_ID);
    pass_array_to_cpp(arg_var);
  }

  template <int ARG_ID, typename T>
  static void LoadArg(emp::vector<T> & arg_var) {
    MAIN_THREAD_EM_ASM({ emp_i.__outgoing_array = emp_i.cb_args[$0]; }, ARG_ID);
  ...
  template <int ARG_ID>
  static void LoadArg(double & arg_var, std::string var) {
    arg_var = MAIN_THREAD_EM_ASM_DOUBLE({ return emp_i.curr_obj[UTF8ToString($0)]; }, var.c_str());
  }
    ...
    char * tmp_var = (char *) MAIN_THREAD_EM_ASM_INT(
      {
        ...
        if (emp_i.curr_obj[key] == null) { emp_i.curr_obj[key] = "undefined"; }
  ...
  template <int ARG_ID, typename JSON_TYPE>
  static typename std::enable_if<JSON_TYPE::n_fields != -1, void>::type LoadArg(JSON_TYPE & arg_var,
                                                                                std::string var) {
  ...
  struct LoadTuple {
    static void LoadJSDataArg(JSON_TYPE & arg_var) {
      ...
      LoadTuple<JSON_TYPE, ARG_ID, FIELD - 1> load_tuple =
        LoadTuple<JSON_TYPE, ARG_ID, FIELD - 1>();
      load_tuple.LoadJSDataArg(arg_var);
    }
  };
  ...
  static emp::sfinae_decoy<void, decltype(&RETURN_TYPE::StoreAsReturn)> StoreReturn(
  ...
  static void StoreReturn(const std::string & ret_var, std::string var) {
    MAIN_THREAD_EM_ASM(
      { emp_i.curr_obj[UTF8ToString($1)] = UTF8ToString($0); },
      ret_var.c_str(),
      var.c_str());
  }
  ...
  // Tuple struct
  template <typename RETURN_TYPE>
  static typename std::enable_if<RETURN_TYPE::n_fields != -1, void>::type StoreReturn(
    const RETURN_TYPE & ret_var) {
    MAIN_THREAD_EM_ASM({
      emp_i.cb_return    = {};
      emp_i.object_queue = [];
    ...
    StoreTuple<RETURN_TYPE, RETURN_TYPE::n_fields> store_tuple =
      StoreTuple<RETURN_TYPE, RETURN_TYPE::n_fields>();
  ...
  namespace internal {
    ...
    public:
      ...
      struct Collect_impl {
        static void CollectArgs(TUPLE_TYPE & tuple) {
          LoadArg_impl<typename std::tuple_element<ARGS_LEFT - 1, TUPLE_TYPE>::type,
                       ARGS_LEFT - 1>(true, std::get<ARGS_LEFT - 1>(tuple));  // Load an arg
    ...
    public:
      ...
      void DoCallback() {
        ...
        RET_TYPE return_val;
        std::apply([&return_val, this](ARG_TYPES... in_args) { return_val = fun(in_args...); },
                   args);
      ...
      MAIN_THREAD_EM_ASM(
        {
          ...
          emp[fun_name] = function() {
            emp_i.cb_args = [];
            for (var i = 0; i < arguments.length; i++) { emp_i.cb_args[i] = arguments[i]; }
  ...
  size_t JSWrap(const FUN_TYPE & in_fun,
                const std::string & fun_name = "",
                bool dispose_on_use          = false) {
  ...
  size_t JSWrapOnce(FUN_TYPE && in_fun) {
    return JSWrap(std::forward<FUN_TYPE>(in_fun), "", true);
  }
...
#ifdef __EMSCRIPTEN_PTHREADS__
  EM_ASM({
    emp_i.pending_offscreen_canvas_ids = emp_i.pending_offscreen_canvas_ids || new Set();
...
#else  // #ifndef __EMSCRIPTEN_PTHREADS__
...
#endif  // #ifndef EM_FUNC_SIG_VI

  //    emscripten_async_queue_on_thread(
  emscripten_dispatch_to_thread(proxy_pthread_id,
                                EM_FUNC_SIG_VI,  // VI = no return value, one argument
                                (void *) &empDoCppCallback,
                                NULL,
                                cb_id);

#endif  // #ifndef __EMSCRIPTEN_PTHREADS__ : #else
}
...
#endif  // #ifndef INCLUDE_EMP_WEB_JSWRAP_HPP_GUARD
