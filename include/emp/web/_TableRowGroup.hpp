 ...
 *
 * @file include/emp/web/_TableRowGroup.hpp
 * @brief The TableRowGroup widget, which behaves like the Table widget, but focuses on a group of rows.
 *
...
#ifndef INCLUDE_EMP_WEB_TABLE_ROW_GROUP_HPP_impl_GUARD
#define INCLUDE_EMP_WEB_TABLE_ROW_GROUP_HPP_impl_GUARD
...
namespace emp { namespace web {
  ...
  public:
    ...
    TableRowGroup & Clear() {
      Info()->ClearRowGroup(cur_row);
      return *this;
    }

    TableRowGroup & ClearStyle() {
      Info()->row_groups[cur_row].extras.style.Clear();
      return *this;
    }

    TableRowGroup & ClearAttr() {
      Info()->row_groups[cur_row].extras.attr.Clear();
      return *this;
    }

    TableRowGroup & ClearListen() {
      Info()->row_groups[cur_row].extras.listen.Clear();
      return *this;
    }

    TableRowGroup & ClearExtras() {
      Info()->row_groups[cur_row].extras.Clear();
      return *this;
    }

    TableRowGroup & ClearChildren() {
      Info()->ClearRowGroupChildren(cur_row);
      return *this;
    }
    ...
    TableRowGroup & SetRowSpan(size_t new_span) {
      ...
      if (old_span != new_span) {
        for (size_t i = old_span; i < new_span; i++) {
          Info()->row_groups[cur_row + i].masked = true;
        }
        for (size_t i = new_span; i < old_span; i++) {
          Info()->row_groups[cur_row + i].masked = false;
        }
      }
...
#endif  // #ifndef INCLUDE_EMP_WEB_TABLE_ROW_GROUP_HPP_impl_GUARD
