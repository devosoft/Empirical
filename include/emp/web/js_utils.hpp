...
namespace emp {
  ...
  static const std::map<std::string, std::string> & get_type_to_string_map() {
    // Using typeid().name() could create problems because it varies by implementation.
    // All that matters is consistency, but two obscure types could technically be given
    // the same name. So far it seems to not be an issue with Emscripten, which is most
    // critical for this code.
    static std::map<std::string, std::string> map_type_names;
    if (map_type_names.size() == 0) {
  ...
  pass_array_to_javascript(C values, emp::vector<int> recursive_el) {
    ...
    emp_assert((map_type_names.contains(typeid(T).name())));
    ...
    MAIN_THREAD_EM_ASM({
      ...
      for (i = 0; i < $1; i++) { curr_array.push(getValue($0 + (i * $2), UTF8ToString($3))); }
  ...
  pass_array_to_javascript(C values, emp::vector<int> recursive_el) {
    ...
    for (auto val : values) {
      (void) val;
      MAIN_THREAD_EM_ASM({ emp_i.__curr_array.push(UTF8ToString($0)); }, val.c_str());
    };
  ...
  pass_array_to_javascript(C values, emp::vector<int> recursive_el) {
    ...
    for (std::size_t j = 0; j < values.size(); j++) {                 // Iterate over array
      for (std::size_t i = 0; i < values[j].var_names.size(); i++) {  // Iterate over object members
        ...
        emp_assert((map_type_names.find(values[j].var_types[i].name()) != map_type_names.end()),
                   values[j].var_types[i].name());
        ...
        }, values[j].pointers[i], type_string.c_str(), var_name.c_str(), j, recursive_el.data());
        // clang-format on
...
/// @endcond
  ...
  void pass_array_to_cpp(emp::array<char, SIZE> & arr, bool recurse = false) {
    ...
    for (size_t i = 0; i < arr.size(); i++) { arr[i] = *(buffer + i * 2); }
  ...
  void pass_vector_to_cpp(emp::vector<emp::vector < T> > &arr, bool recurse = false) {
    ...
    for (int i = 0; i < size; i++) {
      MAIN_THREAD_EM_ASM(
        { emp_i.__outgoing_array = emp_i.__temp_array[emp_i.__temp_array.length - 1][$0]; },
        i);
      while ((int) arr.size() <= i) { arr.push_back(emp::vector<T>()); }
  ...
  void pass_map_to_javascript(const std::map<KEY_T, VAL_T> & dict) {
    ...
    for (typename std::map<KEY_T, VAL_T>::const_iterator it = dict.begin(); it != dict.end();
         ++it) {
    ...
    MAIN_THREAD_EM_ASM({ emp_i.__incoming_map_keys = emp_i.__incoming_array; });
    ...
    MAIN_THREAD_EM_ASM({
      ...
      emp_i.__incoming_map_keys.forEach(
        function(key, val){emp_i.__incoming_map[key] = emp_i.__incoming_map_values[val]});
  ...
  void pass_map_to_javascript(const emp::array<KEY_T, SIZE> & keys,
                              const emp::array<VAL_T, SIZE> & values) {
  ...
  int GetViewPortSize() {
    return MAIN_THREAD_EM_ASM_INT({
      return Math.min(Math.max(document.documentElement.clientWidth, window.innerWidth || 0),
...
#endif  // #ifndef INCLUDE_EMP_WEB_JS_UTILS_HPP_GUARD
