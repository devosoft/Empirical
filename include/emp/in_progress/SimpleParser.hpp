...
namespace emp {
  class SimpleParser {
    ...
    struct ValueType {
      ...
      ValueType & operator=(value_fun_t in_fun) {
        type = FUNCTION;
        fun  = in_fun;
        return *this;
      }
      ...
      value_fun_t AsFun() {
        if (type == FUNCTION) {
          ...
          return [v = value](emp::DataMap &) { return v; };
        }
      }
    };
    ...
    error_fun_t error_fun = [](const std::string & msg) {
  ...
  public:
    ...
    void AddOp(const std::string & op, std::function<double(double)> fun) { unary_ops[op] = fun; }
    ...
    void AddDefaultOperators() {
      ...
      binary_ops["||"].Set( ++prec, [](double x, double y){
        return (x != 0.0) || (y != 0.0); } );
      binary_ops["&&"].Set( ++prec, [](double x, double y){
        return (x != 0.0) && (y != 0.0); } );
      binary_ops["=="].Set( ++prec, [](double x, double y){
    ...
    void AddDefaultFunctions() {
      ...
      functions["CLAMP"].Set3([](double x, double y, double z) {
    ...
    ValueType ParseValue(const DataMap & dm, pos_t & pos) {
      ...
      if (emp::Has(unary_ops, pos->lexeme)) {
        ...
        if (val.type == ValueType::VALUE) {
          ...
          return (value_fun_t)[fun = val.fun, op](emp::DataMap & dm) { return op(fun(dm)); }
        }
      }
      ...
      if (is_fun) {
        if (!emp::Has(functions, name)) {
          return AddError("Call to unknown function '", name, "'.");
        }
        ++pos;
        ...
        switch (args.size()) {
          ...
          case 1:
            if (!functions[name].fun1) {
              AddError("Function '", name, "' cannot have 1 arguments.");
            }
          ...
          case 2:
            if (!functions[name].fun2) {
              AddError("Function '", name, "' cannot have 2 arguments.");
            }
            out_fun = [fun = functions[name].fun2, arg0 = args[0].AsFun(), arg1 = args[1].AsFun()](
                        emp::DataMap & dm) { return fun(arg0(dm), arg1(dm)); };
            break;
          case 3:
            if (!functions[name].fun3) {
              AddError("Function '", name, "' cannot have 3 arguments.");
            }
            out_fun = [fun  = functions[name].fun3,
                       arg0 = args[0].AsFun(),
                       arg1 = args[1].AsFun(),
                       arg2 = args[2].AsFun()](emp::DataMap & dm) {
              return fun(arg0(dm), arg1(dm), arg2(dm));
            };
            break;
          default: AddError("Too many arguments for function '", name, "'.");
        }
        return out_fun;
      }
        ...
        if (Has(binary_ops, pos->lexeme)) {
          ...
          if (val1.type == ValueType::VALUE) {
            if (val2.type == ValueType::VALUE) {
              ...
              val1 = (value_fun_t)[val1_num = val1.value, val2_fun = val2.fun, op_fun = op.fun](
                emp::DataMap & dm) {
            ...
            if (val2.type == ValueType::VALUE) {
              val1 = (value_fun_t)[val1_fun = val1.fun, val2_num = val2.value, op_fun = op.fun](
                emp::DataMap & dm) {
                return op_fun(val1_fun(dm), val2_num);
              }
            } else {
              val1 = (value_fun_t)[val1_fun = val1.fun, val2_fun = val2.fun, op_fun = op.fun](
                emp::DataMap & dm) {
          ...
          AddError("Operator '", pos->lexeme, "' NOT found!");
        }
      }
...
#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_SIMPLE_PARSER_HPP_GUARD
