  ...
  public:
    ...
    int AddPattern(const std::string & name,
      ...
      if (patterns.find(name) != patterns.end()) {
        emp::LibraryWarning("Attempting to add multiple lexer patterns for '",
                            name,
                            "'.  Ignoring.");
        return -1;
      }
    ...
    emp::Token GetToken() {
      while (next_char > 0) {       // Keep looping until we find a token or hit EOF.
        if (is_digit(next_char)) {  // Must be a number
          ...
          while (is_digit(next_char = is.get())) { cur_lexeme.push_back(next_char); }
        ...
        if (is_idchar(next_char)) {  // Must be ID or Keyword (number already captured)
          cur_lexeme.resize(1);
          cur_lexeme[0] = next_char;
          while (is_idchar(next_char = is.get())) { cur_lexeme.push_back(next_char); }

          // Determine if the current lexeme is a command.
          auto map_ptr = command_map.find(cur_lexeme);
          if (map_ptr != command_map.end()) { return map_ptr->second; }
        ...
        switch (prev_char) {
          ...
          case ' ':
          case '\t':
          case '\r': break;  // Skip WS; don't return token.
          case '\n':
          case ';':  return Token(Token::ENDLINE);
          case '+':
            if (next_char == '=') {
              next_char = is.get();
...
#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_CONFIG_LEXER_HPP_GUARD
