 ...
 *
 * @file include/emp/in_progress/Ptr-overload-fix.hpp
...
#ifndef INCLUDE_EMP_IN_PROGRESS_PTR_OVERLOAD_FIX_HPP_GUARD
#define INCLUDE_EMP_IN_PROGRESS_PTR_OVERLOAD_FIX_HPP_GUARD
...
namespace emp {
  ...
  template <typename T>
  inline void FillMemoryFunction(emp::Ptr<unsigned char> mem_ptr,
  ...
  public:
    ...
    bool OK() const noexcept {
      ...
      if (status == PtrStatus::ARRAY) {
        if (array_bytes == 0) {
          return false;  // Arrays cannot be size 0.
        }
        if (count == 0) {
          return false;  // Active arrays must have pointers to them.
        }
      }
      if (status == PtrStatus::ACTIVE) {
        if (array_bytes > 0) {
          return false;  // non-arrays must be array size 0.
        }
        if (count == 0) {
          return false;  // Active pointers must have references to them.
        }
      }
      return true;
    }
  ...
  public:
    ~PtrTracker() {
      ...
      if (undeleted_info.size()) {
        ...
        if (undeleted_info.size() > 10) { std::cerr << "  ..." << std::endl; }
        abort();
      }
    ...
    bool IsDeleted(size_t id) const {
      if (id == UNTRACKED_ID) {
        return false;  // Not tracked, so not deleted.
      }
  ...
  public:
    ...
    Ptr() : BasePtr<TYPE>(nullptr, UNTRACKED_ID) {
      if (internal::ptr_debug) { std::cout << "null construct." << std::endl; }
    }
    ...
    Ptr(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, T2 *> in_ptr, bool track = false)
      : BasePtr<TYPE>(in_ptr, UNTRACKED_ID) {
      if (internal::ptr_debug) {
    ...
    template <typename T2>
    Ptr(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, T2 *> _ptr,
        size_t array_size,
        bool track)
      : BasePtr<TYPE>(_ptr, UNTRACKED_ID) {
    ...
    template <typename T2>
    Ptr(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, Ptr<T2>> _in)
      : BasePtr<TYPE>(_in.Raw(), _in.GetID()) {
    ...
    template <typename T2>
    Ptr<TYPE> & operator=(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, T2 *> _in) & {
    ...
    template <typename T2>
    Ptr<TYPE> & operator=(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, Ptr<T2>> _in) & {
    ...
    bool operator==(const T & in_ptr) const {
      if constexpr (std::is_same<T, Ptr<TYPE>>()) {
  ...
  protected:
    void * ptr;  ///< The raw pointer associated with this Ptr object.
  public:
    BasePtr(void * in_ptr) : ptr(in_ptr) {}
  };
  ...
  protected:
    const void * ptr;  ///< The raw pointer associated with this Ptr object.
  public:
    BasePtr(const void * in_ptr) : ptr(in_ptr) {}
  };
  ...
  public:
    ...
    template <typename T2>
    Ptr(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, T2 *> in_ptr, bool = false)
      : BasePtr<TYPE>(in_ptr) {}

    // Ptr(TYPE * in_ptr, bool=false) : BasePtr<TYPE>(in_ptr) {}

    /// Construct from array
    template <typename T2>
    Ptr(std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, T2 *> _ptr, size_t, bool)
      : BasePtr<TYPE>(_ptr) {}

    /// From compatible Ptr
    template <typename T2>
    Ptr(Ptr<T2> _in, std::enable_if_t<std::is_convertible_v<T2 *, TYPE *>, bool> = true)
      : BasePtr<TYPE>(_in.Raw()) {}
    ...
    template <typename T2>
    Ptr<T2> ConstCast() const {
      return const_cast<T2 *>(ptr);
    }

    template <typename T2>
    Ptr<T2> DynamicCast() const {
      return dynamic_cast<T2 *>(ptr);
    }

    template <typename T2>
    Ptr<T2> ReinterpretCast() const {
      return reinterpret_cast<T2 *>(ptr);
    }
...
#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_PTR_OVERLOAD_FIX_HPP_GUARD
