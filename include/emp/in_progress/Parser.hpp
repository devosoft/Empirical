...
namespace emp {
  ...
  public:
    ...
    int GetID(const std::string & name) {
      ...
      if (spos >= 0) {
        return symbols[(size_t) spos].id;  // ...if so, return it.
      }
      size_t tid = lexer.GetTokenID(name);  // Otherwise, check for token name.
      if (Lexer::TokenOK(tid)) {
        return tid;  // ...if so, return id.
      }
    ...
    void Process(std::istream & is, bool test_valid = true) {
      ...
      while (progress) {
        ...
        for (auto & r : rules) {
          ...
          if (s.nullable) {
            continue;  // If a symbol is already nullable, skip it.
          }
          ...
          for (size_t sid : r.pattern) {
            ...
            if (pos < 0 || symbols[(size_t) pos].nullable == false) {
              cur_nullable = false;
              break;
            }
          }
    ...
    void Print(std::ostream & os = std::cout) const {
      ...
      for (const auto & s : symbols) {
        os << "symbol '" << s.name << "' (id " << s.id << ") has " << s.rule_ids.size()
           << " patterns.";
...
#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_PARSER_HPP_GUARD
