...
namespace emp {
  ...
  public:
    ...
    bool IsConnected(IndexRange in) const {
      return (in.start >= start && in.start <= end) || (start >= in.start && start <= in.end);
    }
  ...
  public:
    ...
    size_t GetStart() const { return range_set.size() ? range_set[0].GetStart() : emp::MAX_SIZE_T; }
    ...
    bool Append(size_t val) {
      if (range_set.size() == 0 || val > GetEnd()) {
        range_set.emplace_back(val);  // New Range
      } else if (val == GetEnd()) {
        range_set.back().SetEnd(val + 1);  // Extend range
      } else {
    ...
    bool Insert(IndexRange in) {
      ...
      if (start_id == end_id) {
        ...
        // Otherwise try to merge it into the existing range (will return false if already there)
          ...
          return range_set[start_id].Merge(in);
        }
      }

      // We are across multiple ranges.  Collapse into first!
      else {
        if (in.GetEnd() + 1 < range_set[end_id].GetStart()) {
          --end_id;  // Don't include end id.
        }
    ...
    bool Remove(size_t val) {
      ...
      if (cur_range.GetSize() == 1) {
        range_set.erase(range_set.begin() + id);
      } else if (cur_range.GetStart() == val) {
        cur_range.SetStart(cur_range.GetStart() + 1);
      } else if (cur_range.GetEnd() - 1 == val) {
        cur_range.SetEnd(cur_range.GetEnd() - 1);
      } else {
  ...
  class IndexBits {
    ...
    size_t _CalcOffset(size_t val) const { return (val >> 6) << 6; }
  ...
  public:
    ...
    size_t GetNumRanges() const { return (bits & ~(bits >> 1)).CountOnes(); }
    ...
    void Remove(size_t val) { bits.Clear(val - offset); }
  };
  ...
  private:
    ...
    void _ReleaseUnion() {
      if (type == index_t::BITS) {
        bits.~IndexBits();
      } else if (type == index_t::RANGES) {
        ranges.~IndexRangeSet();
      }
    }
    ...
    void _ToBits() {
      ...
      switch (type) {
        ...
        case index_t::VALS1: new_bits.Insert(vals.id1); break;
        case index_t::RANGES:
          for (const auto & range : ranges.GetRanges()) { new_bits.Insert(range); }
          break;
      }
    ...
    void _ToRanges() {
      ...
      if (type == index_t::RANGES) {
        return;  // Already ranges format!
      }
      ...
      switch (type) {
        ...
        case index_t::VALS1: new_ranges.Insert(vals.id1); break;
        case index_t::BITS:
          for (const auto & range : ranges.GetRanges()) { new_ranges.Insert(range); }
          break;
      }
  ...
  public:
    ...
    ~IndexSet() {
      switch (type) {
        case index_t::ARRAY: ids.array.ids.DeleteArray(); break;
    ...
    size_t GetMin() const {
      switch (type) {
        ...
        case index_t::BITS:  return emp::find_bit(ids.bits.bits[0]) + ids.bits.offset;
      }
    }
    ...
    bool IsConsecutive() const {
      switch (type) {
        ...
        case index_t::VALS3:
          return ids.vals.id3 == ids.vals.id2 + 1 && ids.vals.id2 == ids.vals.id1 + 1;
        case index_t::RANGE: return true;
        case index_t::ARRAY:
        case index_t::BITS:  return GetSize() == GetMax() - GetMin() + 1;
      }
    }
...
#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_INDEX_SET_HPP_GUARD
