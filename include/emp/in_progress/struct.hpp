/**
 * This file is part of Empirical, https://github.com/devosoft/Empirical
 * Copyright (C) 2021 Michigan State University
 * MIT Software license; see doc/LICENSE.md
 *
 * @file include/emp/in_progress/struct.hpp
 * @brief A re-engineering of tuple_struct.h, intended to be usable throughout Empirical.
 *
 * The EMP_STRUCT macro builds an inexpensive struct that
 * * Can fully reflect on its contents. (todo)
 * * Is easily serializable (todo)
 * * Can be effortlessly moved between C++ and Javascript (todo)
 *
 * EMP_STRUCT( NAME,
 *             TYPE1, ID1, DEFAULT1,
 *             TYPE2, ID2, DEFAULT2,
 *             etc... )
 *
 */

#pragma once

#ifndef INCLUDE_EMP_IN_PROGRESS_STRUCT_HPP_GUARD
#define INCLUDE_EMP_IN_PROGRESS_STRUCT_HPP_GUARD

#ifndef DOXYGEN_SHOULD_SKIP_THIS
#define EMP_STRUCT(NAME, ...)                               \
  struct NAME {                                              \
    EMP_ASSEMBLE_MACRO(EMP_STRUCT_MEM_, __VA_ARGS__)     \
  };

EMP_STRUCT_MEM_3(TYPE, ID, DEFAULT, ...) TYPE ID;
EMP_STRUCT_MEM_6(TYPE, ID, DEFAULT, ...) TYPE ID;
EMP_STRUCT_MEM_3(__VA_ARGS__)
EMP_STRUCT_MEM_30(TYPE, ID, DEFAULT, ...) TYPE ID;
EMP_STRUCT_MEM_27(__VA_ARGS__)

#endif /*EMP_STRUCT( NAME, ...)*/    // #ifndef DOXYGEN_SHOULD_SKIP_THIS
#endif /*DOXYGEN_SHOULD_SKIP_THIS*/  // #ifndef INCLUDE_EMP_IN_PROGRESS_STRUCT_HPP_GUARD

#endif  // #ifndef EMP_IN_PROGRESS_STRUCT_HPP_INCLUDE
