 ...
 *
 * @file include/emp/in_progress/Empower/VarInfo.hpp
 * @brief Generic info about a single variable (across MemoryImages)
 */

#pragma once

#ifndef INCLUDE_EMP_IN_PROGRESS_EMPOWER_VAR_INFO_HPP_GUARD
#define INCLUDE_EMP_IN_PROGRESS_EMPOWER_VAR_INFO_HPP_GUARD
...
namespace emp {
  ...
  public:
    ...
    Var GetVar(MemoryImage & memory) const { return Var(type, memory, mem_pos); }

    void DefaultConstruct(MemoryImage & memory) const { type.DefaultConstruct(mem_pos, memory); }
    ...
    void Destruct(MemoryImage & memory) const { type.Destruct(mem_pos, memory); }
  };


}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_EMPOWER_VAR_INFO_HPP_GUARD
