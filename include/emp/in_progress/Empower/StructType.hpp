 ...
 *
 * @file include/emp/in_progress/Empower/StructType.hpp
...
#ifndef INCLUDE_EMP_IN_PROGRESS_EMPOWER_STRUCT_TYPE_HPP_GUARD
#define INCLUDE_EMP_IN_PROGRESS_EMPOWER_STRUCT_TYPE_HPP_GUARD
...
namespace emp {
  ...
  public:
    ...
    const VarInfo & GetVarInfo(size_t id) const { return vars[id]; }

    /// Access a VarInfo based on its name.
    const VarInfo & GetVarInfo(const std::string & name) const { return GetVarInfo(GetID(name)); }
    ...
    void AddVar(const std::string & name) {
      ...
      emp_assert(name_map.find(name) == name_map.end(),
                 "All member vars in emp::Struct must be unique!");
...
#endif  // #ifndef INCLUDE_EMP_IN_PROGRESS_EMPOWER_STRUCT_TYPE_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: tmanager vinfo
