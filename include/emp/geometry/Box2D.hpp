...
namespace emp {
  ...
  public:
    ...
    Box2D<TYPE> & Translate(Point2D<TYPE> shift) {
      ul_corner += shift;
      return *this;
    }
    ...
    constexpr bool HasOverlap(const Box2D<TYPE> & other) const {
      ...
      if (other.GetTop() > GetBottom()) {
        return false;  // Other box starts BELOW this box.
      }
      if (other.GetLeft() > GetRight()) {
        return false;  // Other box is to the RIGHT of this one.
      }
      if (other.GetRight() < GetLeft()) {
        return false;  // Other box is to the LEFT of this one.
      }
      return true;
    }
  };
