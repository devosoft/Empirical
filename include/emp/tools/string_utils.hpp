...
namespace emp {
  ...
  static inline bool is_composed_of(const std::string & test_str, const std::string & char_set) {
    for (char x : test_str) {
      if (!is_one_of(x, char_set)) { return false; }
    }
    return true;
  }
  ...
  inline bool has_alphanumeric(const std::string & str) {
    return AlphanumericCharSet().HasAny(str);
  }
  ...
  inline bool is_digits(const std::string & str) {
    if (str.size() == 0) {
      return false;  // If string is empty, there are NO digits.
    }
  ...
  inline bool is_number(const std::string & str) {
    if (str.size() == 0) {
      return false;  // If string is empty, not a number!
    }
    size_t pos = 0;
    if (has_one_of_at(str, "+-", pos)) {
      ++pos;  // skip leading +/-
    }
    while (has_digit_at(str, pos)) {
      ++pos;  // Any number of digits (zero is okay)
    }
    if (has_char_at(str, '.', pos)) {  // If there's a DECIMAL PLACE, look for more digits.
      ++pos;                           // Skip over the dot.
      if (!has_digit_at(str, pos++)) {
        return false;  // Must have at least one digit after '.'
      }
      while (has_digit_at(str, pos)) {
        ++pos;  // Any number of digits.
      }
    }
    if (has_one_of_at(str, "eE", pos)) {  // If there's an e... SCIENTIFIC NOTATION
      ...
      if (!has_digit_at(str, pos++)) {
        return false;  // Must have at least one digit after 'e'
      }
      while (has_digit_at(str, pos)) {
        ++pos;  // Allow for MORE digits.
      }
    }
  ...
  inline bool is_alphanumeric(const std::string & str) {
    if (str.size() == 0) {
      return false;  // If string is empty, there are NO characters.
    }
    return AlphanumericCharSet().Has(
      str);  // Otherwise return false if any character is not a digit.
  }

  inline bool is_whitespace(const std::string & str) {
    if (str.size() == 0) {
      return true;  // Empty string is just whitespace.
    }
  ...
  inline bool is_identifier(const std::string & str) {
    ...
    if (is_digit(str[0])) {
      return false;  // Identifiers cannot begin with a number.
    }
  ...
  static inline bool has_one_of(const std::string & str, const std::string & char_set) {
    for (char c : str) {
      if (is_one_of(c, char_set)) { return true; }
    }
    return false;
  }
  ...
  static inline bool is_valid(const std::string & str, FUNS... funs) {
    for (char x : str) {
      if (!is_valid(x, funs...)) { return false; }
    }
    return true;
  }
  ...
  static inline size_t find_quote_match(std::string_view in_string,
    ...
    for (size_t pos = start_pos + 1; pos < in_string.size(); ++pos) {
      ...
      if (in_string[pos] == mark) { return pos; }
    }
  ...
  static inline size_t find_paren_match(std::string_view in_string,
                                        const size_t start_pos = 0,
                                        const char open        = '(',
                                        const char close       = ')',
  ...
  static inline size_t find(std::string_view in_string,
                            std::string target,
                            size_t start_pos,
    ...
    for (size_t scan_pos = 0; scan_pos < found_pos && found_pos != std::string::npos; scan_pos++) {
  ...
  static inline void find_all(std::string_view in_string,
                              char target,
                              emp::vector<size_t> & results,
                              const bool skip_quotes = false,
                              bool skip_parens       = false,
                              bool skip_braces       = false,
                              bool skip_brackets     = false) {
    results.resize(0);
  ...
  static inline emp::vector<size_t> find_all(std::string_view in_string,
                                             char target,
                                             bool skip_quotes   = false,
                                             bool skip_parens   = false,
                                             bool skip_braces   = false,
  ...
  static inline size_t find_any_of_from(const std::string & test_str,
                                        size_t start_pos,
                                        std::string test1,
                                        Ts... tests) {
  ...
  static inline size_t find_id(std::string_view in_string,
                               std::string target,
                               size_t start_pos,
                               ...
                               bool skip_brackets = false) {
    size_t pos =
      emp::find(in_string, target, start_pos, skip_quotes, skip_parens, skip_braces, skip_brackets);
  ...
  template <typename T>
  inline typename std::enable_if<!emp::IsIterable<T>::value, std::string>::type to_literal(
    const T & value) {
  ...
  [[nodiscard]] static inline std::string to_roman_numeral(int val,
                                                           const std::string & prefix = "") {
    std::string ret_string(prefix);
    if (val < 0) {
      ret_string += to_roman_numeral(-val, "-");
    } else if (val > 3999) {
      ;
    }  // Out of bounds; return a blank;
    else if (val >= 1000) {
      ret_string += to_roman_numeral(val - 1000, "M");
    } else if (val >= 900) {
      ret_string += to_roman_numeral(val - 900, "CM");
    } else if (val >= 500) {
      ret_string += to_roman_numeral(val - 500, "D");
    } else if (val >= 400) {
      ret_string += to_roman_numeral(val - 400, "CD");
    } else if (val >= 100) {
      ret_string += to_roman_numeral(val - 100, "C");
    } else if (val >= 90) {
      ret_string += to_roman_numeral(val - 90, "XC");
    } else if (val >= 50) {
      ret_string += to_roman_numeral(val - 50, "L");
    } else if (val >= 40) {
      ret_string += to_roman_numeral(val - 40, "XL");
    } else if (val >= 10) {
      ret_string += to_roman_numeral(val - 10, "X");
    } else if (val == 9) {
      ret_string += "IX";
    } else if (val >= 5) {
      ret_string += to_roman_numeral(val - 5, "V");
    } else if (val == 4) {
      ret_string += "IV";
    } else if (val > 0) {
      ret_string += to_roman_numeral(val - 1, "I");
    }
  ...
  static inline void compress_whitespace(std::string & in_string) {
    ...
    if (pos && last_whitespace) {
      pos--;  // If the end of the line is whitespace, remove it.
    }
  ...
  static inline void remove_punctuation(std::string & in_string) {
    ...
    for (size_t i = 0; i < strlen; i++) {
      ...
      if (is_alphanumeric(cur_char) || is_whitespace(cur_char)) { in_string[pos++] = cur_char; }
    }
  ...
  [[nodiscard]] static inline std::string combine_strings(const string_vec_t & strings,
                                                          std::string spacer = " ") {
  ...
  [[nodiscard]] static inline string_vec_t transform_strings(
    const string_vec_t & in_strings,
    std::function<std::string(const std::string &)> fun) {
    string_vec_t out_strings(in_strings.size());
    for (size_t i = 0; i < in_strings.size(); i++) { out_strings[i] = fun(in_strings[i]); }
    return out_strings;
  }

  /// Put all strings provided in quotes (Like 'this'), pre- and post-fixing another string if
  /// provided.
  [[nodiscard]] static inline string_vec_t quote_strings(const string_vec_t & in_strings,
                                                         const std::string quote = "'") {
    return transform_strings(in_strings,
                             [quote](const std::string & str) { return quote + str + quote; });
  }
  ...
  static inline std::string string_pop_fixed(std::string & in_string,
                                             std::size_t end_pos,
                                             size_t delim_size = 0) {
    if (end_pos == 0) {
  ...
  [[nodiscard]] static inline std::string string_get_range(const std::string & in_string,
  ...
  [[nodiscard]] inline std::string string_get(const std::string & in_string,
                                              const char delim = ' ',
                                              size_t start_pos = 0) {
  ...
  inline std::string string_pop_to(std::string & in_string,
  ...
  [[nodiscard]] inline std::string string_get_to(const std::string & in_string,
                                                 const std::string & delim = " ",
                                                 size_t start_pos          = 0) {
    return string_get_range(in_string, start_pos, in_string.find(delim, start_pos));
  }
  ...
  [[nodiscard]] inline std::string string_get_word(const std::string & in_string,
                                                   size_t start_pos = 0) {
    // Whitespace = ' ' '\n' '\r' or '\t'
  ...
  [[nodiscard]] inline std::string string_get_line(const std::string & in_string,
                                                   size_t start_pos = 0) {
    return string_get(in_string, '\n', start_pos);
  }
...
#pragma GCC diagnostic pop
  ...
  [[nodiscard]] std::string replace_macro(const std::string & str,
  ...
  inline std::string repeat(const std::string & value, const size_t n) {
    ...
    return std::accumulate(std::begin(repeated), std::end(repeated), std::string{});
  }
  ...
  static inline void slice(const std::string_view & in_string,
  ...
  static inline std::map<std::string, std::string> slice_assign(
    ...
    auto assign_set =
      emp::slice(in_string, delim, max_split, keep_quotes, keep_parens, keep_braces, keep_brackets);
  ...
  namespace internal {
    ...
    template <typename T>
    const T & to_stream_item(const T & in, ...) {
      return in;
    }
  ...
  inline emp::vector<T> from_strings(const emp::vector<std::string> & string_v) {
    ...
    for (size_t i = 0; i < string_v.size(); i++) { vals[i] = from_string<T>(string_v[i]); }
    return vals;
  }
  ...
  static inline bool is_literal_string(const std::string & value, const std::string & quote_marks) {
    if (value.size() < 2) {
      return false;  // Two short to contain even quote marks!
    }
    char quote = value[0];
    if (!is_one_of(quote, quote_marks)) {
      return false;  // Must be working with allowed quote mark.
    }
    if (value.back() != quote) {
      return false;  // Must use same quote at front and back.
    }
    ...
    for (size_t pos = 1; pos < value.size() - 1; pos++) {
      ...
      if (value[pos] == '\\') {
        ...
        switch (value[pos]) {
          ...
          case '`':   // Back quote
            continue;
          default: return "Unknown escape character.";
        }
      }
    }
  ...
  static inline std::string to_escaped_string(char value) {
    ...
    switch (value) {
      ...
      // case '\?': return "\\\?";
      default: ss << value; return ss.str();
    };
  }
  ...
  static inline std::string from_literal_string(const std::string & value,
    ...
    emp_assert(is_literal_string(value, quote_marks),
               value,
               diagnose_literal_string(value, quote_marks));
    ...
    for (size_t pos = 1; pos < value.size() - 1; pos++) {
      ...
      switch (value[pos]) {
        ...
        default:   emp_assert(false, "unknown escape char used; probably need to update converter!");
      }
    }
  ...
  [[nodiscard]] std::string replace_vars(const std::string & in_string, const MAP_T & var_map) {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      if (result[i] != '$') {
        continue;  // Replacement tag must start with a '$'.
      }
      if (result.size() <= i + 2) {
        break;  // Not enough room for a replacement tag.
      }
      ...
      if (replacement_it == var_map.end()) {
        emp::notify::Exception("emp::string_utils::replace_vars::missing_var",
                               emp::to_string("Lookup variable not found in var_map (key=",
                                              key,
                                              ")"),
                               key);
  ...
  template <typename FUN_T>
  [[nodiscard]] std::string replace_macro(const std::string & in_string,
                                          std::string macro_name,
                                          FUN_T && macro_fun,
                                          bool skip_quotes) {
    std::stringstream out;
    ...
    for (size_t macro_pos = emp::find_id(in_string, macro_name, 0, skip_quotes);
      ...
      if (in_string[macro_pos] != '(') {
        emp::notify::Warning("Line ",
                             line_num,
                             ": Invalid MACRO instance of '",
                             macro_name,
                             "' - found ",
                             in_string[macro_pos],
                             "instead of '('.");
  ...
  static inline size_t next_pos(std::string_view in_string,
                                size_t pos,
  ...
  static inline void slice(const std::string_view & in_string,
    ...
    out_set.resize(0);
    if (in_string.empty() || max_split == 0) {
...
#endif  // #ifndef INCLUDE_EMP_TOOLS_STRING_UTILS_HPP_GUARD
