...
namespace emp {
  ...
  struct TrackedVar {
    ...
    TrackedVar(const TrackedVar & _in) : ptr(nullptr) {
      if (_in.ptr) { ptr = _in.ptr->Clone(); }
    }
  ...
  struct TrackedInfo_Value : public TrackedInfo_Base {
    ...
    emp::Ptr<TrackedInfo_Base> Clone() const override { return emp::NewPtr<this_t>(value); }
  };
  ...
  public:
    ...
    ~TypeTracker() {
      for (auto x : fun_map) {
        x.second.Delete();  // Clear out Functions.
      }
    }
    ...
    constexpr static size_t GetID() {
      static_assert(get_type_index<T, TYPES...>() != -1,
                    "Can only get IDs for pre-specified types.");
    ...
    static OUT_T Cast(TrackedVar & tt) {
      return tt.ptr.Cast<wrap_t<OUT_T>>()->value;
    }
...
#endif  // #ifndef INCLUDE_EMP_TOOLS_TYPE_TRACKER_HPP_GUARD
