...
#ifndef INCLUDE_EMP_TOOLS_STRING_SYNTAX_HPP_impl_GUARD
#define INCLUDE_EMP_TOOLS_STRING_SYNTAX_HPP_impl_GUARD
...
namespace emp {
  ...
  public:
    ...
    StringSyntax(std::string quotes, std::string parens = "") : StringSyntax() {
      emp_assert(parens.size() % 2 == 0,
                 "String::StringSyntax must have odd number of paren chars.");
    ...
    char & GetMatch(char c) {
      emp_assert(c >= 0);
      return char_matches[static_cast<size_t>(c)];
    }
...
#endif  // #ifndef INCLUDE_EMP_TOOLS_STRING_SYNTAX_HPP_impl_GUARD
