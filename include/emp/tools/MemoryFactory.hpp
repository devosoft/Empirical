 ...
 *
 * @file include/emp/tools/MemoryFactory.hpp
...
#ifndef INCLUDE_EMP_TOOLS_MEMORY_FACTORY_HPP_GUARD
#define INCLUDE_EMP_TOOLS_MEMORY_FACTORY_HPP_GUARD
...
namespace emp {
  ...
  namespace internal {
    ...
    protected:
      ...
      size_t ToID(emp::Ptr<T> in) { return (in.Raw() - data.PoolPtr().Raw()) / data.mem_count; }
    public:
      MemFactory_impl() {
      ...
      ~MemFactory_impl() {
        ...
        if constexpr (DATA_T::is_dynamic) {
          ...
          for (auto & mem : retired_mem) { mem.begin_ptr.DeleteArray(); }
        }
      }
      ...
      bool IsFreeID(size_t id) const {
        return std::any_of(data.FreeBegin(), data.FreeBegin() + free_count, id);
      }
      ...
      void Release(emp::Ptr<T> in) {
        ...
        if constexpr (!is_trivial) { std::destroy(in.Raw(), (in + data.mem_count).Raw()); }
        ...
        if constexpr (data.is_dynamic) {
          if (!IsCurrent(in)) {
            ...
            size_t pool_id;
            while (pool_id < retired_mem.size() && !retired_mem[pool_id].Included(in)) {
              ++pool_id;
            }
  ...
  template <typename T>
  using MemoryFactory = internal::MemFactory_impl<T, internal::MemFactory_DynamicData<T>>;

}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_TOOLS_MEMORY_FACTORY_HPP_GUARD
