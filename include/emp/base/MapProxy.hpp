...
namespace emp {
  ...
  public:
    ...
    T & operator=(R_T && _in) {
      is_init      = true;
      return value = std::forward<R_T>(_in);
    }

    // Setup other assignment operators
    template <typename R_T>
    T & operator+=(R_T && _in) {
      return emp_GetValue() += std::forward<R_T>(_in);
    }
    ...
    template <typename R_T>
    auto operator+(const R_T & r) const {
      return emp_GetValue() + r;
    }
    ...
    // Setup member access
    template <typename R_T>
    auto & operator[](const R_T & r) {
      emp_assert(is_init);
      return value[r];
    }

    template <typename R_T>
    const auto & operator[](const R_T & r) const {
      emp_assert(is_init);
      return value[r];
    }
    ...
    auto operator()(R_Ts &&... rs) {
      emp_assert(is_init);
      return value(std::forward<R_Ts>(rs)...);
    }

    template <typename... R_Ts>
    auto operator()(R_Ts &&... rs) const {
      emp_assert(is_init);
      return value(std::forward<R_Ts>(rs)...);
    }
    ...
    operator T &() {
      return value;
    }  // Note that a non-const reference doesn't need var to be initialized yet.
  ...
  template <typename>
  struct is_MapProxy : public std::false_type {};
  ...
  template <typename T1,
            typename T2,
            typename std::enable_if<is_MapProxy<T1>() == false>::type * = nullptr>
