...
namespace emp {
  ...
  struct array_iterator {
    ...
    bool OK(bool begin_ok = true, bool end_ok = true) const {
      ...
      if (it < arr_ptr->begin()) {
        return false;  // Iterator before array start.
      }
      if (it > arr_ptr->end()) {
        return false;  // Iterator after array end.
      }
      if (!begin_ok && it == arr_ptr->begin()) {
        return false;  // Iterator not allowed at start.
      }
      if (!end_ok && it == arr_ptr->end()) {
        return false;  // Iterator not allowed at end.
      }
      return true;
    }
    ...
    this_t operator++(int /*x*/) {
      emp_assert(OK(true, false));
      return this_t(it++, arr_ptr);
    }

    this_t & operator--() {
      emp_assert(OK(false, true));
      --it;
      return *this;
    }
    ...
    ptrdiff_t operator-(const this_t & in) {
      emp_assert(OK());
      return it - in.it;
    }
