...
namespace emp {
    ...
    bool save_lexeme, bool save_line_id, bool save_col_id) const
  {
    ...
    file.AddCode("  }")
        ...
        .AddCode("")
        .AddCode("  // Test if a specific token is next.")
        .AddCode("  bool Is(int id) const { return Any() && Peek() == id; }")
        .AddCode("")
        .AddCode("  // Get the next token, but don't remove it from the queue.")
        .AddCode("  const ", token_name, " & Peek() const {")
        .AddCode("    if (None()) return eof_token;")
        .AddCode("    return tokens[token_id];")
        .AddCode("  }")
        .AddCode("")
        .AddCode("  // Get the next token, removing it from the queue.")
        .AddCode("  const ", token_name, " & Use() {")
        .AddCode("    if (None()) return eof_token;")
        .AddCode("    return tokens[token_id++];")
        .AddCode("  }")
        .AddCode("")
        .AddCode("  // Use the next token if it is the expected type; otherwise error.")
        ...
        .AddCode("  }")
        .AddCode("")
        .AddCode("  // If the next token is one of the provided ids, use it an return the ID used.")
