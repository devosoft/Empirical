...
#ifndef INCLUDE_EMP_COMPILER_TOKEN_TYPE_HPP_impl_GUARD
#define INCLUDE_EMP_COMPILER_TOKEN_TYPE_HPP_impl_GUARD
...
namespace emp {
  ...
  struct TokenType {
    ...
    TokenType()
      : name("")
      , desc("Unable to parse input!")
      , regex("")
      , id(-1)
      , save_lexeme(true)
      , save_token(true) {}

    TokenType(const String & _name,
              const String & _regex,
              int _id,
              bool _save_l         = true,
              bool _save_t         = true,
    ...
    void Print(std::ostream & os = std::cout) const {
      os << "Name:" << name << "  RegEx:" << regex.ToString() << "  ID:" << id
         << "  save_lexeme:" << save_lexeme << "  save_token:" << save_token << std::endl;
    }
  };

}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_COMPILER_TOKEN_TYPE_HPP_impl_GUARD
