...
#endif  // #ifndef EMP_LOG_MATCHBIN_FILENAME
...
namespace emp::internal {
  ...
  class MatchBinLog {
    ...
    using logbuffer_t = robin_hood::unordered_map<LogEntry, size_t, TupleHash<LogEntry> >;
...
#else   // #ifdef EMP_LOG_MATCHBIN
    static constexpr bool logging_enabled = false;
#endif  // #ifdef EMP_LOG_MATCHBIN : #else
    ...
    std::function<std::string(const datapoint_t)> get_logbuffer_type =
      [](const datapoint_t datapoint) { return datapoint.first.buffer; };
    ...
    void SetupDatafile() {
      ...
      datafile.AddContainerFun(get_hit_count_log,
                               "hit_count",
                               "Counter for how many times this match occurred");
  ...
  public:
    MatchBinLog()
      : log_counter(0), instance_id(MakeID()), logging_activated(false), datafile(filestream) {
    ...
    /// Clear the logbuffer
    void ClearLogBuffer() {
      if constexpr (logging_enabled) { logbuffer.clear(); }
    }
  ...
  class MatchBinCache {
    ...
    template <typename Val, typename Metric, typename Selector_, typename Regulator>
...
#endif  // #ifdef EMP_THREAD_SAFE

    static constexpr bool cache_available = std::is_base_of<CacheStateBase, cache_state_t>::value;
    bool caching_activated{cache_available};

    // caches
    // cache of regulated scores
    robin_hood::unordered_flat_map<query_t, cache_state_t> cache_regulated;
    // cache of raw scores
    robin_hood::unordered_flat_map<query_t, cache_state_t> cache_raw;
  ...
  public:
    ...
    cache_state_t & GetRegulated(const query_t & query) { return cache_regulated.at(query); }

    /// Gets query from regulated cache.
    /// User must check for existence of query in cache before calling this method.
    cache_state_t & GetRaw(const query_t & query) { return cache_raw.at(query); }
...
namespace emp {
  ...
  struct MatchBinState {
    ...
    struct entry {
      ...
      void serialize(Archive & ar) {
        ar(CEREAL_NVP(val), CEREAL_NVP(regulator), CEREAL_NVP(tag));
      }
    };
  ...
  public:
    ...
    virtual const typename Regulator::view_t & ViewRegulator(const uid_t uid) const   = 0;
  ...
  template <typename Val, typename Metric, typename Selector, typename Regulator>
  class MatchBin
    : public BaseMatchBin<Val, typename Metric::query_t, typename Metric::tag_t, Regulator> {
  ...
  public:
    ...
    MatchBin(emp::Random & rand) : selector(rand) { ; }

    // have to define this manually due to mutexes
    MatchBin(const MatchBin & other)
      : state(other.state), uid_stepper(other.uid_stepper), selector(other.selector) {}
    ...
    emp::vector<uid_t> Match(const query_t & query,
                             size_t n = std::numeric_limits<size_t>::max()) override {
      const auto makeResult = [&]() {
        ...
        std::transform(std::begin(state.data),
                       std::end(state.data),
                       std::back_inserter(scores),
                       [&](const auto & pair) {
                         const auto & [uid, pack] = pair;
                         return std::pair{uid, pack.regulator(Metric::calculate(query, pack.tag))};
                       });
      ...
      if (result.empty()) { log.LogMiss(query, "regulated"); }
      for (const auto & uid : result) { log.LogMatch(query, GetTag(uid), "regulated"); }
#endif  // #ifdef EMP_LOG_MATCHBIN
    ...
    /// Calling with n = std::numeric_limits<size_t>::max() means
    /// delegate choice for how many values to return to the Selector.
    emp::vector<uid_t> MatchRaw(const query_t & query,
                                size_t n = std::numeric_limits<size_t>::max()) override {
      const auto makeResult = [&]() {
        // compute distance between query and all stored tags
        emp::vector<std::pair < uid_t, double> > scores;
        ...
        std::transform(std::begin(state.data),
                       ...
                       [&](const auto & pair) {
                         const auto & [uid, pack] = pair;
                         return std::pair{uid, Metric::calculate(query, pack.tag)};
                       });
...
#ifdef EMP_LOG_MATCHBIN
      if (result.empty()) { log.LogMiss(query, "raw"); }
      // store counts for results
      for (const auto & uid : result) { log.LogMatch(query, GetTag(uid), "raw"); }
#endif  // #ifdef EMP_LOG_MATCHBIN
    ...
    void ClearCache() override { cache.Clear(); }

    /// Attempt to activate result caching.
    /// @return true if caching activated
    bool ActivateCaching() override { return cache.Activate(); }

    void DeactivateCaching() override { cache.Deactivate(); }
    ...
    void AdjRegulator(const uid_t uid, const typename Regulator::adj_t & amt) override {
      emp_assert(state.data.find(uid) != std::end(state.data));
    ...
    void SetRegulator(const uid_t uid, const typename Regulator::set_t & set) override {
      emp_assert(state.data.find(uid) != std::end(state.data));
    ...
    /// Set an item's regulator value.
    void SetRegulator(const uid_t uid, const Regulator & set) override {
      emp_assert(state.data.find(uid) != std::end(state.data));

      if (set != std::exchange(state.data.at(uid).regulator, set)) { cache.ClearRegulated(); }
    }

    /// View an item's regulator value.
    const typename Regulator::view_t & ViewRegulator(const uid_t uid) const override {
      emp_assert(state.data.find(uid) != std::end(state.data));
    ...
    void DecayRegulator(const uid_t uid, const int steps) override {
      ...
      if (state.data.at(uid).regulator.Decay(steps)) { cache.ClearRegulated(); }
    }
    ...
    void ImprintRegulators(const BaseMatchBin<Val, query_t, tag_t, Regulator> & target) override {
      ImprintRegulators(target.GetState());
    }
    ...
    void ImprintRegulators(const state_t & target) override {
      for (const auto & [uid, pack] : state.data) {
        ...
        std::transform(std::cbegin(target.data),
                       ...
                       [this, uid = uid](const auto & target_pair) {
                         ...
                         return std::pair{target_uid,
                                          Metric::calculate(target_pack.tag, GetTag(uid))};
                       });
        SetRegulator(uid,
                     target.data
                       .at(std::min_element(std::begin(scores),
                                            std::end(scores),
...
namespace cereal {
  ...
  void save(Archive & archive, const robin_hood::unordered_flat_map<size_t, T> & m) {
    std::unordered_map<size_t, T> t;
    std::transform(std::begin(m),
                   std::end(m),
...
#endif  // #ifndef INCLUDE_EMP_MATCHING_MATCH_BIN_HPP_GUARD
