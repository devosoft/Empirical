 ...
 *
 * @file include/emp/matching/regulators/PlusCountdownRegulator.hpp
...
#ifndef INCLUDE_EMP_MATCHING_REGULATORS_PLUS_COUNTDOWN_REGULATOR_HPP_GUARD
#define INCLUDE_EMP_MATCHING_REGULATORS_PLUS_COUNTDOWN_REGULATOR_HPP_GUARD
...
namespace emp {
  ...
  struct PlusCountdownRegulator {
    ...
    __attribute__((hot)) float operator()(const float raw_score) const {
      const float res = std::clamp(slope * state + raw_score, 0.0f, 1.0f);
      emp_assert(state <= 0.0f || res >= raw_score || std::isnan(state), state, res, raw_score);
      emp_assert(state >= 0.0f || res <= raw_score || std::isnan(state), state, res, raw_score);
      emp_assert((res >= 0.0f && res <= 1.0f) || std::isnan(state), res);
      return res;
    }
    ...
    bool Decay(const int steps) {
      ...
      return timer == 0 ? (std::exchange(state, 0.0f) != 0.0f) : false;
    }
...
#endif  // #ifndef INCLUDE_EMP_MATCHING_REGULATORS_PLUS_COUNTDOWN_REGULATOR_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: deci adj amt
