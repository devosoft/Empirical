...
namespace emp {
  ...
  private:
    ...
    res_t * DoRegulatedLookup(const query_t & query) { return cache_regulated.get(query); }
  ...
  public:
    ...
    /// function.
    __attribute__((hot)) res_t MatchRegulated(const query_t & query) {
      if constexpr (RegulatedCacheSize > 0) {
        if (const auto res = DoRegulatedLookup(query); res != nullptr) { return *res; }
      }
    ...
    __attribute__((hot)) res_t MatchRaw(const query_t & query) {
    ...
    const view_t & ViewRegulator(const uid_t uid) const { return data.at(uid).reg.View(); }
    ...
    bool HasVal(const Val & val) const {
      const auto found = std::find_if(std::begin(data), std::end(data), [&val](const auto & entry) {
    ...
    uid_t GetUid(const tag_t & tag) const {
      const auto found = std::find_if(std::begin(data), std::end(data), [&tag](const auto & entry) {
...
#endif  // #ifndef INCLUDE_EMP_MATCHING_MATCH_DEPOSITORY_HPP_GUARD
