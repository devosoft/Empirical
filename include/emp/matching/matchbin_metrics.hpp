...
namespace emp {
  ...
  struct HammingMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    ...
    std::string name() const override { return emp::to_string(Width) + "-bit " + base(); }

    std::string base() const override { return "Hamming Metric"; }

    double operator()(const query_t & a, const tag_t & b) const override { return calculate(a, b); }
  ...
  struct HashMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    ...
    inline static double calculate(const query_t & a, const tag_t & b) {
      ...
      const std::span<const std::byte> input_as_bytes(reinterpret_cast<std::byte *>(input),
                                                      sizeof(input));
...
#include <openssl/sha.h>
  ...
  struct CryptoHashMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    ...
    inline static double calculate(const query_t & a, const tag_t & b) {
      ...
      const std::string hashme{reinterpret_cast<const char *>(digest.data()), digest.size()};

      return (static_cast<double>(std::hash<std::string>{}(hashme)) /
              std::numeric_limits<size_t>::max());
    }
  };
#endif  // #ifdef EMP_HAS_CRYPTO
  ...
  struct AbsDiffMetric : public BaseMetric<int, int> {
    ...
    std::string base() const override { return "Absolute Integer Difference Metric"; }
  ...
  /// Adapted from Downing, Keith L. Intelligence emerging: adaptivity and search in evolving neural systems. MIT Press, 2015.
  template <size_t Width>
  struct ApproxSingleStreakMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    using query_t = emp::BitSet<Width>;
    using tag_t   = emp::BitSet<Width>;
    ...
    static inline double ProbabilityKBitSequence(const size_t k) {
      // Bad Math
      // ... at least clamp it
      return std::clamp(static_cast<double>(Width - k + 1) / std::pow(2, k), 0.0, 1.0);
    }
  };
  ...
  /// Adapted from Downing, Keith L. Intelligence emerging: adaptivity and search in evolving neural systems. MIT Press, 2015.
  template <size_t Width>
  struct ApproxDualStreakMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    using query_t = emp::BitSet<Width>;
    using tag_t   = emp::BitSet<Width>;
  ...
  struct ExactStreakDistribution {
    robin_hood::unordered_map<std::tuple<size_t, size_t>, /* (min_heads, num_coins) */
                              double,
    ...
    ExactStreakDistribution() {
      // prep the cache
      for (size_t min_heads = 0; min_heads <= N; ++min_heads) { CalcStreakProbability(min_heads); }
    }

    double GetStreakProbability(const size_t min_heads, const size_t num_coins = N) const {
      return computed.at({min_heads, num_coins});
    }

    double CalcStreakProbability(const size_t min_heads, const size_t num_coins = N) {
      ...
      for (size_t first_tail = 0; first_tail < min_heads; ++first_tail) {
        res += (emp::Pow(head_prob, static_cast<double>(first_tail)) * (1.0 - head_prob) *
                CalcStreakProbability(min_heads, num_coins - first_tail - 1));
      }
  ...
  /// Adapted from Downing, Keith L. Intelligence emerging: adaptivity and search in evolving neural systems. MIT Press, 2015.
  template <size_t Width>
  struct ExactDualStreakMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    using query_t = emp::BitSet<Width>;
    using tag_t   = emp::BitSet<Width>;
  ...
  /// Adapted from Downing, Keith L. Intelligence emerging: adaptivity and search in evolving neural systems. MIT Press, 2015.
  template <size_t Width>
  struct ExactSingleStreakMetric : public BaseMetric<emp::BitSet<Width>, emp::BitSet<Width>> {
    using query_t = emp::BitSet<Width>;
    using tag_t   = emp::BitSet<Width>;
  ...
  struct CacheMod : public Metric {
    ...
    static inline robin_hood::
      unordered_map<std::tuple<query_t, tag_t>, double, emp::TupleHash<query_t, tag_t>>
        cache;
  ...
  struct PowMod : public Metric {
    ...
    std::string name() const override {
      return (emp::to_string(Root::num) + "/" + emp::to_string(Root::den) + " Power " +
              metric.name());
    }
  ...
  struct LogMod : public Metric {
    ...
    std::string name() const override {
      return (emp::to_string(Base::num) + "/" + emp::to_string(Base::den) + " Logarithm " +
              metric.name());
    }
    ...
    inline static double calculate(const query_t & a, const tag_t & b) {
      ...
      const double antilog = ((2 - base) + 2 * (base - 1) * (std::abs(raw) + 0.5));
      return 0.5 * (1.0 + std::copysign(emp::Log(antilog, base), raw));
    }
  ...
  namespace internal {
    ...
    public:
      EstimatedLookupTable(const Metric & metric) {
        ...
        for (auto front = std::begin(observations); front != std::end(observations);
             front =
               std::upper_bound(front,
          ...
          if (front != std::prev(back)) { table.push_back(*std::prev(back)); }
        }
      }
      ...
      double operator()(const double raw) const {
        ...
        if (tail->first == raw && std::next(tail) != std::end(table) &&
        ...
        const double raw_frac =
          (tail->first == head->first) ? 0.5 : ((raw - head->first) / (tail->first - head->first));
  ...
  namespace internal {
    ...
    struct lookup_holder {
      const Metric metric;
      const internal::EstimatedLookupTable<Metric, Samples> lookup;
  ...
  struct MeanDimMod
    : public BaseMetric<std::array<typename Metric::query_t, Dim>,
  ...
  struct HarmonicDimMod
    ...
    inline static double calculate(const query_t & a, const tag_t & b) {
      ...
      for (size_t d = 0; d < Dim; ++d) { res += 1 / Metric::metric(a[d], b[d]); }
  ...
  struct FlatMod
    : public BaseMetric<emp::BitSet<std::tuple_size<typename DimMetric::query_t>::value *
                                    DimMetric::query_t::value_type::GetCTSize()>,
                        emp::BitSet<std::tuple_size<typename DimMetric::tag_t>::value *
                                    DimMetric::tag_t::value_type::GetCTSize()>> {
    using query_t = emp::BitSet<std::tuple_size<typename DimMetric::query_t>::value *
                                DimMetric::query_t::value_type::GetCTSize()>;
    using tag_t   = emp::BitSet<std::tuple_size<typename DimMetric::query_t>::value *
                                DimMetric::tag_t::value_type::GetCTSize()>;
...
#endif  // #ifndef INCLUDE_EMP_MATCHING_MATCHBIN_METRICS_HPP_GUARD

// Local settings for Empecable file checker.
// empecable_words: isocpp unif bino cumu distn askamathematician uniformified downing's lcrypto ctors hashme bernoulli shactx amt srtate cumsum cmp uncg antilog thasher sha lssl qhasher
