...
namespace emp {
  ...
  struct RouletteCacheState : public CacheStateBase {
    ...
    RouletteCacheState(emp::IndexMap & im,
                       emp::vector<size_t> ids,
                       emp::Random & r,
                       const size_t default_n_)
      : indexMap(im), uids(std::move(ids)), rand(&r), default_n(default_n_) {
      ;
    }
    ...
    emp::optional<emp::vector<size_t>> operator()(size_t n) override {
      ...
      for (size_t i = 0; i < n; ++i) {
        ...
        const double match_pos =
          (indexMap.GetSize() == 1 ? 0 : rand->GetDouble(indexMap.GetWeight()));
  ...
  struct SieveCacheState : public CacheStateBase {
    ...
    SieveCacheState(emp::vector<size_t> uids_,
                    const emp::vector<double> & probs_,
                    emp::Random & r,
                    const size_t default_n_)
      : uids(std::move(uids_)), probs(probs_), rand(&r), default_n(default_n_) {
    ...
    emp::optional<emp::vector<size_t>> operator()(size_t n) override {
      ...
      for (size_t i = 0; i < uids.size() && res.size() < n; ++i) {
        if (probs[i] == 1.0 || rand->GetDouble() < probs[i]) { res.push_back(uids[i]); }
      }
  ...
  struct RankedCacheState : public CacheStateBase {
    ...
    RankedCacheState(emp::vector<size_t> uids_, size_t n, size_t default_n_)
      : uids(std::move(uids_)), requestSize(n), default_n(default_n_) {
      ;
    }
    ...
    emp::optional<emp::vector<size_t>> operator()(size_t n) override {
      ...
      return emp::vector<size_t>(std::begin(uids), std::begin(uids) + n);
    }
  };
  ...
  struct SelectorBase {
    ...
    virtual CacheType operator()(emp::vector<std::pair < size_t, double> > scores, size_t n) = 0;
  ...
  struct RankedSelector : public SelectorBase<RankedCacheState> {
    ...
    std::string name() const override {
      return emp::to_string("Ranked Selector (",
                            "ThreshRatio: ",
                            ThreshRatio::num,
                            "/",
                            ThreshRatio::den,
    ...
    RankedCacheState operator()(emp::vector<std::pair < size_t, double> > scores,
                                size_t n) override {
      if (n == std::numeric_limits<size_t>::max()) { n = DefaultN; }

      // treat any negative numerator as positive infinity
      constexpr double thresh = (ThreshRatio::num < 0 ? std::numeric_limits<double>::infinity()
      ...
      std::partial_sort(std::begin(scores),
      ...
      const auto back =
        std::upper_bound(std::begin(scores),
                         std::begin(scores) + std::min(n, scores.size()),
                         thresh,
                         [](const double amt, const auto & data) { return amt < data.second; });

      emp::vector<size_t> res;
      res.reserve(std::distance(std::begin(scores), back));
      std::transform(std::begin(scores), back, std::back_inserter(res), [](const auto & pair) {
        return pair.first;
      });
  ...
  template <typename ThreshRatio      = std::ratio<-1, 1>,  // we treat neg numerator as +infty
  ...
  struct RouletteSelector : public SelectorBase<RouletteCacheState> {
    ...
    RouletteSelector(emp::Random & rand_) : rand(rand_) { ; }

    std::string name() const override {
      return emp::to_string("Roulette Selector (",
                            "ThreshRatio: ",
                            ThreshRatio::num,
                            "/",
                            ThreshRatio::den,
    ...
    RouletteCacheState operator()(emp::vector<std::pair < size_t, double> > scores,
                                  size_t n) override {
      if (n == std::numeric_limits<size_t>::max()) { n = DefaultN; }
      ...
      constexpr double max_baseline =
        (MaxBaselineRatio::num < 0 ? std::numeric_limits<double>::infinity()
                                   : static_cast<double>(MaxBaselineRatio::num) /
                                       static_cast<double>(MaxBaselineRatio::den));

      const double min_score =
        std::min_element(std::begin(scores), std::end(scores), [](const auto & a, const auto & b) {
          return a.second < b.second;
        })->second;

      const size_t partition = std::distance(std::begin(scores),
                                             std::partition(std::begin(scores),
                                                            std::end(scores),
                                                            [thresh](const auto & pair) {
                                                              return pair.second <= thresh;
                                                            }));
      ...
      std::transform(std::begin(scores),
  ...
  /// baseline = min(min_score, MaxBaselineRatio)
  /// normalized_score = score - baseline
  template <typename ThreshRatio      = std::ratio<13, 10>,  // treat neg numerator as +infty
  ...
  struct ExpRouletteSelector : public SelectorBase<RouletteCacheState> {
    ...
    ExpRouletteSelector(emp::Random & rand_) : rand(rand_) { ; }

    std::string name() const override {
      return emp::to_string("Exponential Roulette Selector (",
                            "ThreshRatio: ",
                            ThreshRatio::num,
                            "/",
                            ThreshRatio::den,
    ...
    RouletteCacheState operator()(emp::vector<std::pair < size_t, double> > scores,
      ...
      constexpr double b = (static_cast<double>(BRatio::num) / static_cast<double>(BRatio::den));
      emp_assert(b > 0 && b < 1);

      constexpr double c = (static_cast<double>(CRatio::num) / static_cast<double>(CRatio::den));
      emp_assert(c > 0);

      constexpr double z = (static_cast<double>(ZRatio::num) / static_cast<double>(ZRatio::den));
      emp_assert(z > 0);
      ...
      for (size_t p = 0; p < partition; ++p) {
        emp_assert(scores[p].second - baseline >= 0);
        match_index.Set(p, std::pow(b, std::pow(c * scores[p].second - baseline, z)));
      }
  ...
  struct SieveSelector : public SelectorBase<SieveCacheState> {
    ...
    SieveSelector(emp::Random & rand_) : rand(rand_) { ; }

    std::string name() const override {
      return emp::to_string("Sieve Selector (",
                            "LockInRatio: ",
                            LockInRatio::num,
                            "/",
                            LockInRatio::den,
    ...
    SieveCacheState operator()(emp::vector<std::pair < size_t, double> > scores,
                               size_t n) override {
      if (n == std::numeric_limits<size_t>::max()) { n = DefaultN; }
      ...
      constexpr double lock_in_raw =
        (static_cast<double>(LockInRatio::num) / static_cast<double>(LockInRatio::den));
      ...
      const double stochastic_raw =
        (static_cast<double>(StochasticRatio::num) / static_cast<double>(StochasticRatio::den));

      const double stochastic =
        stochastic_raw < 0.0 ? ((-1.0 * stochastic_raw) * lock_in) : stochastic_raw;

      const auto partition = std::partition(std::begin(scores),
                                            std::end(scores),
      ...
      std::transform(std::begin(scores),
                     ...
                     [lock_in, stochastic](const auto & pair) {
                       ...
                       const double intermediate =
                         stochastic ? std::max(0.0, (raw_score - lock_in) / stochastic) : 0.0;
      ...
      return SieveCacheState(uids, probabilities, rand, DefaultN);
    }
  };

}  // namespace emp

#endif  // #ifndef INCLUDE_EMP_MATCHING_MATCHBIN_SELECTORS_HPP_GUARD
