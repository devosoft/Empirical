...
namespace emp {
  ...
  template <class T>
  class memo_function;  // Not defined.
  ...
  public:
    ...
    return_t operator()(const A1 & k1, const A2 & k2, const EXTRA &... k_extra) {
      ...
      if (cache_it != cache_map.end()) { return cache_it->second; }
      return cache_map.emplace(std::make_tuple(k1, k2, k_extra...), fun(k1, k2, k_extra...))
        .first->second;
    }
    ...
    operator std::function<R(A1, A2, EXTRA...)>() const {
      return [this](A1 k1, A2 k2, EXTRA... k_extra) { return operator()(k1, k2, k_extra...); };
    }
  ...
  public:
    ...
    memo_function(T && fun_info)
      : cached_value(), has_cache(false), fun(std::forward<T>(fun_info)) {
      ;
    }
...
#endif  // #ifndef INCLUDE_EMP_FUNCTIONAL_MEMO_FUNCTION_HPP_GUARD
