...
public:
  struct Button {
    ...
    template <typename B> void press(bool pressed, B button) {
  ...
  public:
    ...
    Modifiers(bool shift = false, bool control = false, bool alt = false,
              bool super = false)
        : shift_set(shift), control_set(control), alt_set(alt),
          super_set(super) {}
#else
...
std::ostream &operator<<(std::ostream &out, const MouseEvent::Modifiers &mod) {
  if (mod.shift())
    out << "Shift";
  if (mod.control())
    out << "Ctrl";
...
std::ostream &operator<<(std::ostream &out, const MouseEvent::Button::Id &id) {
  switch (id) {
...
std::ostream &operator<<(std::ostream &out, const MouseEvent::Button &button) {
...
template <typename Fn> class Event {
private:
...
public:
  ...
  template <typename... U> void fire(U &&... args) {
    if (listeners.empty())
      return;
  ...
  template <typename... U> void operator()(U &&... args) {
    fire(std::forward<U>(args)...);
  }

  template <typename L> void bind(L listener) {
    listeners.emplace_back(listener);
  }

  template <typename L> bool unbind(const L &listener) {
    auto location =
        std::find_if(listeners.begin(), listeners.end(),
                     [listener = std::forward<L>(listener)](const auto &other) {
...
template <typename Fn> class EventHandle {
private:
  Event<Fn> *event;
...
public:
  ...
  template <typename L> bool unbind(L &&listener) {
...
public:
...
#endif
  {
    ...
    glfwSetFramebufferSizeCallback(window, [](GLFWwindow *window, int width,
                                              int height) {
      GLCanvas *me = static_cast<GLCanvas *>(glfwGetWindowUserPointer(window));
    ...
    glfwSetMouseButtonCallback(window, [](GLFWwindow *window, int b, int action,
                                          int mods) {
      GLCanvas *me = static_cast<GLCanvas *>(glfwGetWindowUserPointer(window));
      ...
      me->lastMouseEvent.modifiers = static_cast<MouseEvent::Modifiers>(mods);

      me->mouse_event.fire(*me, me->lastMouseEvent);

      me->lastMouseEvent.button.wasPressed = me->lastMouseEvent.button.pressed;
    });
#endif
    makeCurrent();
  ...
  template <BufferType TYPE> BufferObject<TYPE> makeBuffer() {
    GLuint handle;
