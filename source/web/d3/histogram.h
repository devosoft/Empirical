...
namespace D3 {
    ...
    public:
        Histogram(){
            EM_ASM_ARGS({js.objects[$0] = d3.histogram();}, this->id);
        };
        ...
        Histogram& SetDomain(double x, double y) {
            EM_ASM_ARGS({js.objects[$0].domain([$1, $2]);}, this->id, x, y);
            return (*this);
        }
        ...
        emp::array<double, 2> GetDomain() {
            EM_ASM_ARGS({emp_i.__outgoing_array = js.objects[$0].domain();}
                        , this->id);
        ...
        Histogram& SetThresholds(int count) {
            EM_ASM_ARGS({js.objects[$0].thresholds($1);}, this->id, count);
            return (*this);
        }
