...
public:
  ...
  FillRenderer(opengl::GLCanvas &canvas, S &&fill_shader = "DefaultSolidColor")
      : fill_shader(std::forward<S>(fill_shader)), vao(canvas.MakeVAO()),
  ...
  template <typename I = instance_attributes_type> void Instance(I &&attrs) {
...
public:
  ...
  TextRenderer(opengl::GLCanvas &canvas, F &&font, S &&shader = "DefaultFont")
      : vao(canvas.MakeVAO()),
  ...
  emp::math::Vec2f
  Measure(const std::string &text, float text_size,
  ...
  void Instance(const instance_attributes_type &attrs) {
    ...
    for (auto &c : attrs.GetText()) {
      ...
      if (info.bitmap_size.x() <= 0 || info.bitmap_size.y() <= 0)
        continue;
      ...
      vertices_buffer.PushData({{min.x(), min.y(), 0}, {tmin.x(), tmax.y()}});
      vertices_buffer.PushData({{max.x(), min.y(), 0}, {tmax.x(), tmax.y()}});

      vertices_buffer.PushData({{min.x(), max.y(), 0}, {tmin.x(), tmin.y()}});
      vertices_buffer.PushData({{max.x(), min.y(), 0}, {tmax.x(), tmax.y()}});

      vertices_buffer.PushData({{min.x(), max.y(), 0}, {tmin.x(), tmin.y()}});
      vertices_buffer.PushData({{max.x(), max.y(), 0}, {tmax.x(), tmin.y()}});
    }
    shader->Use();
    vao.bind();
...
public:
  ...
  Graphics(opengl::GLCanvas &canvas, F &&font,
           std::shared_ptr<scenegraph::Camera> camera,
           std::shared_ptr<scenegraph::Eye> eye)
      : fill_renderer(canvas), text_renderer(canvas, std::forward<F>(font)),
        camera(camera), eye(eye) {}
  ...
  Pen<FillRenderer> Fill(const Mesh &mesh) {
    return {
        &fill_renderer, {camera->GetProjection(), eye->CalculateView()}, mesh};
  }
  ...
  Pen<TextRenderer> Text() {
    return {&text_renderer, {camera->GetProjection(), eye->CalculateView()}};
  }
};
} // namespace graphics
} // namespace emp
