...
public:
  ...
  virtual emp::math::Vec<float, D> UpdateMinSize(const graphics::Graphics &g) {
    return {0};
  }

  // This should always be called soon before a call to render relative
  virtual emp::math::Vec<float, D>
  UpdateSize(const graphics::Graphics &g,
             const emp::math::Vec<float, D> &max_size) {
    return max_size;
  }

  virtual void
  RenderRelative(emp::graphics::Graphics &g, const math::Mat4x4f &transform,
                 const emp::math::Vec<float, D> &allocated_size) = 0;
};

namespace emp__impl_emp_scenegraph_core {

constexpr decltype(auto) GetMajorAxis(emp::math::Vec<float, 2> &dest,
                                      const emp::math::Vec<float, 2> &source) {}
} // namespace emp__impl_emp_scenegraph_core

template <size_t D> class Stack : public Node<D> {
private:
...
template <size_t> struct FlowDirection;

template <> struct FlowDirection<1> {
  enum { X } axis = X;
  bool reversed = false;
};
template <> struct FlowDirection<2> {
  enum { X, Y } axis = X;
  bool reversed = false;
};
template <> struct FlowDirection<3> {
  enum { X, Y, Z } axis = X;
  bool reversed = false;
};
...
constexpr emp::math::Region<float, 2>
GetMajorAxisRegion(const FlowDirection<2> &direction,
                   const emp::math::Region<float, 2> &max_region,
                   float major_start, float major_length) {
  auto min = max_region.min;
  auto max = max_region.max;

  if (direction.reversed) {
    GetMajorAxis(direction, max) -= major_start;
    GetMajorAxis(direction, min) = GetMajorAxis(direction, max) - major_length;
  } else {
    GetMajorAxis(direction, min) += major_start;
    GetMajorAxis(direction, max) = GetMajorAxis(direction, min) + major_length;
  }
...
template <size_t D>
constexpr emp::math::Vec<float, D>
GetSize(const FlowDirection<D> &direction,
...
template <size_t D> struct FlowMember {
...
template <size_t D> class Flow : public Node<D> {
private:
  bool expands;
...
public:
  ...
  emp::math::Vec<float, D> UpdateMinSize(const graphics::Graphics &g) override {
  ...
  UpdateSize(const graphics::Graphics &g,
    ...
    float major_axis_length =
        emp__impl_emp_scenegraph_core::GetMajorAxis(direction, free_space_size);
    ...
    for (auto &child : children) {
      if (child.node) {
        ...
        float min_space_length =
            child.weight < 0 ? 0 : emp__impl_emp_scenegraph_core::GetMajorAxis(
        ...
        emp__impl_emp_scenegraph_core::GetMajorAxis(direction, size) +=
            emp__impl_emp_scenegraph_core::GetMajorAxis(direction, child.size);
      }
    }
  ...
  void RenderRelative(emp::graphics::Graphics &g,
    ...
    emp__impl_emp_scenegraph_core::GetMajorAxis(direction, position) =
        emp__impl_emp_scenegraph_core::GetMajorAxis(direction, allocated_size);
...
template <size_t D> class Text : public scenegraph::Node<D> {
private:
  std::string text;
  float size;
