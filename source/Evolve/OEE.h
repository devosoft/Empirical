...
namespace emp {
    ...
    template <typename ORG, typename ORG_INFO=ORG, typename SKEL_TYPE=ORG_INFO, typename DATA_STRUCT=emp::datastruct::no_data>
    class OEETracker {
        private:
        ...
        using fun_calc_data_t = std::function<SKEL_TYPE(ORG_INFO&)>; // TODO: Allow other skeleton types
        ...
        void Update(size_t gen, int ud = -1) {
            if (Mod((int)gen, resolution) == 0) {
                ...
                for (auto tax : sys_active) {
                    active[i].taxon = tax;
                    ORG_INFO info = tax->GetInfo();
                    active[i].skel.New(skeleton_fun(info));
        ...
        void CalcStats(size_t ud) {
            ...
            for (auto & tax : coal_set) {
                // std::cout << "Evaluating org id: " << emp::to_string(tax.first) << ")" << std::endl;
                if (!Has(prev_coal_set, tax.first)) {
                    change++;
                    // std::cout << "Org id: " << emp::to_string(tax.first) << ") increased change" << std::endl;
                }
                if (!seen->contains(emp::to_string(tax.first))) {
                    novelty++;
                    // std::cout << "seen: ";
                    // for (int val : seen) {
                    //     std::cout << val << " ";
                    // }
                    // std::cout << std::endl;
                    seen->insert(emp::to_string(tax.first));
                    // std::cout << "Org id: " << emp::to_string(tax.first) << ") increased novelty" << std::endl;
                }
                double complexity = complexity_fun(tax.first);
                // std::cout << "Complexity: " << complexity << std::endl;
                if (complexity > most_complex) {
                    // std::cout << "Org id: " << emp::to_string(tax.first) << ") increased complextiy " << complexity << std::endl;
        ...
        std::map<SKEL_TYPE, int> CoalescenceFilter(size_t ud) {
            ...
            for ( snapshot_info_t & t : snapshots.front()) {
                if (Has(extant_canopy_roots, t.taxon)) {
                    ...
                    //     t.taxon->GetData().skeleton.Delete();
                    // }
                    // ORG_INFO i = t.taxon->GetInfo();
                    // t.taxon->GetData().skeleton.New(skeleton_fun(i));
    ...
    emp::vector<int> Skeletonize(BitVector & org, std::function<double(BitVector&)> fit_fun) {
        emp_assert(org.size() > 0, "Empty org passed to skeletonize");

        emp::vector<int> skeleton;
        double fitness = fit_fun(org);
        BitVector test_org = BitVector(org);
