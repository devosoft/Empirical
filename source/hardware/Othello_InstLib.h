...
namespace emp {
  ...
  struct AvidaCPU_InstLib : public InstLib<HARDWARE_T, ARG_T, ARG_COUNT> {
    ...
    static void Inst_SetBoard(hardware_t & hw, const inst_t & inst) {
      hw.SetBoard();
    }

    static void Inst_GetSquareCurr(hardware_t & hw, const inst_t & inst) {

      double out = hw.GetSquareCurr(int(hw.regs[inst.args[0]]));
      hw.regs[inst.args[1]] = out;
    }

    static void Inst_GetValidAbove(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidAbove(int(hw.regs[inst.args[0]]));
      //std::cout << "A: "<<out<<std::endl;
      hw.regs[inst.args[1]] = out;
    }

    static void Inst_GetValidBelow(hardware_t & hw, const inst_t & inst) {
      //std::cout<<"HERE"<<std::endl;
      double out = hw.GetValidBelow(int(hw.regs[inst.args[0]]));
      hw.regs[inst.args[1]] = out;
      //std::cout << "B: "<<out<<std::endl;
    }

    static void Inst_GetValidLeft(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidLeft(int(hw.regs[inst.args[0]]));
      hw.regs[inst.args[1]] = out;
      //std::cout << "L: "<<out<<std::endl;
    }

    static void Inst_GetValidRight(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidRight(int(hw.regs[inst.args[0]]));
      hw.regs[inst.args[1]] = out;
      //std::cout << "R: "<<out<<std::endl;
    }

    static void Inst_GetValidUL(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidUL(int(hw.regs[inst.args[0]]));
      hw.regs[inst.args[1]] = out;
      //std::cout << "UL: "<<out<<std::endl;
    }

    static void Inst_GetValidUR(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidUR(int(hw.regs[inst.args[0]]));
      hw.regs[inst.args[1]] = out;
      //std::cout << "UR: "<<out<<std::endl;
    }

    static void Inst_GetValidLL(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidLL(int(hw.regs[inst.args[0]]));
      //std::cout << "LL: "<<out<<std::endl;
      hw.regs[inst.args[1]] = out;
    }

    static void Inst_GetValidLR(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetValidLR(int(hw.regs[inst.args[0]]));
      //std::cout << "LR: "<<out<<std::endl;
      hw.regs[inst.args[1]] = out;
    }

    static void Inst_GetMem(hardware_t & hw, const inst_t & inst) {
      double out = hw.GetMem(inst.args[0], int(hw.regs[inst.args[1]]));
      hw.regs[inst.args[2]] = out;
      //std::cout<< "######### GETT Block: " << block << " Pos: " << int(posReg) << " Value: " << int(out) << std::endl;
    }

    static void Inst_SetMem(hardware_t & hw, const inst_t & inst) {
      hw.SetMem(inst.args[0], hw.regs[inst.args[1]], hw.regs[inst.args[2]]);
      //std::cout<< "SET Block: " << block << " Pos: " << int(posReg) << " Value: " << int(value) << std::endl;
    }

    static void Inst_CopyMem(hardware_t & hw, const inst_t & inst) {
      hw.CopyMem(inst.args[0], inst.args[1]);
      //std::cout<< "COPY Block To: " << block_to << " Block From: " << block_from << std::endl;
    }

    static void Inst_ShiftMem(hardware_t & hw, const inst_t & inst) {
      hw.ShiftMem(inst.args[0], hw.regs[inst.args[1]]);
    }
    ...
    static const this_t & DefaultInstLib() {
      ...
      if (inst_lib.GetSize() == 0) {
        ...
        inst_lib.AddInst("SetMem", Inst_SetMem, 3, "Put reg Arg3 into mem block Arg1 at position reg Arg2");//@OTHELLO
