...
#ifndef EMP_OTHELLO_GP_H
#define EMP_OTHELLO_GP_H
...
namespace emp {
  ...
  public:
    ...
    emp::array< std::unordered_map<int, double>, CPU_SIZE > mem; // @OTHELLO
    emp::vector<double> board; // @OTHELLO
  ...
  public:
    ...
    double GetMem(size_t block, int posReg) { //@OTHELLO Start
    ...
    double GetSquareCurr(int pos) { return board[pos % BOARD_SIZE]; } // TODO Is this ok?

    bool GetValidAbove(size_t pos) {
      int square = pos - 8;

      if (pos > 63 || board[pos] != EMPTY) {return 0;}
