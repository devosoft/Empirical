...
//
//  The template *must* define MEMBER to identify the type of organism in the population.
...
#define EMP_SETUP_EVO_WORLD_DEFAULT_ARGS(FUN_VAR, METHOD, RTYPE, ...)               \
  ...
  public:                                                                         \
  const std::function<RTYPE(MEMBER*,__VA_ARGS__)>& GetDefault ## METHOD ## Fun() const { \
    return FUN_VAR;                                                               \
  }                                                                               \
  void SetDefault ## METHOD ## Fun(const std::function<RTYPE(MEMBER*,__VA_ARGS__)>& f) { \
    FUN_VAR = f;                                                                  \
  }                                                                               \
  protected:
...
namespace evo {
  ...
  protected:
    ...
    EMP_SETUP_EVO_WORLD_DEFAULT_ARGS(default_placement_fun, PlaceOffspring, int, int)
  ...
  public:
    ...
    World(emp::Random & random, const std::string & pop_name="emp::evo::World")
      : World(pop_name) { random_ptr = &random; }
