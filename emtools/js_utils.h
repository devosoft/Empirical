...
//
// For example, say we have a templated function that takes a pointer to type T.
// We find out the appropriate string for type T:
...
// arrays containing all of the types defined in GetTypeToStringMap, which
...
void PassArrayToJavascript(std::array<T,SIZE> values,\
  ...
  //Nested arrays should be handled by a different version of this function
  //first.
...
template<std::size_t SIZE, typename T>
void PassArrayToJavascript(std::array<T,SIZE> values){
  PassArrayToJavascript(values, std::vector<int>(0));
}

//This version of the function handles nested arrays, by making recursive calls
...
void PassArrayToJavascript(std::array<std::array<T, SIZE1>, SIZE2> values,\
			   std::vector<int> recursive_el = std::vector<int>()) {
