mergeInto(LibraryManager.library, {
    $emp_kinetic: { objs:[], images:[], image_load_count:0, image_error_count:0, callbacks:{} },
    ...
    EMP_Tween_Build__deps: ['$emp_kinetic'],
    EMP_Tween_Build: function(target_id, seconds) {
        var settings_id = emp_kinetic.objs.length;
        emp_kinetic.objs[settings_id] = {
            node: emp_kinetic.objs[target_id],
            duration: seconds
        };  // Fill out configuration info.
        emp_kinetic.objs[settings_id+1] = 0; // Preserve for tween.
        return settings_id;        
    },

    EMP_Tween_Clone__deps: ['$emp_kinetic'],
    EMP_Tween_Clone: function(orig_settings_id) {
        var settings_id = emp_kinetic.objs.length;

        // Copy the original settings into the new structure.
        emp_kinetic.objs[settings_id] = {};
        for (var key in emp_kinetic.objs[orig_settings_id]) {
            (emp_kinetic.objs[settings_id])[ key ] = (emp_kinetic.objs[orig_settings_id])[ key ];
        }

        emp_kinetic.objs[settings_id+1] = 0; // Preserve for tween object.
        return settings_id;        
    },


    EMP_Rect_Build__deps: ['$emp_kinetic'],
    EMP_Rect_Build: function(_x, _y, _w, _h, _fill, _stroke, _stroke_width, _draggable) {
        ...
        emp_kinetic.objs[obj_id] = new Kinetic.Rect({       // Build the new rectangle!
            x: _x,
            y: _y,
            width: _w,
            height: _h,
    ...
    EMP_RegularPolygon_Build__deps: ['$emp_kinetic'],
    EMP_RegularPolygon_Build: function(_x, _y, _sides, _radius, _fill, _stroke, _stroke_width, _draggable) {
        var obj_id = emp_kinetic.objs.length;                   // Determine the next free id for a Kinetic object.
        _fill = Pointer_stringify(_fill);                    // Make sure string values are properly converted (colors)
        _stroke = Pointer_stringify(_stroke);
        emp_kinetic.objs[obj_id] = new Kinetic.RegularPolygon({ // Build the new regular polygon!
                x: _x,
                y: _y,
                sides: _sides,
                radius: _radius,
    ...
    EMP_Animation_Build__deps: ['$emp_kinetic'],
    EMP_Animation_Build: function(callback_ptr, layer_id) {
        var obj_id = emp_kinetic.objs.length;                   // Determine the next free id for a Kinetic object.
        emp_kinetic.objs[obj_id] = new Kinetic.Animation(function(frame) {
            ...
            Module._free(ptr);
        }, emp_kinetic.objs[layer_id]);
        return obj_id;
    },


    EMP_Animation_Build_NoFrame__deps: ['$emp_kinetic'],
    ...
    EMP_Custom_Shape_Build__deps: ['$emp_kinetic'],
    EMP_Custom_Shape_Build: function(_x, _y, _w, _h, draw_callback) {
        var obj_id = emp_kinetic.objs.length;                   // Determine the next free id for a Kinetic object
        emp_kinetic.objs[obj_id] = new Kinetic.Shape({          // Build the shape!
            x: _x,
            y: _y,
            width: _w,
            height: _h,
            drawFunc: function(canvas) {                     // For an arbitrary shape, we just have a draw function
                emp_kinetic.canvas = canvas;
                emp_kinetic.ctx = canvas._context;              // WTF??  This should be canvas.getContext();
                empJSDoCallback(draw_callback, 0);
                emp_kinetic.canvas = null;
