...
then
    ...
    echo "** Usage: ./g_script.sh run_names num_runs."
    exit 1
fi
...
# do setup
echo "Making fast-grid..."
make fast-grid &> /dev/null

echo "Updating list of runs..."
echo "${prefix}" >> runs/names

# grab config
head -n 25 ./grid-exp.cc | tail -n 8 > runs/${confname}
...
for ((i=0; i<"$2"; i++)); do
    ...
    ./grid-exp runs/${fname} && echo -e "\tDone with ${fname}" &
done
