...
template <typename ORG>
using PoolWorld = emp::evo::World<ORG, emp::evo::PopulationManager_Pools<ORG>>;
...
int main(int argc, char* argv[])
{
  ...
  if (argc == 1) { // program name, no prefix
    ...
    std::cerr << "** Usage: ./pool-test output-prefix";
  }
...
//  emp::evo::EAWorld<BitOrg> mixed_pop(random);
  ...
  emp::evo::StatsManager_DefaultStats<BitOrg, emp::evo::PopulationManager_Pools<BitOrg>>
      pools_stats (&pool_pop, prefix + "pool.csv");

  mixed_stats.fit_fun = fit_func;
  grid_stats.fit_fun = fit_func;
  pools_stats.fit_fun = fit_func;
...
//          std::cerr << "ZergRush" << std::endl;
  ...
  pool_pop.SetDefaultMutateFun( [](BitOrg* org, emp::Random& random) {
    bool mutated = false;    
      for (size_t site = 0; site < N; site++) {
        if (random.P(MUTATION_RATE)) {
          (*org)[site] = !(*org)[site];
...
//    std::cout << "," << emp::evo::AverageFitness(fit_func, mixed_pop) << std::endl;
    ...
    std::cout<<"Pop Size: "<<count<<std::endl;
    pool_pop.Update();
    pool_pop.MutatePop();
