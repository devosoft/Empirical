...
#define DEBUG_PRINT(X) std::cout << #X << " : " << emp::GetTypeID<X>().GetName() << '\n'
...
int main()
{
  ...
  std::cout << "test_t = " << emp::GetTypeID<test_t>().GetName() << '\n';

  std::cout << "Num types = " << test_t::GetSize() << '\n';
  std::cout << "float pos = " << test_t::GetID<float>() << '\n';

  std::cout << "Num types (with add) = " << test_t::add<long long>::GetSize() << '\n';
  std::cout << "float pos (with pop) = " << test_t::pop::GetID<float>() << '\n';

  using test2_t = emp::TypePackFill<int, 4>;
  using test3_t = emp::TypePack<uint64_t>;

  std::cout << '\n';
  std::cout << "Num test_t types:  " << test_t::GetSize() << '\n';
  std::cout << "Num test2_t types: " << test2_t::GetSize() << '\n';
  std::cout << "Num test3_t types: " << test3_t::GetSize() << '\n';
  ...
  std::cout << "Sum4(1,2,3,4) = " << fun(1,2,3,4) << '\n';

  using test4_t = test2_t::shrink<2>;
  std::cout << "After shrink<2>, size of type = " << test4_t::GetSize() << '\n';
  std::cout << "After merge of t1 and t2, size = "
	    << test_t::merge<test2_t>::GetSize() << '\n';
  ...
  std::cout << "Can we reverse a TypePack? -> " << str << '\n';

  // Let's try filtering!
  using test_filtered = test_t::filter<std::is_integral>;
  std::cout << "Number of integral types in test_t = " << test_filtered::GetSize() << '\n';


  using test_A = emp::TypePack<HasA, std::string, bool, HasA2, HasA, int>;
  std::cout << "test_A = " << emp::GetTypeID<test_A>().GetName() << '\n';

  using test_exist = test_A::filter<MemberA>;
  std::cout << "Number that have a member A = " << test_exist::GetSize() << '\n';
  ...
  std::cout << "\nMore filter tests...\n";
  std::cout << "emp::test_type<MemberA, int>() = " << emp::test_type<MemberA, int>() << '\n';
  std::cout << "emp::test_type<MemberA, HasA>() = " << emp::test_type<MemberA, HasA>() << '\n';
  std::cout << "emp::test_type<MemberA, HasA2>(); = " << emp::test_type<MemberA, HasA2>() << '\n';
  std::cout << "emp::test_type<std::is_integral, int>() = " << emp::test_type<std::is_integral, int>() << '\n';
  std::cout << "emp::test_type<std::is_integral, HasA>() = " << emp::test_type<std::is_integral, HasA>() << '\n';
  std::cout << "emp::test_type<std::is_integral, HasA2>() = " << emp::test_type<std::is_integral, HasA2>() << '\n';

  using wrap_v_t = test_t::wrap<std::vector>;
  std::cout << "wrap_v_t = " << emp::GetTypeID<wrap_v_t>().GetName() << '\n';
  using wrap_A_t = test_A::wrap<MemberA>;
  std::cout << "wrap_A_t = " << emp::GetTypeID<wrap_A_t>().GetName() << '\n';

  std::cout << '\n';
  using shuffle_t = test_t::select<2,3,4,1,3,3,3,0>;
  std::cout << "Shuffle with test_t::select<>:" << emp::GetTypeID<shuffle_t>().GetName() << '\n';
