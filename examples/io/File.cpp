...
int main()
{
  ...
  emp::vector<emp::String> first_col = spreadsheet.ExtractCol();
  std::cout << "\nAfter column is extracted:" << std::endl;
  spreadsheet.Write(std::cout);
  std::cout << "Extracted column: " << emp::MakeString(first_col) << std::endl;

  emp::vector<size_t> second_col = spreadsheet.ExtractColAs<size_t>();
  std::cout << "\nAfter another column is extracted as size_t:" << std::endl;
  spreadsheet.Write(std::cout);
  std::cout << "Extracted column: " << emp::MakeString(second_col) << std::endl;

  emp::vector<emp::String> first_row = spreadsheet.ExtractRow();
  std::cout << "\nAfter a row is extracted:" << std::endl;
  spreadsheet.Write(std::cout);
  std::cout << "Extracted row: " << emp::MakeString(first_row) << std::endl;

  emp::vector<std::size_t> second_row = spreadsheet.ExtractRowAs<size_t>();
  std::cout << "\nAfter a row is extracted as size_t:" << std::endl;
  spreadsheet.Write(std::cout);
  std::cout << "Extracted row: " << emp::MakeString(second_row) << std::endl;
  ...
  spreadsheet.Write(std::cout);
  std::cout << "Extracted data: " << emp::MakeString(full_data) << std::endl;
